# -*- coding: utf-8 -*-
"""
Literature Knowledge Graph Node Conformance Audit Script v8_post_disambiguation_conformance_audit.py
========================================================================
Functions:
  ...
  4. Graph structure conformance audit: check whether nested structures and property names are compliant
     - For algorithm-type nodes (15-19), additionally validate the node_importance field

Inputs:
  - Graph JSON: A1-node_merged_disambiguated/[...].json (post-disambiguation output)
...
Node field spec:
  Every node has (11 fields): node_id, node_type, node_original_name, node_name,
  node_description, node_num, node_cite_score, node_cite_count, node_weight,
  node_algorithm_class, node_id_list
  Algorithm-type nodes (15-19) additionally have: node_importance (3-from-1: "最高重要性"/"一般重要性"/"Not Mentioned")
"""
...
INPUT_GRAPH_PATH = r"./output/final_merged/A1-node_merged_disambiguated/[2277EAKD][ZZZRPFBV]_merged_nodes_conformance_audit_merged_conformance_audit_disambiguated.json"
...
def audit_graph_structure(papers: list[dict]) -> list[dict]:
        ...
        "nodes": [
          {
            ...
            "node_num": <value>,
            "node_cite_score": <value>,
            "node_cite_count": <value>,
            "node_weight": <value>,
            "node_algorithm_class": <value>,
            "node_id_list": <value>,
            # The following fields are optional / conditionally required:
            # - node_importance: required only for algorithm-type nodes (15-19) and must be one of "最高重要性" / "一般重要性" / "Not Mentioned"; may be omitted or null for non-algorithm nodes
          },
          ...
        ]
      },
    ...
    Audit contents:
      ...
      3) Algorithm-type nodes (15-19) must contain the node_importance field with a value of one of "最高重要性" / "一般重要性" / "Not Mentioned"
    ...
    # Required second-level fields for each item in nodes[] (non-algorithm nodes)
    NODE_REQUIRED_FIELDS = {
        ...
        "node_cite_count",
        "node_weight",
        "node_algorithm_class",
        "node_id_list",
    }

    # Additional required field for algorithm-type nodes (15-19) and its legal values
    ALGO_NODE_TYPES = {15, 16, 17, 18, 19}
    VALID_NODE_IMPORTANCE = {"最高重要性", "一般重要性", "Not Mentioned"}
    ...
    for paper in papers:
        ...
        if not isinstance(nodes, list):
            ...
            for idx, node in enumerate(nodes):
                ...
                # 2) Are the second-level fields complete? (NODE_REQUIRED_FIELDS are the core required fields)
                ...
                # 3) For algorithm-type nodes (15-19), additionally check node_importance
                node_type_str = node.get("node_type", "")
                is_algo_node = False
                import_match = re.match(r'^(\d+)-', node_type_str)
                if import_match:
                    type_num = int(import_match.group(1))
                    if type_num in ALGO_NODE_TYPES:
                        is_algo_node = True

                if is_algo_node:
                    # The node_importance field must exist
                    if "node_importance" not in node:
                        issues.append(
                            f"nodes[{idx}] (node_id={node_id_val}, node_type={node_type_str}) "
                            f"is an algorithm-type node; missing the node_importance field"
                        )
                    else:
                        imp_val = node.get("node_importance")
                        # The value can be null (initial state) or one of the legal three-from-one strings
                        if imp_val is not None and imp_val not in VALID_NODE_IMPORTANCE:
                            issues.append(
                                f"nodes[{idx}] (node_id={node_id_val}, node_type={node_type_str}) "
                                f"node_importance value is invalid: '{imp_val}'; expected one of {{\"最高重要性\", \"一般重要性\", \"Not Mentioned\"}}"
                            )
                else:
                    # Non-algorithm-type nodes should not have node_importance (or the value should be null)
                    if "node_importance" in node and node["node_importance"] is not None:
                        issues.append(
                            f"nodes[{idx}] (node_id={node_id_val}, node_type={node_type_str}) "
                            f"is a non-algorithm-type node; node_importance should be null, but is '{node['node_importance']}'"
                        )

                # 4) Extra second-level fields (informational only, not a violation)
                extra_node_fields = set(node.keys()) - NODE_REQUIRED_FIELDS - {"node_importance"}
    ...
    print(f"  Checked graph case_id count: {total_papers}")
