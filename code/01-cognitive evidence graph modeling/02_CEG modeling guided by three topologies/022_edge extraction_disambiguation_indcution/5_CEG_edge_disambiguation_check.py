...
Features:
  Perform 5 categories of conformance audits on the JSON produced by "edge merge":
    ...
    2. Node node_name conformance audit (N-of-1 rule): strictly match the node_name
       of source/target nodes; non-compliant ones are reassigned via LLM; still
       non-compliant after LLM are forced to N-of-1 by program.
    3. Edge completeness audit: completeness audits for 01-default edges, 02-causal edges,
       02-causal edges supplement, 03-evidence edges; invalid case_ids are written to the audit log md file.
    4. Graph-structure conformance audit: check whether the nested structure and property names are compliant.
    5. Output the audited graph JSON to the same directory as the input, with the suffix "_conformance_audit".

Inputs:
  1. Edge-graph JSON:
     ./data/03_induction/B1-edges_disambiguation/225KHNN8+KC8MEE2V+merged-edges_audit_merged_audit_secondary_audit_disambiguated.json   <-- RELATIVE PATH placeholder
...
Outputs:
  - Audited graph JSON: <input_filename>_conformance_audit.json
    Location (explicitly configured in OUTPUT_DIR):
    ./data/03_induction/B1-edges_disambiguation/   <-- RELATIVE PATH placeholder
  - Audit log MD: <input_filename>_conformance_audit.md (same directory as output JSON), recording:
...
INPUT_GRAPH_PATH = (
    ...
    r"./data/03_induction/B1-edges_disambiguation/225KHNN8+KC8MEE2V+merged-edges_audit_merged_audit_secondary_audit_disambiguated.json"
)
...
PROMPT_FILES = [
    # NOTE: relative path placeholders for code review / publication
...
# --- Output directory (explicitly configured, easy to modify later) ---
# Output file is saved under this directory, file name is generated by appending a fixed suffix to the input file name:
#   - Audited graph JSON: <input_filename>_conformance_audit.json
#   - Audit log MD: <input_filename>_conformance_audit.md
# (The input file name already ends with "_disambiguation", the suffix does not repeat this marker.)
OUTPUT_DIR = (
    # NOTE: relative path placeholder for code review / publication
    r"./data/03_induction/B1-edges_disambiguation"
)

# --- Output file suffix (shared by JSON and MD, file type distinguished by extension) ---
# The input file name already ends with "_disambiguation", the suffix no longer needs to repeat "disambiguation".
OUTPUT_SUFFIX = "_conformance_audit"

# --- LLM configuration (consistent with v7_merge_conformance_audit.py) ---
...
# ============================================================================
# N-of-1 rules (extracted from Prompt md, written into the program)
# ============================================================================
...
def extract_n_from_llm_response(text: str, options: set[str]) -> Optional[str]:
    ...
    text = text.strip()
    # 1) Try exact whole-text match first
    for opt in options:
        if text == opt:
            return opt
    # 2) Find any fully contained option in the text
    for opt in options:
        if opt in text:
            return opt
    return None
...
def load_json(path: str) -> list:
    """Load a JSON file whose outermost element is a list."""
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, list):
        raise ValueError(f"Input JSON outermost is not a list, but {type(data).__name__}: {path}")
    return data
...
def build_output_path(input_path: str, suffix: str = OUTPUT_SUFFIX) -> str:
    """
    Generate output path based on input file name + the explicit OUTPUT_DIR.

    The output directory uses the explicit OUTPUT_DIR configured at the top (no longer
    the same directory as input), file name = <input basename without extension> + suffix
    + original extension.
    """
...
def _build_case_line_map_from_text(json_text: str) -> dict[str, int]:
    """
    Parse each top-level case_id object's start line number (1-based) in the file
    from the formatted JSON text.

    Implementation:
      1) Iterate the entire json_text and build a lookup table (char offset -> line number);
      2) Use json.JSONDecoder().raw_decode to sequentially parse each top-level object,
         obtain its start char offset, and convert to line number via the lookup table.
    """
    # 1) char offset -> 1-based line number lookup table
    # line_starts[i] represents the start char offset of line (i+1)
    ...
    def pos_to_line(p: int) -> int:
        # Binary search: find the largest line_starts[i] <= p
    ...
    # 2) sequentially parse each top-level object
    while idx < n:
...
def audit_null_values(papers: list[dict]) -> tuple[list[dict], dict]:
    """
    Iterate each edge, for source_node_original_name/source_node_name,
    target_node_original_name/target_node_name, delete null-value edges.

    Judgment rules (per user requirements):
      1) source: "not mentioned" + null
      2) source: "" + ""
      3) source: null + null
      4) source: "not mentioned" + null  (fully duplicate of 1), retained per user original)
      5) source: "" + ""                (fully duplicate of 2), retained per user original)
      6) source: null + null            (fully duplicate of 3), retained per user original)
      -- and the same rules applied to target.

    Returns:
      (updated papers, statistics dict)
      statistics dict: { edge_group: {"null_edges": N, "deleted_edges": N1, "status": str}, ... }
    """
    print("\n" + "=" * 70)
...
def _llm_correct_edge(
    ...
    """Call the LLM to re-assign non-compliant node_name via N-of-1 selection."""
    prompt = _build_llm_prompt_edge(
        side, node_id, node_type, node_name, node_original_name, options
    )
    messages = [
        {"role": "system", "content": "You are a strict knowledge graph node classification assistant."},
...
def _program_correct_edge(node_name: str, node_original_name: str, options: set[str]) -> Optional[str]:
    """Program-layer keyword-based fallback N-of-1 (edge node version)."""
...
def audit_node_name_for_edges(papers: list[dict]) -> tuple[list[dict], dict]:
    """
    Perform node_name conformance audit (N-of-1) on the source/target nodes of
    each edge in the edge graph.

    Note: edge node naming uses prefixes: "source_node_type"/"source_node_name"/
    "source_node_original_name" and "target_*".

    Returns:
      (updated papers, statistics dictionary)
      statistics dictionary: { node_type: {"total": N2, "non_compliant": N3, "compliant": N4,
            ...
            for side in ("source", "target"):
                ...
                if llm_fixed is not None:
                    ...
                    if prog_fixed is None:
                        # final fallback: take the first option
    ...
    # Console output (grouped by node_type; only aggregate node_types that have N-of-1 rules)
    for nt in sorted(stats.keys()):
        s = stats[nt]
        if s["total"] == 0:
            continue
        print(
            f"node_type: {nt}. Total node_name count: {s['total']}, "
            f"non-compliant node_name count: {s['non_compliant']}, "
            f"compliant node_name count: {s['compliant']}, "
            f"node_name fixed by LLM: {s['llm_fixed']}, "
            f"node_name fixed by program fallback after LLM failure: {s['program_fixed']}."
        )
...
def _get_node_type_group(paper: dict) -> set[str]:
    """Collect source/target node_type deduplicated set from all edges of the paper."""
    ...
    # Also check the nodes[] field (although the edge graph's nodes is usually [], keep as fallback)
...
def _filter_full_by_node_type_group(
    ...
    """For preset rules of the form (source, target) or (source, target, edge_type),
    filter using node_type_group: all strings in a candidate tuple must appear in
    node_type_group to be considered valid; otherwise the candidate is removed
    (because the case does not have the corresponding node_type, and the candidate
    edge does not apply to this case).

    Note: edge_type is in English (e.g. 'contains'), which is typically not in
    node_type_group; such non-node_type string fields are automatically skipped.
    """
    # Known edge_type set (those appearing in the preset rules), which should not
    # participate in the node_type_group filtering.
    EDGE_TYPES: set[str] = {
    ...
    for tup in full_list:
        # Extract node_type fields (i.e. not edge_type)
...
def audit_edge_completeness(papers: list[dict]) -> tuple[list[dict], dict]:
    ...
    Edge completeness audit:
      - 01-default edge: requires each case's 01-default edge set == the "01-full-set" filtered by node_type_group
      - 02-causal edge: requires each case's 02-causal edge set ⊇ the "02-full-set" filtered by node_type_group
      - 02-causal edge supplement: requires each case's 02-causal edge supplement set ⊇ at least 1 of the "02-full-set-supplement"
      - 03-evidence edge: requires each case's 03-evidence edge set ⊇ at least 1 of the "03-full-set"
    """
    print("\n" + "=" * 70)
...
def run():
    ...
    # audited graph JSON
    output_json_path = build_output_path(INPUT_GRAPH_PATH)
    ...
    # audit log md: same directory as output JSON (OUTPUT_DIR), same file name suffix, extension .md
    log_md_path = build_output_path(INPUT_GRAPH_PATH).replace(".json", ".md")
