...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `3LHABXRM_E171` | `motivates` | 09-Problem Scenario | Noise robustness(Uncertainty) |  | 19-Training Optimization Algorithm | Grid Search |  |
| 2 | `3LHABXRM_E172` | `motivates` | 12-Training Data Availability | sampling rate is 200 S/s, and the sampling time is 20 s(Sufficient) |  | 18-Data Generation Algorithm | AMESim-Simulink Joint Simulation |  |
| 3 | `3LHABXRM_E173` | `motivates` | 12-Training Data Availability | sampling rate is 200 S/s, and the sampling time is 20 s(Sufficient) |  | 19-Training Optimization Algorithm | Grid Search |  |
| 4 | `3LHABXRM_E174` | `motivates` | 13-Noise Level | corrupted by white noise with signal-noise ratio to be 20 dB(High Noise) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 5 | `3LHABXRM_E175` | `motivates` | 13-Noise Level | corrupted by white noise with signal-noise ratio to be 20 dB(High Noise) |  | 16-Feature Extraction Algorithm | PCA |  |
| 6 | `3LHABXRM_E176` | `motivates` | 09-Problem Scenario | Noise robustness(Uncertainty) |  | 18-Data Generation Algorithm | AMESim-Simulink Joint Simulation |  |
| 7 | `3LHABXRM_E177` | `motivates` | 09-Problem Scenario | Noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 8 | `3LHABXRM_E178` | `motivates` | 09-Problem Scenario | Noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | PCA |  |
| 9 | `3LHABXRM_E179` | `motivates` | 09-Problem Scenario | Noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | SVM |  |
| 10 | `3LHABXRM_E180` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | PCA |  |
| 11 | `3LHABXRM_E181` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | SVM |  |
| 12 | `3LHABXRM_E182` | `motivates` | 06-Fault Severity | FMC gain = 0, Signal output = 0, equivalent orifice diameter = 5, Gain = 15(Single Severity) |  | 17-Core Classifier Algorithm | SVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `3LHABXRM_E171`：Noise robustness(Uncertainty) motivates Grid Search
  - `3LHABXRM_E176`：Noise robustness(Uncertainty) motivates AMESim-Simulink Joint Simulation
  - `3LHABXRM_E177`：Noise robustness(Uncertainty) motivates Normalization
  - `3LHABXRM_E178`：Noise robustness(Uncertainty) motivates PCA
  - `3LHABXRM_E179`：Noise robustness(Uncertainty) motivates SVM
