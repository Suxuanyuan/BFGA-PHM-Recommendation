...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `Z993LHZQ_E092` | `motivates` | 13-Noise Level | commonly contaminated by heavy noises under the working environment(High Noise) |  | 15-Data Preprocessing Algorithm | Band-pass filter |  |
| 2 | `Z993LHZQ_E093` | `motivates` | 13-Noise Level | commonly contaminated by heavy noises under the working environment(High Noise) |  | 16-Feature Extraction Algorithm | Minimum entropy deconvolution |  |
| 3 | `Z993LHZQ_E094` | `motivates` | 09-Problem Scenario | contaminated by heavy noises(Uncertainty) |  | 15-Data Preprocessing Algorithm | Band-pass filter |  |
| 4 | `Z993LHZQ_E095` | `motivates` | 09-Problem Scenario | impact-induced natural periodic impulses(Other) |  | 15-Data Preprocessing Algorithm | Band-pass filter |  |
| 5 | `Z993LHZQ_E096` | `motivates` | 09-Problem Scenario | contaminated by heavy noises(Uncertainty) |  | 16-Feature Extraction Algorithm | Minimum entropy deconvolution |  |
| 6 | `Z993LHZQ_E097` | `motivates` | 09-Problem Scenario | impact-induced natural periodic impulses(Other) |  | 16-Feature Extraction Algorithm | Minimum entropy deconvolution |  |
| 7 | `Z993LHZQ_E098` | `motivates` | 09-Problem Scenario | contaminated by heavy noises(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope spectrum analysis |  |
| 8 | `Z993LHZQ_E099` | `motivates` | 09-Problem Scenario | impact-induced natural periodic impulses(Other) |  | 17-Core Classifier Algorithm | Envelope spectrum analysis |  |
| 9 | `Z993LHZQ_E100` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Minimum entropy deconvolution |  |
| 10 | `Z993LHZQ_E101` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Envelope spectrum analysis |  |
| 11 | `Z993LHZQ_E102` | `motivates` | 06-Fault Severity | 0.5 mm width, 0.5 mm depth(Single Severity) |  | 17-Core Classifier Algorithm | Envelope spectrum analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `Z993LHZQ_E094`：contaminated by heavy noises(Uncertainty) motivates Band-pass filter
  - `Z993LHZQ_E095`：impact-induced natural periodic impulses(Other) motivates Band-pass filter
  - `Z993LHZQ_E096`：contaminated by heavy noises(Uncertainty) motivates Minimum entropy deconvolution
  - `Z993LHZQ_E097`：impact-induced natural periodic impulses(Other) motivates Minimum entropy deconvolution
  - `Z993LHZQ_E098`：contaminated by heavy noises(Uncertainty) motivates Envelope spectrum analysis
  - `Z993LHZQ_E099`：impact-induced natural periodic impulses(Other) motivates Envelope spectrum analysis
