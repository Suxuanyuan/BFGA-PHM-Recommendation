...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `NMKCY68M_E126` | `motivates` | 13-Noise Level | heavy noise, noisy environment, large vibration noise(High Noise) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 2 | `NMKCY68M_E127` | `motivates` | 13-Noise Level | heavy noise, noisy environment, large vibration noise(High Noise) |  | 16-Feature Extraction Algorithm | LTSA |  |
| 3 | `NMKCY68M_E128` | `motivates` | 09-Problem Scenario | noise robustness / residual noise suppression(Uncertainty) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 4 | `NMKCY68M_E129` | `motivates` | 09-Problem Scenario | incipient fault / early stage of damage propagation(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 5 | `NMKCY68M_E130` | `motivates` | 09-Problem Scenario | noise robustness / residual noise suppression(Uncertainty) |  | 16-Feature Extraction Algorithm | LTSA |  |
| 6 | `NMKCY68M_E131` | `motivates` | 09-Problem Scenario | incipient fault / early stage of damage propagation(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | LTSA |  |
| 7 | `NMKCY68M_E132` | `motivates` | 09-Problem Scenario | noise robustness / residual noise suppression(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope Demodulation |  |
| 8 | `NMKCY68M_E133` | `motivates` | 09-Problem Scenario | incipient fault / early stage of damage propagation(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Envelope Demodulation |  |
| 9 | `NMKCY68M_E134` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | LTSA |  |
| 10 | `NMKCY68M_E135` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Envelope Demodulation |  |
| 11 | `NMKCY68M_E136` | `motivates` | 06-Fault Severity | 0.5 mm slit width, wearing fault at the sixth cycle(Single Severity) |  | 17-Core Classifier Algorithm | Envelope Demodulation |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `NMKCY68M_E128`：noise robustness / residual noise suppression(Uncertainty) motivates EMD
  - `NMKCY68M_E129`：incipient fault / early stage of damage propagation(Early Degradation Prediction) motivates EMD
  - `NMKCY68M_E130`：noise robustness / residual noise suppression(Uncertainty) motivates LTSA
  - `NMKCY68M_E131`：incipient fault / early stage of damage propagation(Early Degradation Prediction) motivates LTSA
  - `NMKCY68M_E132`：noise robustness / residual noise suppression(Uncertainty) motivates Envelope Demodulation
  - `NMKCY68M_E133`：incipient fault / early stage of damage propagation(Early Degradation Prediction) motivates Envelope Demodulation
