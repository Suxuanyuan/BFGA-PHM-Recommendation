...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `5L5QPGV5_E068` | `motivates` | 13-Noise Level | noise, low SNR, background environment interference, colored noise(High Noise) |  | 15-Data Preprocessing Algorithm | Wavelet Packet Transform |  |
| 2 | `5L5QPGV5_E069` | `motivates` | 13-Noise Level | noise, low SNR, background environment interference, colored noise(High Noise) |  | 16-Feature Extraction Algorithm | Teager Energy Spectral Kurtosis |  |
| 3 | `5L5QPGV5_E070` | `motivates` | 09-Problem Scenario | Sensitivity to noise and non-periodic transient impulses(Uncertainty) |  | 15-Data Preprocessing Algorithm | Wavelet Packet Transform |  |
| 4 | `5L5QPGV5_E071` | `motivates` | 09-Problem Scenario | Sensitivity to noise and non-periodic transient impulses(Uncertainty) |  | 16-Feature Extraction Algorithm | Teager Energy Spectral Kurtosis |  |
| 5 | `5L5QPGV5_E072` | `motivates` | 09-Problem Scenario | Sensitivity to noise and non-periodic transient impulses(Uncertainty) |  | 17-Core Classifier Algorithm | Generalized Cross-Correlation |  |
| 6 | `5L5QPGV5_E073` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Teager Energy Spectral Kurtosis |  |
| 7 | `5L5QPGV5_E074` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Generalized Cross-Correlation |  |
| 8 | `5L5QPGV5_E075` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Generalized Cross-Correlation |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `5L5QPGV5_E070`：Sensitivity to noise and non-periodic transient impulses(Uncertainty) motivates Wavelet Packet Transform
  - `5L5QPGV5_E071`：Sensitivity to noise and non-periodic transient impulses(Uncertainty) motivates Teager Energy Spectral Kurtosis
  - `5L5QPGV5_E072`：Sensitivity to noise and non-periodic transient impulses(Uncertainty) motivates Generalized Cross-Correlation
