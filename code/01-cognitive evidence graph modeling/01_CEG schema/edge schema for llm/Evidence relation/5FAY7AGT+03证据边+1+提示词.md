...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `5FAY7AGT_E095` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 19-Training Optimization Algorithm | Majorization-Minimization |  |
| 2 | `5FAY7AGT_E096` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 19-Training Optimization Algorithm | Majorization-Minimization |  |
| 3 | `5FAY7AGT_E097` | `motivates` | 12-Training Data Availability | Sufficient |  | 19-Training Optimization Algorithm | Majorization-Minimization |  |
| 4 | `5FAY7AGT_E098` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 16-Feature Extraction Algorithm | Sparse Multiperiod Group Lasso |  |
| 5 | `5FAY7AGT_E099` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 16-Feature Extraction Algorithm | Sparse Multiperiod Group Lasso |  |
| 6 | `5FAY7AGT_E100` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 16-Feature Extraction Algorithm | Sparse Multiperiod Group Lasso |  |
| 7 | `5FAY7AGT_E101` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 8 | `5FAY7AGT_E102` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 9 | `5FAY7AGT_E103` | `motivates` | 07-Compound Fault | compound faults(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Sparse Multiperiod Group Lasso |  |
| 10 | `5FAY7AGT_E104` | `motivates` | 07-Compound Fault | compound faults(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 11 | `5FAY7AGT_E105` | `motivates` | 06-Fault Severity | run-to-failure(Multiple Severities) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `5FAY7AGT_E095`：compound faults(Compound Faults) motivates Majorization-Minimization
  - `5FAY7AGT_E096`：heavy background noise(Uncertainty) motivates Majorization-Minimization
  - `5FAY7AGT_E099`：compound faults(Compound Faults) motivates Sparse Multiperiod Group Lasso
  - `5FAY7AGT_E100`：heavy background noise(Uncertainty) motivates Sparse Multiperiod Group Lasso
  - `5FAY7AGT_E101`：compound faults(Compound Faults) motivates Envelope Analysis
  - `5FAY7AGT_E102`：heavy background noise(Uncertainty) motivates Envelope Analysis
