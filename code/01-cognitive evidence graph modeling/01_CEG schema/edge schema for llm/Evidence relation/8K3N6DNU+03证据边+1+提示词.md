...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `8K3N6DNU_E105` | `motivates` | 13-Noise Level | slightly noisy, for the low frequencies / decreasing the signal to noise ratio(High Noise) |  | 15-Data Preprocessing Algorithm | Zero-phase Butterworth filter |  |
| 2 | `8K3N6DNU_E106` | `motivates` | 13-Noise Level | slightly noisy, for the low frequencies / decreasing the signal to noise ratio(High Noise) |  | 16-Feature Extraction Algorithm | Impedance angle extraction |  |
| 3 | `8K3N6DNU_E107` | `motivates` | 09-Problem Scenario | change in the impedance spectrum due to degradation(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | Zero-phase Butterworth filter |  |
| 4 | `8K3N6DNU_E108` | `motivates` | 09-Problem Scenario | change in the impedance spectrum due to degradation(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | Impedance angle extraction |  |
| 5 | `8K3N6DNU_E109` | `motivates` | 09-Problem Scenario | change in the impedance spectrum due to degradation(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Artificial Neural Network |  |
| 6 | `8K3N6DNU_E110` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Impedance angle extraction |  |
| 7 | `8K3N6DNU_E111` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Artificial Neural Network |  |
| 8 | `8K3N6DNU_E112` | `motivates` | 06-Fault Severity | degradation(Multiple Severities) |  | 17-Core Classifier Algorithm | Artificial Neural Network |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `8K3N6DNU_E107`：change in the impedance spectrum due to degradation(Distribution Discrepancy) motivates Zero-phase Butterworth filter
  - `8K3N6DNU_E108`：change in the impedance spectrum due to degradation(Distribution Discrepancy) motivates Impedance angle extraction
  - `8K3N6DNU_E109`：change in the impedance spectrum due to degradation(Distribution Discrepancy) motivates Artificial Neural Network
