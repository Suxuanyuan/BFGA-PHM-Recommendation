...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `KJAK9E8Y_E111` | `motivates` | 13-Noise Level | Vibration signals collected from real industry are often noised(High Noise) |  | 15-Data Preprocessing Algorithm | Overlapping sampling |  |
| 2 | `KJAK9E8Y_E112` | `motivates` | 13-Noise Level | Vibration signals collected from real industry are often noised(High Noise) |  | 16-Feature Extraction Algorithm | DMCNet |  |
| 3 | `KJAK9E8Y_E113` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | Overlapping sampling |  |
| 4 | `KJAK9E8Y_E114` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | DMCNet |  |
| 5 | `KJAK9E8Y_E115` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax |  |
| 6 | `KJAK9E8Y_E116` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | DMCNet |  |
| 7 | `KJAK9E8Y_E117` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `KJAK9E8Y_E118` | `motivates` | 06-Fault Severity | worn tooth (level 1-5)(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `KJAK9E8Y_E113`：noise robustness(Uncertainty) motivates Overlapping sampling
  - `KJAK9E8Y_E114`：noise robustness(Uncertainty) motivates DMCNet
  - `KJAK9E8Y_E115`：noise robustness(Uncertainty) motivates Softmax
