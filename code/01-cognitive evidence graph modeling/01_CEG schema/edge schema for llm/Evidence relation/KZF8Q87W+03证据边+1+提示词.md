...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `KZF8Q87W_E079` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | Dynamic Joint Distribution Alignment |  |
| 2 | `KZF8Q87W_E080` | `motivates` | 09-Problem Scenario | uncertainty of pseudo labels(Uncertainty) |  | 19-Training Optimization Algorithm | Dynamic Joint Distribution Alignment |  |
| 3 | `KZF8Q87W_E081` | `motivates` | 12-Training Data Availability | 1400 samples per domain in Case I, 2000 samples per domain in Case II(Sufficient) |  | 19-Training Optimization Algorithm | Dynamic Joint Distribution Alignment |  |
| 4 | `KZF8Q87W_E082` | `motivates` | 13-Noise Level | unspecified(Normal) |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 5 | `KZF8Q87W_E083` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 6 | `KZF8Q87W_E084` | `motivates` | 09-Problem Scenario | uncertainty of pseudo labels(Uncertainty) |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 7 | `KZF8Q87W_E085` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `KZF8Q87W_E086` | `motivates` | 09-Problem Scenario | uncertainty of pseudo labels(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `KZF8Q87W_E087` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 10 | `KZF8Q87W_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `KZF8Q87W_E089` | `motivates` | 06-Fault Severity | defect width(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `KZF8Q87W_E079`：domain shift(Distribution Discrepancy) motivates Dynamic Joint Distribution Alignment
  - `KZF8Q87W_E080`：uncertainty of pseudo labels(Uncertainty) motivates Dynamic Joint Distribution Alignment
  - `KZF8Q87W_E083`：domain shift(Distribution Discrepancy) motivates ResNet-50
  - `KZF8Q87W_E084`：uncertainty of pseudo labels(Uncertainty) motivates ResNet-50
  - `KZF8Q87W_E085`：domain shift(Distribution Discrepancy) motivates Softmax
  - `KZF8Q87W_E086`：uncertainty of pseudo labels(Uncertainty) motivates Softmax
