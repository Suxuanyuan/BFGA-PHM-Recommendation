...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `PDECHWK4_E095` | `motivates` | 13-Noise Level | high frequency noises(High Noise) |  | 16-Feature Extraction Algorithm | Fast Fourier Transform |  |
| 2 | `PDECHWK4_E096` | `motivates` | 09-Problem Scenario | Ground fault detection across different stages of variable speed drives(Complex Systems) |  | 16-Feature Extraction Algorithm | Fast Fourier Transform |  |
| 3 | `PDECHWK4_E097` | `motivates` | 09-Problem Scenario | High ground fault resistance(Other) |  | 16-Feature Extraction Algorithm | Fast Fourier Transform |  |
| 4 | `PDECHWK4_E098` | `motivates` | 09-Problem Scenario | Ground fault detection across different stages of variable speed drives(Complex Systems) |  | 17-Core Classifier Algorithm | Threshold Decision |  |
| 5 | `PDECHWK4_E099` | `motivates` | 09-Problem Scenario | High ground fault resistance(Other) |  | 17-Core Classifier Algorithm | Threshold Decision |  |
| 6 | `PDECHWK4_E100` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Fast Fourier Transform |  |
| 7 | `PDECHWK4_E101` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Threshold Decision |  |
| 8 | `PDECHWK4_E102` | `motivates` | 06-Fault Severity | ground fault resistance (Rf)(Multiple Severities) |  | 17-Core Classifier Algorithm | Threshold Decision |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `PDECHWK4_E096`：Ground fault detection across different stages of variable speed drives(Complex Systems) motivates Fast Fourier Transform
  - `PDECHWK4_E097`：High ground fault resistance(Other) motivates Fast Fourier Transform
  - `PDECHWK4_E098`：Ground fault detection across different stages of variable speed drives(Complex Systems) motivates Threshold Decision
  - `PDECHWK4_E099`：High ground fault resistance(Other) motivates Threshold Decision
