...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `R6BR7SNQ_E110` | `motivates` | 09-Problem Scenario | large distribution differences between the training data and testing data(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | MK-MMD |  |
| 2 | `R6BR7SNQ_E111` | `motivates` | 12-Training Data Availability | Typically, 10000 samples of 2000 sample lengths are selected from source domain and target domain, respectively. And then, the spectra of the vibration signals are calculated by FFT. ... we use half for training the model and the other half for testing.(Sufficient) |  | 19-Training Optimization Algorithm | MK-MMD |  |
| 3 | `R6BR7SNQ_E112` | `motivates` | 13-Noise Level | diesel engine makes a lot of noise during operation, so the data collected in this case is the most convincing to verify the accuracy of the model(High Noise) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 4 | `R6BR7SNQ_E113` | `motivates` | 13-Noise Level | diesel engine makes a lot of noise during operation, so the data collected in this case is the most convincing to verify the accuracy of the model(High Noise) |  | 16-Feature Extraction Algorithm | FFNN |  |
| 5 | `R6BR7SNQ_E114` | `motivates` | 09-Problem Scenario | large distribution differences between the training data and testing data(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 6 | `R6BR7SNQ_E115` | `motivates` | 09-Problem Scenario | large distribution differences between the training data and testing data(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | FFNN |  |
| 7 | `R6BR7SNQ_E116` | `motivates` | 09-Problem Scenario | large distribution differences between the training data and testing data(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `R6BR7SNQ_E117` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | FFNN |  |
| 9 | `R6BR7SNQ_E118` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 10 | `R6BR7SNQ_E119` | `motivates` | 06-Fault Severity | 0.18 mm, 0.36 mm, 0.53 mm; 0.2 mm, 0.6 mm, 1.2 mm(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `R6BR7SNQ_E110`：large distribution differences between the training data and testing data(Distribution Discrepancy) motivates MK-MMD
  - `R6BR7SNQ_E114`：large distribution differences between the training data and testing data(Distribution Discrepancy) motivates FFT
  - `R6BR7SNQ_E115`：large distribution differences between the training data and testing data(Distribution Discrepancy) motivates FFNN
  - `R6BR7SNQ_E116`：large distribution differences between the training data and testing data(Distribution Discrepancy) motivates Softmax
