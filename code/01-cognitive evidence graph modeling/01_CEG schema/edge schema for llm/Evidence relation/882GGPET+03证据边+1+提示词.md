...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `882GGPET_E174` | `motivates` | 13-Noise Level | noisy or unsettled data(High Noise) |  | 15-Data Preprocessing Algorithm | Bandpass Filter |  |
| 2 | `882GGPET_E175` | `motivates` | 13-Noise Level | noisy or unsettled data(High Noise) |  | 16-Feature Extraction Algorithm | Orbit Plot |  |
| 3 | `882GGPET_E176` | `motivates` | 09-Problem Scenario | discriminating faults with similar spectral features(Other) |  | 15-Data Preprocessing Algorithm | Bandpass Filter |  |
| 4 | `882GGPET_E177` | `motivates` | 09-Problem Scenario | different dynamic characteristics(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | Bandpass Filter |  |
| 5 | `882GGPET_E178` | `motivates` | 09-Problem Scenario | discriminating faults with similar spectral features(Other) |  | 16-Feature Extraction Algorithm | Orbit Plot |  |
| 6 | `882GGPET_E179` | `motivates` | 09-Problem Scenario | different dynamic characteristics(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | Orbit Plot |  |
| 7 | `882GGPET_E180` | `motivates` | 09-Problem Scenario | discriminating faults with similar spectral features(Other) |  | 17-Core Classifier Algorithm | Orbit Shape Analysis |  |
| 8 | `882GGPET_E181` | `motivates` | 09-Problem Scenario | different dynamic characteristics(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Orbit Shape Analysis |  |
| 9 | `882GGPET_E182` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Orbit Plot |  |
| 10 | `882GGPET_E183` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Orbit Shape Analysis |  |
| 11 | `882GGPET_E184` | `motivates` | 06-Fault Severity | 1.43*10^-4 kg m unbalance, 3.2 mm bow run out, 4 mm deep crack, 0.8 mm misalignment, 0.1 mm rub gap(Single Severity) |  | 17-Core Classifier Algorithm | Orbit Shape Analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `882GGPET_E176`：discriminating faults with similar spectral features(Other) motivates Bandpass Filter
  - `882GGPET_E177`：different dynamic characteristics(Distribution Discrepancy) motivates Bandpass Filter
  - `882GGPET_E178`：discriminating faults with similar spectral features(Other) motivates Orbit Plot
  - `882GGPET_E179`：different dynamic characteristics(Distribution Discrepancy) motivates Orbit Plot
  - `882GGPET_E180`：discriminating faults with similar spectral features(Other) motivates Orbit Shape Analysis
  - `882GGPET_E181`：different dynamic characteristics(Distribution Discrepancy) motivates Orbit Shape Analysis
