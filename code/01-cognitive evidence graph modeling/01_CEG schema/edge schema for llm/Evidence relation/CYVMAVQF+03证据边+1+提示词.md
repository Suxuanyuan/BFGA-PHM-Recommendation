...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `CYVMAVQF_E092` | `motivates` | 13-Noise Level | alleviate noises(High Noise) |  | 15-Data Preprocessing Algorithm | Connected Components Labeling |  |
| 2 | `CYVMAVQF_E093` | `motivates` | 13-Noise Level | alleviate noises(High Noise) |  | 16-Feature Extraction Algorithm | CNN |  |
| 3 | `CYVMAVQF_E094` | `motivates` | 09-Problem Scenario | 1-D vibration signal feature extraction and representation challenge(Other) |  | 15-Data Preprocessing Algorithm | Connected Components Labeling |  |
| 4 | `CYVMAVQF_E095` | `motivates` | 09-Problem Scenario | 1-D vibration signal feature extraction and representation challenge(Other) |  | 16-Feature Extraction Algorithm | CNN |  |
| 5 | `CYVMAVQF_E096` | `motivates` | 09-Problem Scenario | 1-D vibration signal feature extraction and representation challenge(Other) |  | 17-Core Classifier Algorithm | Softmax |  |
| 6 | `CYVMAVQF_E097` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CNN |  |
| 7 | `CYVMAVQF_E098` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `CYVMAVQF_E099` | `motivates` | 06-Fault Severity | fault width (0.18 mm, 0.36 mm, 0.53 mm, 0.71 mm)(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `CYVMAVQF_E094`：1-D vibration signal feature extraction and representation challenge(Other) motivates Connected Components Labeling
  - `CYVMAVQF_E095`：1-D vibration signal feature extraction and representation challenge(Other) motivates CNN
  - `CYVMAVQF_E096`：1-D vibration signal feature extraction and representation challenge(Other) motivates Softmax
