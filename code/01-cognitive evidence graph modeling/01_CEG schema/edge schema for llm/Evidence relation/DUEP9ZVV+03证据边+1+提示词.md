...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `DUEP9ZVV_E126` | `motivates` | 09-Problem Scenario | noise robustness and experimental variations(Uncertainty) |  | 19-Training Optimization Algorithm | Learning Vector Quantization 3 |  |
| 2 | `DUEP9ZVV_E127` | `motivates` | 12-Training Data Availability | 450 grayscale images in total, 300 used for training(Sufficient) |  | 19-Training Optimization Algorithm | Learning Vector Quantization 3 |  |
| 3 | `DUEP9ZVV_E128` | `motivates` | 13-Noise Level | environment with ambient noise, ambient sound pressure level (SPL) is recorded as 53.2 dB in average(High Noise) |  | 15-Data Preprocessing Algorithm | Autocorrelation |  |
| 4 | `DUEP9ZVV_E129` | `motivates` | 13-Noise Level | environment with ambient noise, ambient sound pressure level (SPL) is recorded as 53.2 dB in average(High Noise) |  | 16-Feature Extraction Algorithm | Discrete Wavelet Transform |  |
| 5 | `DUEP9ZVV_E130` | `motivates` | 09-Problem Scenario | noise robustness and experimental variations(Uncertainty) |  | 15-Data Preprocessing Algorithm | Autocorrelation |  |
| 6 | `DUEP9ZVV_E131` | `motivates` | 09-Problem Scenario | noise robustness and experimental variations(Uncertainty) |  | 16-Feature Extraction Algorithm | Discrete Wavelet Transform |  |
| 7 | `DUEP9ZVV_E132` | `motivates` | 09-Problem Scenario | noise robustness and experimental variations(Uncertainty) |  | 17-Core Classifier Algorithm | Kohonen Self-Organizing Map |  |
| 8 | `DUEP9ZVV_E133` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Discrete Wavelet Transform |  |
| 9 | `DUEP9ZVV_E134` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Kohonen Self-Organizing Map |  |
| 10 | `DUEP9ZVV_E135` | `motivates` | 06-Fault Severity | 3 broken rotor bars over 18, short circuit of two adjacent coils for a few millimeters(Single Severity) |  | 17-Core Classifier Algorithm | Kohonen Self-Organizing Map |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `DUEP9ZVV_E126`：noise robustness and experimental variations(Uncertainty) motivates Learning Vector Quantization 3
  - `DUEP9ZVV_E130`：noise robustness and experimental variations(Uncertainty) motivates Autocorrelation
  - `DUEP9ZVV_E131`：noise robustness and experimental variations(Uncertainty) motivates Discrete Wavelet Transform
  - `DUEP9ZVV_E132`：noise robustness and experimental variations(Uncertainty) motivates Kohonen Self-Organizing Map
