...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `IK4D6KTP_E060` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | DAE |  |
| 2 | `IK4D6KTP_E061` | `motivates` | 09-Problem Scenario | prone to perturbation by different sample sets, input features, algorithm parameters(Uncertainty) |  | 16-Feature Extraction Algorithm | DAE |  |
| 3 | `IK4D6KTP_E062` | `motivates` | 09-Problem Scenario | prone to perturbation by different sample sets, input features, algorithm parameters(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax Regression |  |
| 4 | `IK4D6KTP_E063` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | DAE |  |
| 5 | `IK4D6KTP_E064` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax Regression |  |
| 6 | `IK4D6KTP_E065` | `motivates` | 06-Fault Severity | 0.007, 0.014, 0.021, 0.028 inches, 1 mm2, 7 mm2(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax Regression |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `IK4D6KTP_E061`：prone to perturbation by different sample sets, input features, algorithm parameters(Uncertainty) motivates DAE
  - `IK4D6KTP_E062`：prone to perturbation by different sample sets, input features, algorithm parameters(Uncertainty) motivates Softmax Regression
...
*This prompt is automatically generated by edge_03_prompt.py (Batch 1, total 6 edges)*
