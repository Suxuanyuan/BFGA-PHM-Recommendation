...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `TERUA48Y_E072` | `motivates` | 13-Noise Level | noise increase (inverter fed)(High Noise) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 2 | `TERUA48Y_E073` | `motivates` | 13-Noise Level | noise increase (inverter fed)(High Noise) |  | 16-Feature Extraction Algorithm | Shifted Mutual Information |  |
| 3 | `TERUA48Y_E074` | `motivates` | 09-Problem Scenario | Cross classification from data of distinct motors / Interchanging motor data(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 4 | `TERUA48Y_E075` | `motivates` | 09-Problem Scenario | Cross classification from data of distinct motors / Interchanging motor data(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | Shifted Mutual Information |  |
| 5 | `TERUA48Y_E076` | `motivates` | 09-Problem Scenario | Cross classification from data of distinct motors / Interchanging motor data(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Multilayer Perceptron |  |
| 6 | `TERUA48Y_E077` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Shifted Mutual Information |  |
| 7 | `TERUA48Y_E078` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Multilayer Perceptron |  |
| 8 | `TERUA48Y_E079` | `motivates` | 06-Fault Severity | 15, 30, 60, and 90 minutes of bearings wear(Multiple Severities) |  | 17-Core Classifier Algorithm | Multilayer Perceptron |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `TERUA48Y_E074`：Cross classification from data of distinct motors / Interchanging motor data(Distribution Discrepancy) motivates Normalization
  - `TERUA48Y_E075`：Cross classification from data of distinct motors / Interchanging motor data(Distribution Discrepancy) motivates Shifted Mutual Information
  - `TERUA48Y_E076`：Cross classification from data of distinct motors / Interchanging motor data(Distribution Discrepancy) motivates Multilayer Perceptron
