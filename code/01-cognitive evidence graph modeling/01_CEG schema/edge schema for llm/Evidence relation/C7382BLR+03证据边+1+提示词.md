...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `C7382BLR_E036` | `motivates` | 09-Problem Scenario | unknown and time-varying noise statistics and model uncertainties(Uncertainty) |  | 17-Core Classifier Algorithm | Adaptive Joint Unscented Kalman Filter |  |
| 2 | `C7382BLR_E037` | `motivates` | 07-Compound Fault | simultaneous actuator faults(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Adaptive Joint Unscented Kalman Filter |  |
| 3 | `C7382BLR_E038` | `motivates` | 06-Fault Severity | partial loss(Multiple Severities) |  | 17-Core Classifier Algorithm | Adaptive Joint Unscented Kalman Filter |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `C7382BLR_E036`：unknown and time-varying noise statistics and model uncertainties(Uncertainty) motivates Adaptive Joint Unscented Kalman Filter
