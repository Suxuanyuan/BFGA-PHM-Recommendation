...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `FY64VA9C_E095` | `motivates` | 13-Noise Level | Noise and interference(High Noise) |  | 15-Data Preprocessing Algorithm | Elliptic IIR Filter |  |
| 2 | `FY64VA9C_E096` | `motivates` | 13-Noise Level | Noise and interference(High Noise) |  | 16-Feature Extraction Algorithm | Mass Laplacian Discriminant Analysis |  |
| 3 | `FY64VA9C_E097` | `motivates` | 09-Problem Scenario | compound fault(Compound Faults) |  | 15-Data Preprocessing Algorithm | Elliptic IIR Filter |  |
| 4 | `FY64VA9C_E098` | `motivates` | 09-Problem Scenario | noise interference(Uncertainty) |  | 15-Data Preprocessing Algorithm | Elliptic IIR Filter |  |
| 5 | `FY64VA9C_E099` | `motivates` | 09-Problem Scenario | compound fault(Compound Faults) |  | 16-Feature Extraction Algorithm | Mass Laplacian Discriminant Analysis |  |
| 6 | `FY64VA9C_E100` | `motivates` | 09-Problem Scenario | noise interference(Uncertainty) |  | 16-Feature Extraction Algorithm | Mass Laplacian Discriminant Analysis |  |
| 7 | `FY64VA9C_E101` | `motivates` | 09-Problem Scenario | compound fault(Compound Faults) |  | 17-Core Classifier Algorithm | Softmax Classifier |  |
| 8 | `FY64VA9C_E102` | `motivates` | 09-Problem Scenario | noise interference(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax Classifier |  |
| 9 | `FY64VA9C_E103` | `motivates` | 07-Compound Fault | compound fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Mass Laplacian Discriminant Analysis |  |
| 10 | `FY64VA9C_E104` | `motivates` | 07-Compound Fault | compound fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Softmax Classifier |  |
| 11 | `FY64VA9C_E105` | `motivates` | 06-Fault Severity | pitting pits, broken(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax Classifier |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `FY64VA9C_E097`：compound fault(Compound Faults) motivates Elliptic IIR Filter
  - `FY64VA9C_E098`：noise interference(Uncertainty) motivates Elliptic IIR Filter
  - `FY64VA9C_E099`：compound fault(Compound Faults) motivates Mass Laplacian Discriminant Analysis
  - `FY64VA9C_E100`：noise interference(Uncertainty) motivates Mass Laplacian Discriminant Analysis
  - `FY64VA9C_E101`：compound fault(Compound Faults) motivates Softmax Classifier
  - `FY64VA9C_E102`：noise interference(Uncertainty) motivates Softmax Classifier
