...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `XDSTTWTP_E096` | `motivates` | 09-Problem Scenario | data from the possible faulty modes are not readily available(Zero Fault Samples) |  | 19-Training Optimization Algorithm | Feature Bagging |  |
| 2 | `XDSTTWTP_E097` | `motivates` | 12-Training Data Availability | Normal 280, we selected randomly 70% of the normal data for training each of the anomaly detector(Sufficient) |  | 19-Training Optimization Algorithm | Feature Bagging |  |
| 3 | `XDSTTWTP_E098` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | Z-score normalization |  |
| 4 | `XDSTTWTP_E099` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | Empirical Mode Decomposition |  |
| 5 | `XDSTTWTP_E100` | `motivates` | 09-Problem Scenario | data from the possible faulty modes are not readily available(Zero Fault Samples) |  | 15-Data Preprocessing Algorithm | Z-score normalization |  |
| 6 | `XDSTTWTP_E101` | `motivates` | 09-Problem Scenario | data from the possible faulty modes are not readily available(Zero Fault Samples) |  | 16-Feature Extraction Algorithm | Empirical Mode Decomposition |  |
| 7 | `XDSTTWTP_E102` | `motivates` | 09-Problem Scenario | data from the possible faulty modes are not readily available(Zero Fault Samples) |  | 17-Core Classifier Algorithm | Hybrid Ensemble Detector |  |
| 8 | `XDSTTWTP_E103` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Empirical Mode Decomposition |  |
| 9 | `XDSTTWTP_E104` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Hybrid Ensemble Detector |  |
| 10 | `XDSTTWTP_E105` | `motivates` | 06-Fault Severity | fault sizes 0.007–0.028 in.(Multiple Severities) |  | 17-Core Classifier Algorithm | Hybrid Ensemble Detector |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `XDSTTWTP_E096`：data from the possible faulty modes are not readily available(Zero Fault Samples) motivates Feature Bagging
  - `XDSTTWTP_E100`：data from the possible faulty modes are not readily available(Zero Fault Samples) motivates Z-score normalization
  - `XDSTTWTP_E101`：data from the possible faulty modes are not readily available(Zero Fault Samples) motivates Empirical Mode Decomposition
  - `XDSTTWTP_E102`：data from the possible faulty modes are not readily available(Zero Fault Samples) motivates Hybrid Ensemble Detector
