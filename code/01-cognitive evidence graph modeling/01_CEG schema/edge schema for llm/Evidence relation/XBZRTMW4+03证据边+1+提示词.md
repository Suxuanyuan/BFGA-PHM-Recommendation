...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `XBZRTMW4_E055` | `motivates` | 09-Problem Scenario | Cooperative control under actuator faults(Complex Systems) |  | 17-Core Classifier Algorithm | Two-stage Kalman filter |  |
| 2 | `XBZRTMW4_E056` | `motivates` | 09-Problem Scenario | Uncertainty in state and fault parameter estimation(Uncertainty) |  | 17-Core Classifier Algorithm | Two-stage Kalman filter |  |
| 3 | `XBZRTMW4_E057` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Two-stage Kalman filter |  |
| 4 | `XBZRTMW4_E058` | `motivates` | 06-Fault Severity | loss of control effectiveness(Multiple Severities) |  | 17-Core Classifier Algorithm | Two-stage Kalman filter |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `XBZRTMW4_E055`：Cooperative control under actuator faults(Complex Systems) motivates Two-stage Kalman filter
  - `XBZRTMW4_E056`：Uncertainty in state and fault parameter estimation(Uncertainty) motivates Two-stage Kalman filter
