...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `JUSHTA5W_E083` | `motivates` | 09-Problem Scenario | limited training samples(Small Fault Samples) |  | 19-Training Optimization Algorithm | PSO |  |
| 2 | `JUSHTA5W_E084` | `motivates` | 12-Training Data Availability | 10 sets of the feature vectors... are used for training purpose(Scarce) |  | 19-Training Optimization Algorithm | PSO |  |
| 3 | `JUSHTA5W_E085` | `motivates` | 13-Noise Level | A white noise with signal to noise ratio of 1dB is added into the input signal(High Noise) |  | 15-Data Preprocessing Algorithm | RSGWPT-EEMD |  |
| 4 | `JUSHTA5W_E086` | `motivates` | 13-Noise Level | A white noise with signal to noise ratio of 1dB is added into the input signal(High Noise) |  | 16-Feature Extraction Algorithm | SVD |  |
| 5 | `JUSHTA5W_E087` | `motivates` | 09-Problem Scenario | limited training samples(Small Fault Samples) |  | 15-Data Preprocessing Algorithm | RSGWPT-EEMD |  |
| 6 | `JUSHTA5W_E088` | `motivates` | 09-Problem Scenario | limited training samples(Small Fault Samples) |  | 16-Feature Extraction Algorithm | SVD |  |
| 7 | `JUSHTA5W_E089` | `motivates` | 09-Problem Scenario | limited training samples(Small Fault Samples) |  | 17-Core Classifier Algorithm | LSSVM |  |
| 8 | `JUSHTA5W_E090` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | SVD |  |
| 9 | `JUSHTA5W_E091` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | LSSVM |  |
| 10 | `JUSHTA5W_E092` | `motivates` | 06-Fault Severity | defect size (0.007 inches, 0.021 inches)(Multiple Severities) |  | 17-Core Classifier Algorithm | LSSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `JUSHTA5W_E083`：limited training samples(Small Fault Samples) motivates PSO
  - `JUSHTA5W_E087`：limited training samples(Small Fault Samples) motivates RSGWPT-EEMD
  - `JUSHTA5W_E088`：limited training samples(Small Fault Samples) motivates SVD
  - `JUSHTA5W_E089`：limited training samples(Small Fault Samples) motivates LSSVM
