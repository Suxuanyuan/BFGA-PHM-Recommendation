...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `XIN8U26C_E099` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 15-Data Preprocessing Algorithm | FEEMD |  |
| 2 | `XIN8U26C_E100` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 16-Feature Extraction Algorithm | MAIHND |  |
| 3 | `XIN8U26C_E101` | `motivates` | 09-Problem Scenario | incipient fault(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | FEEMD |  |
| 4 | `XIN8U26C_E102` | `motivates` | 09-Problem Scenario | heavy background noise and interference harmonics(Uncertainty) |  | 15-Data Preprocessing Algorithm | FEEMD |  |
| 5 | `XIN8U26C_E103` | `motivates` | 09-Problem Scenario | incipient fault(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | MAIHND |  |
| 6 | `XIN8U26C_E104` | `motivates` | 09-Problem Scenario | heavy background noise and interference harmonics(Uncertainty) |  | 16-Feature Extraction Algorithm | MAIHND |  |
| 7 | `XIN8U26C_E105` | `motivates` | 09-Problem Scenario | incipient fault(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 8 | `XIN8U26C_E106` | `motivates` | 09-Problem Scenario | heavy background noise and interference harmonics(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 9 | `XIN8U26C_E107` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | MAIHND |  |
| 10 | `XIN8U26C_E108` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 11 | `XIN8U26C_E109` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `XIN8U26C_E101`：incipient fault(Early Degradation Prediction) motivates FEEMD
  - `XIN8U26C_E102`：heavy background noise and interference harmonics(Uncertainty) motivates FEEMD
  - `XIN8U26C_E103`：incipient fault(Early Degradation Prediction) motivates MAIHND
  - `XIN8U26C_E104`：heavy background noise and interference harmonics(Uncertainty) motivates MAIHND
  - `XIN8U26C_E105`：incipient fault(Early Degradation Prediction) motivates Envelope Analysis
  - `XIN8U26C_E106`：heavy background noise and interference harmonics(Uncertainty) motivates Envelope Analysis
