...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `HK5DWGTC_E198` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | sliding window |  |
| 2 | `HK5DWGTC_E199` | `motivates` | 09-Problem Scenario | incipient faults(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | sliding window |  |
| 3 | `HK5DWGTC_E200` | `motivates` | 09-Problem Scenario | sparse and unbalanced data sets(Small Fault Samples) |  | 15-Data Preprocessing Algorithm | sliding window |  |
| 4 | `HK5DWGTC_E201` | `motivates` | 09-Problem Scenario | complex chemical processes(Complex Systems) |  | 15-Data Preprocessing Algorithm | sliding window |  |
| 5 | `HK5DWGTC_E202` | `motivates` | 09-Problem Scenario | incipient faults(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | B-LSTM |  |
| 6 | `HK5DWGTC_E203` | `motivates` | 09-Problem Scenario | sparse and unbalanced data sets(Small Fault Samples) |  | 17-Core Classifier Algorithm | B-LSTM |  |
| 7 | `HK5DWGTC_E204` | `motivates` | 09-Problem Scenario | complex chemical processes(Complex Systems) |  | 17-Core Classifier Algorithm | B-LSTM |  |
| 8 | `HK5DWGTC_E205` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | B-LSTM |  |
| 9 | `HK5DWGTC_E206` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | B-LSTM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `HK5DWGTC_E199`：incipient faults(Early Degradation Prediction) motivates sliding window
  - `HK5DWGTC_E200`：sparse and unbalanced data sets(Small Fault Samples) motivates sliding window
  - `HK5DWGTC_E201`：complex chemical processes(Complex Systems) motivates sliding window
  - `HK5DWGTC_E202`：incipient faults(Early Degradation Prediction) motivates B-LSTM
  - `HK5DWGTC_E203`：sparse and unbalanced data sets(Small Fault Samples) motivates B-LSTM
  - `HK5DWGTC_E204`：complex chemical processes(Complex Systems) motivates B-LSTM
