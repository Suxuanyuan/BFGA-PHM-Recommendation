...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `D8NK2FMJ_E095` | `motivates` | 12-Training Data Availability | imbalanced training data(Scarce) |  | 18-Data Generation Algorithm | GAN |  |
| 2 | `D8NK2FMJ_E096` | `motivates` | 13-Noise Level | additional Gaussian noise(High Noise) |  | 16-Feature Extraction Algorithm | CNN |  |
| 3 | `D8NK2FMJ_E097` | `motivates` | 09-Problem Scenario | imbalanced training data(Small Fault Samples) |  | 18-Data Generation Algorithm | GAN |  |
| 4 | `D8NK2FMJ_E098` | `motivates` | 09-Problem Scenario | environmental noise, robustness against noise(Uncertainty) |  | 18-Data Generation Algorithm | GAN |  |
| 5 | `D8NK2FMJ_E099` | `motivates` | 09-Problem Scenario | imbalanced training data(Small Fault Samples) |  | 16-Feature Extraction Algorithm | CNN |  |
| 6 | `D8NK2FMJ_E100` | `motivates` | 09-Problem Scenario | environmental noise, robustness against noise(Uncertainty) |  | 16-Feature Extraction Algorithm | CNN |  |
| 7 | `D8NK2FMJ_E101` | `motivates` | 09-Problem Scenario | imbalanced training data(Small Fault Samples) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `D8NK2FMJ_E102` | `motivates` | 09-Problem Scenario | environmental noise, robustness against noise(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `D8NK2FMJ_E103` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CNN |  |
| 10 | `D8NK2FMJ_E104` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `D8NK2FMJ_E105` | `motivates` | 06-Fault Severity | fault diameters of 7, 14 and 21 mils; incipient, medium and severe faults(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `D8NK2FMJ_E097`：imbalanced training data(Small Fault Samples) motivates GAN
  - `D8NK2FMJ_E098`：environmental noise, robustness against noise(Uncertainty) motivates GAN
  - `D8NK2FMJ_E099`：imbalanced training data(Small Fault Samples) motivates CNN
  - `D8NK2FMJ_E100`：environmental noise, robustness against noise(Uncertainty) motivates CNN
  - `D8NK2FMJ_E101`：imbalanced training data(Small Fault Samples) motivates Softmax
  - `D8NK2FMJ_E102`：environmental noise, robustness against noise(Uncertainty) motivates Softmax
