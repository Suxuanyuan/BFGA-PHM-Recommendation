...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `USHRQLGT_E092` | `motivates` | 09-Problem Scenario | variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | SAPSO |  |
| 2 | `USHRQLGT_E093` | `motivates` | 12-Training Data Availability | Size of training samples: 20(Scarce) |  | 19-Training Optimization Algorithm | SAPSO |  |
| 3 | `USHRQLGT_E094` | `motivates` | 13-Noise Level | vibration signals(Normal) |  | 15-Data Preprocessing Algorithm | Min-Max Normalization |  |
| 4 | `USHRQLGT_E095` | `motivates` | 13-Noise Level | vibration signals(Normal) |  | 16-Feature Extraction Algorithm | Isomap |  |
| 5 | `USHRQLGT_E096` | `motivates` | 09-Problem Scenario | variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | Min-Max Normalization |  |
| 6 | `USHRQLGT_E097` | `motivates` | 09-Problem Scenario | variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | Isomap |  |
| 7 | `USHRQLGT_E098` | `motivates` | 09-Problem Scenario | variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | IKELM |  |
| 8 | `USHRQLGT_E099` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Isomap |  |
| 9 | `USHRQLGT_E100` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | IKELM |  |
| 10 | `USHRQLGT_E101` | `motivates` | 06-Fault Severity | fault diameters of 0.007, 0.014, 0.021 and 0.028 in.(Multiple Severities) |  | 17-Core Classifier Algorithm | IKELM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `USHRQLGT_E092`：variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) motivates SAPSO
  - `USHRQLGT_E096`：variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) motivates Min-Max Normalization
  - `USHRQLGT_E097`：variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) motivates Isomap
  - `USHRQLGT_E098`：variable operating conditions / cross-condition diagnosis(Distribution Discrepancy) motivates IKELM
