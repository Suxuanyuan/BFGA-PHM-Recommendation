...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `U4YESBV2_E093` | `motivates` | 13-Noise Level | noises and interfering vibrations(High Noise) |  | 15-Data Preprocessing Algorithm | ANC |  |
| 2 | `U4YESBV2_E094` | `motivates` | 13-Noise Level | noises and interfering vibrations(High Noise) |  | 16-Feature Extraction Algorithm | FCO |  |
| 3 | `U4YESBV2_E095` | `motivates` | 09-Problem Scenario | interfering vibration signals / noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | ANC |  |
| 4 | `U4YESBV2_E096` | `motivates` | 09-Problem Scenario | unknown time-varying rotational speed without tachometer(Other) |  | 15-Data Preprocessing Algorithm | ANC |  |
| 5 | `U4YESBV2_E097` | `motivates` | 09-Problem Scenario | interfering vibration signals / noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | FCO |  |
| 6 | `U4YESBV2_E098` | `motivates` | 09-Problem Scenario | unknown time-varying rotational speed without tachometer(Other) |  | 16-Feature Extraction Algorithm | FCO |  |
| 7 | `U4YESBV2_E099` | `motivates` | 09-Problem Scenario | interfering vibration signals / noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | ROS |  |
| 8 | `U4YESBV2_E100` | `motivates` | 09-Problem Scenario | unknown time-varying rotational speed without tachometer(Other) |  | 17-Core Classifier Algorithm | ROS |  |
| 9 | `U4YESBV2_E101` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | FCO |  |
| 10 | `U4YESBV2_E102` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | ROS |  |
| 11 | `U4YESBV2_E103` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | ROS |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `U4YESBV2_E095`：interfering vibration signals / noise robustness(Uncertainty) motivates ANC
  - `U4YESBV2_E096`：unknown time-varying rotational speed without tachometer(Other) motivates ANC
  - `U4YESBV2_E097`：interfering vibration signals / noise robustness(Uncertainty) motivates FCO
  - `U4YESBV2_E098`：unknown time-varying rotational speed without tachometer(Other) motivates FCO
  - `U4YESBV2_E099`：interfering vibration signals / noise robustness(Uncertainty) motivates ROS
  - `U4YESBV2_E100`：unknown time-varying rotational speed without tachometer(Other) motivates ROS
