...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `YIAUUHV3_E109` | `motivates` | 09-Problem Scenario | varying working conditions(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | NMPT |  |
| 2 | `YIAUUHV3_E110` | `motivates` | 09-Problem Scenario | insufficient training samples in target task(Small Fault Samples) |  | 19-Training Optimization Algorithm | NMPT |  |
| 3 | `YIAUUHV3_E111` | `motivates` | 12-Training Data Availability | insufficient training samples in target task(Scarce) |  | 19-Training Optimization Algorithm | NMPT |  |
| 4 | `YIAUUHV3_E112` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | ITD |  |
| 5 | `YIAUUHV3_E113` | `motivates` | 09-Problem Scenario | varying working conditions(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | ITD |  |
| 6 | `YIAUUHV3_E114` | `motivates` | 09-Problem Scenario | insufficient training samples in target task(Small Fault Samples) |  | 16-Feature Extraction Algorithm | ITD |  |
| 7 | `YIAUUHV3_E115` | `motivates` | 09-Problem Scenario | varying working conditions(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | LSSVM |  |
| 8 | `YIAUUHV3_E116` | `motivates` | 09-Problem Scenario | insufficient training samples in target task(Small Fault Samples) |  | 17-Core Classifier Algorithm | LSSVM |  |
| 9 | `YIAUUHV3_E117` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | ITD |  |
| 10 | `YIAUUHV3_E118` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | LSSVM |  |
| 11 | `YIAUUHV3_E119` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | LSSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `YIAUUHV3_E109`：varying working conditions(Distribution Discrepancy) motivates NMPT
  - `YIAUUHV3_E110`：insufficient training samples in target task(Small Fault Samples) motivates NMPT
  - `YIAUUHV3_E113`：varying working conditions(Distribution Discrepancy) motivates ITD
  - `YIAUUHV3_E114`：insufficient training samples in target task(Small Fault Samples) motivates ITD
  - `YIAUUHV3_E115`：varying working conditions(Distribution Discrepancy) motivates LSSVM
  - `YIAUUHV3_E116`：insufficient training samples in target task(Small Fault Samples) motivates LSSVM
