...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `BYTKWXDE_E090` | `motivates` | 13-Noise Level | stochastic systems, measurement noise, process noise(High Noise) |  | 16-Feature Extraction Algorithm | Noise Deviation |  |
| 2 | `BYTKWXDE_E091` | `motivates` | 09-Problem Scenario | unknown residual distribution in nonlinear stochastic systems(Uncertainty) |  | 16-Feature Extraction Algorithm | Noise Deviation |  |
| 3 | `BYTKWXDE_E092` | `motivates` | 09-Problem Scenario | correlated residuals(Other) |  | 16-Feature Extraction Algorithm | Noise Deviation |  |
| 4 | `BYTKWXDE_E093` | `motivates` | 09-Problem Scenario | unknown residual distribution in nonlinear stochastic systems(Uncertainty) |  | 17-Core Classifier Algorithm | Mann-Whitney test |  |
| 5 | `BYTKWXDE_E094` | `motivates` | 09-Problem Scenario | correlated residuals(Other) |  | 17-Core Classifier Algorithm | Mann-Whitney test |  |
| 6 | `BYTKWXDE_E095` | `motivates` | 07-Compound Fault | Sensor fault and component fault are considered simultaneously(Compound Fault Across Structures) |  | 16-Feature Extraction Algorithm | Noise Deviation |  |
| 7 | `BYTKWXDE_E096` | `motivates` | 07-Compound Fault | Sensor fault and component fault are considered simultaneously(Compound Fault Across Structures) |  | 17-Core Classifier Algorithm | Mann-Whitney test |  |
| 8 | `BYTKWXDE_E097` | `motivates` | 06-Fault Severity | c = 0.3, r = 0.5cm, f2,1,k = -4 x 10^-5 m^3/s, f2,2,k = 3 x 10^-5 m^3/s(Single Severity) |  | 17-Core Classifier Algorithm | Mann-Whitney test |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `BYTKWXDE_E091`：unknown residual distribution in nonlinear stochastic systems(Uncertainty) motivates Noise Deviation
  - `BYTKWXDE_E092`：correlated residuals(Other) motivates Noise Deviation
  - `BYTKWXDE_E093`：unknown residual distribution in nonlinear stochastic systems(Uncertainty) motivates Mann-Whitney test
  - `BYTKWXDE_E094`：correlated residuals(Other) motivates Mann-Whitney test
