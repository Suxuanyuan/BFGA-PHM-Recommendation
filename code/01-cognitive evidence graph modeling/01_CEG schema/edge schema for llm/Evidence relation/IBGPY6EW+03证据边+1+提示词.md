...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `IBGPY6EW_E123` | `motivates` | 09-Problem Scenario | labeling condition monitoring data is expensive / difficult or expensive to collect self-contained negative samples(Small Fault Samples) |  | 19-Training Optimization Algorithm | Manifold Regularization |  |
| 2 | `IBGPY6EW_E124` | `motivates` | 09-Problem Scenario | single-conditions labeled mode / change over time as a function of the operating conditions(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | Manifold Regularization |  |
| 3 | `IBGPY6EW_E125` | `motivates` | 12-Training Data Availability | labeled training data(Scarce) |  | 19-Training Optimization Algorithm | Manifold Regularization |  |
| 4 | `IBGPY6EW_E126` | `motivates` | 13-Noise Level | vibration signals(Normal) |  | 16-Feature Extraction Algorithm | Wavelet Packet Transform |  |
| 5 | `IBGPY6EW_E127` | `motivates` | 09-Problem Scenario | labeling condition monitoring data is expensive / difficult or expensive to collect self-contained negative samples(Small Fault Samples) |  | 16-Feature Extraction Algorithm | Wavelet Packet Transform |  |
| 6 | `IBGPY6EW_E128` | `motivates` | 09-Problem Scenario | single-conditions labeled mode / change over time as a function of the operating conditions(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | Wavelet Packet Transform |  |
| 7 | `IBGPY6EW_E129` | `motivates` | 09-Problem Scenario | labeling condition monitoring data is expensive / difficult or expensive to collect self-contained negative samples(Small Fault Samples) |  | 17-Core Classifier Algorithm | LapSVM |  |
| 8 | `IBGPY6EW_E130` | `motivates` | 09-Problem Scenario | single-conditions labeled mode / change over time as a function of the operating conditions(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | LapSVM |  |
| 9 | `IBGPY6EW_E131` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Wavelet Packet Transform |  |
| 10 | `IBGPY6EW_E132` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | LapSVM |  |
| 11 | `IBGPY6EW_E133` | `motivates` | 06-Fault Severity | three severity levels (0.007, 0.014 and 0.021 in)(Multiple Severities) |  | 17-Core Classifier Algorithm | LapSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `IBGPY6EW_E123`：labeling condition monitoring data is expensive / difficult or expensive to collect self-contained negative samples(Small Fault Samples) motivates Manifold Regularization
  - `IBGPY6EW_E124`：single-conditions labeled mode / change over time as a function of the operating conditions(Distribution Discrepancy) motivates Manifold Regularization
  - `IBGPY6EW_E127`：labeling condition monitoring data is expensive / difficult or expensive to collect self-contained negative samples(Small Fault Samples) motivates Wavelet Packet Transform
  - `IBGPY6EW_E128`：single-conditions labeled mode / change over time as a function of the operating conditions(Distribution Discrepancy) motivates Wavelet Packet Transform
  - `IBGPY6EW_E129`：labeling condition monitoring data is expensive / difficult or expensive to collect self-contained negative samples(Small Fault Samples) motivates LapSVM
  - `IBGPY6EW_E130`：single-conditions labeled mode / change over time as a function of the operating conditions(Distribution Discrepancy) motivates LapSVM
