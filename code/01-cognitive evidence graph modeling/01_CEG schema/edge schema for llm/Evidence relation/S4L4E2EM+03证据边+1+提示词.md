...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `S4L4E2EM_E095` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | CMMD |  |
| 2 | `S4L4E2EM_E096` | `motivates` | 09-Problem Scenario | vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) |  | 19-Training Optimization Algorithm | CMMD |  |
| 3 | `S4L4E2EM_E097` | `motivates` | 12-Training Data Availability | 200 samples with the same label(Sufficient) |  | 19-Training Optimization Algorithm | CMMD |  |
| 4 | `S4L4E2EM_E098` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | FFT |  |
| 5 | `S4L4E2EM_E099` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 6 | `S4L4E2EM_E100` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 7 | `S4L4E2EM_E101` | `motivates` | 09-Problem Scenario | vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 8 | `S4L4E2EM_E102` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 9 | `S4L4E2EM_E103` | `motivates` | 09-Problem Scenario | vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 10 | `S4L4E2EM_E104` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `S4L4E2EM_E105` | `motivates` | 09-Problem Scenario | vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) |  | 17-Core Classifier Algorithm | Softmax |  |
| 12 | `S4L4E2EM_E106` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | ResNet-50 |  |
| 13 | `S4L4E2EM_E107` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 14 | `S4L4E2EM_E108` | `motivates` | 06-Fault Severity | 0.007, 0.014, and 0.021 inches; 0.2 and 0.30 mm(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `S4L4E2EM_E095`：domain shift(Distribution Discrepancy) motivates CMMD
  - `S4L4E2EM_E096`：vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) motivates CMMD
  - `S4L4E2EM_E100`：domain shift(Distribution Discrepancy) motivates FFT
  - `S4L4E2EM_E101`：vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) motivates FFT
  - `S4L4E2EM_E102`：domain shift(Distribution Discrepancy) motivates ResNet-50
  - `S4L4E2EM_E103`：vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) motivates ResNet-50
  - `S4L4E2EM_E104`：domain shift(Distribution Discrepancy) motivates Softmax
  - `S4L4E2EM_E105`：vibration data and labels are difficult to obtain for each working condition(Small Fault Samples) motivates Softmax
