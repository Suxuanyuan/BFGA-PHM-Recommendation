...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `GXTZLAV2_E089` | `motivates` | 09-Problem Scenario | small samples(Small Fault Samples) |  | 19-Training Optimization Algorithm | PSO |  |
| 2 | `GXTZLAV2_E090` | `motivates` | 09-Problem Scenario | reduce the uncertainty of diagnosis(Uncertainty) |  | 19-Training Optimization Algorithm | PSO |  |
| 3 | `GXTZLAV2_E091` | `motivates` | 12-Training Data Availability | Ten segments were randomly selected (as the training samples) from the twenty segments(Scarce) |  | 19-Training Optimization Algorithm | PSO |  |
| 4 | `GXTZLAV2_E092` | `motivates` | 13-Noise Level | the bearings data from CWRU is relatively clean(Normal) |  | 16-Feature Extraction Algorithm | KPCA |  |
| 5 | `GXTZLAV2_E093` | `motivates` | 09-Problem Scenario | small samples(Small Fault Samples) |  | 16-Feature Extraction Algorithm | KPCA |  |
| 6 | `GXTZLAV2_E094` | `motivates` | 09-Problem Scenario | reduce the uncertainty of diagnosis(Uncertainty) |  | 16-Feature Extraction Algorithm | KPCA |  |
| 7 | `GXTZLAV2_E095` | `motivates` | 09-Problem Scenario | small samples(Small Fault Samples) |  | 17-Core Classifier Algorithm | LSSVM |  |
| 8 | `GXTZLAV2_E096` | `motivates` | 09-Problem Scenario | reduce the uncertainty of diagnosis(Uncertainty) |  | 17-Core Classifier Algorithm | LSSVM |  |
| 9 | `GXTZLAV2_E097` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | KPCA |  |
| 10 | `GXTZLAV2_E098` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | LSSVM |  |
| 11 | `GXTZLAV2_E099` | `motivates` | 06-Fault Severity | 0.18 mm, 0.54 mm(Multiple Severities) |  | 17-Core Classifier Algorithm | LSSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `GXTZLAV2_E089`：small samples(Small Fault Samples) motivates PSO
  - `GXTZLAV2_E090`：reduce the uncertainty of diagnosis(Uncertainty) motivates PSO
  - `GXTZLAV2_E093`：small samples(Small Fault Samples) motivates KPCA
  - `GXTZLAV2_E094`：reduce the uncertainty of diagnosis(Uncertainty) motivates KPCA
  - `GXTZLAV2_E095`：small samples(Small Fault Samples) motivates LSSVM
  - `GXTZLAV2_E096`：reduce the uncertainty of diagnosis(Uncertainty) motivates LSSVM
