...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `2PX7N2QZ_E089` | `motivates` | 13-Noise Level | Field noise / ambient noise(High Noise) |  | 15-Data Preprocessing Algorithm | Hanning-windowed DFT |  |
| 2 | `2PX7N2QZ_E090` | `motivates` | 13-Noise Level | Field noise / ambient noise(High Noise) |  | 16-Feature Extraction Algorithm | Spectrum Correction |  |
| 3 | `2PX7N2QZ_E091` | `motivates` | 09-Problem Scenario | short-sampled observations(Small Fault Samples) |  | 15-Data Preprocessing Algorithm | Hanning-windowed DFT |  |
| 4 | `2PX7N2QZ_E092` | `motivates` | 09-Problem Scenario | short-sampled observations(Small Fault Samples) |  | 16-Feature Extraction Algorithm | Spectrum Correction |  |
| 5 | `2PX7N2QZ_E093` | `motivates` | 09-Problem Scenario | short-sampled observations(Small Fault Samples) |  | 17-Core Classifier Algorithm | k-means |  |
| 6 | `2PX7N2QZ_E094` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Spectrum Correction |  |
| 7 | `2PX7N2QZ_E095` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | k-means |  |
| 8 | `2PX7N2QZ_E096` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | k-means |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `2PX7N2QZ_E091`：short-sampled observations(Small Fault Samples) motivates Hanning-windowed DFT
  - `2PX7N2QZ_E092`：short-sampled observations(Small Fault Samples) motivates Spectrum Correction
  - `2PX7N2QZ_E093`：short-sampled observations(Small Fault Samples) motivates k-means
