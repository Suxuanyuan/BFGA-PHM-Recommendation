...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `Y9ITG6D6_E118` | `motivates` | 09-Problem Scenario | model sparsity and real-time testing constraint(Other) |  | 19-Training Optimization Algorithm | Dual reduced learning |  |
| 2 | `Y9ITG6D6_E119` | `motivates` | 12-Training Data Availability | 100 samples for each fault pattern(Sufficient) |  | 19-Training Optimization Algorithm | Dual reduced learning |  |
| 3 | `Y9ITG6D6_E120` | `motivates` | 13-Noise Level | measurement noise with variance of 0.00152(High Noise) |  | 15-Data Preprocessing Algorithm | Min-max normalization |  |
| 4 | `Y9ITG6D6_E121` | `motivates` | 09-Problem Scenario | model sparsity and real-time testing constraint(Other) |  | 15-Data Preprocessing Algorithm | Min-max normalization |  |
| 5 | `Y9ITG6D6_E122` | `motivates` | 09-Problem Scenario | model sparsity and real-time testing constraint(Other) |  | 17-Core Classifier Algorithm | DR-KELM |  |
| 6 | `Y9ITG6D6_E123` | `motivates` | 07-Compound Fault | SE1 -0.5% and SW1 -1%, SE2 -0.7% and SW2 -1%, SE3 -1% and SW3 -1%, SE4 -0.4% and SW4 -1%, SE4 -0.6% and SW4 +1%(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | DR-KELM |  |
| 7 | `Y9ITG6D6_E124` | `motivates` | 06-Fault Severity | SE1 -1%, SW1 -1%, SE2 -1%, SW2 -1%, SE3 -1%, SW3 +1%, SE4 -1%, SW4 -1%(Single Severity) |  | 17-Core Classifier Algorithm | DR-KELM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `Y9ITG6D6_E118`：model sparsity and real-time testing constraint(Other) motivates Dual reduced learning
  - `Y9ITG6D6_E121`：model sparsity and real-time testing constraint(Other) motivates Min-max normalization
  - `Y9ITG6D6_E122`：model sparsity and real-time testing constraint(Other) motivates DR-KELM
