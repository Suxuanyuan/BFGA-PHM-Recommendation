...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `FLDQRAD4_E124` | `motivates` | 09-Problem Scenario | strong noise(Uncertainty) |  | 19-Training Optimization Algorithm | FISTA |  |
| 2 | `FLDQRAD4_E125` | `motivates` | 12-Training Data Availability | Sufficient |  | 19-Training Optimization Algorithm | FISTA |  |
| 3 | `FLDQRAD4_E126` | `motivates` | 13-Noise Level | strong noise(High Noise) |  | 15-Data Preprocessing Algorithm | Low-pass filter |  |
| 4 | `FLDQRAD4_E127` | `motivates` | 13-Noise Level | strong noise(High Noise) |  | 16-Feature Extraction Algorithm | SSTFA |  |
| 5 | `FLDQRAD4_E128` | `motivates` | 09-Problem Scenario | strong noise(Uncertainty) |  | 15-Data Preprocessing Algorithm | Low-pass filter |  |
| 6 | `FLDQRAD4_E129` | `motivates` | 09-Problem Scenario | strong noise(Uncertainty) |  | 16-Feature Extraction Algorithm | SSTFA |  |
| 7 | `FLDQRAD4_E130` | `motivates` | 09-Problem Scenario | strong noise(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope demodulation spectrum |  |
| 8 | `FLDQRAD4_E131` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | SSTFA |  |
| 9 | `FLDQRAD4_E132` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Envelope demodulation spectrum |  |
| 10 | `FLDQRAD4_E133` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Envelope demodulation spectrum |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `FLDQRAD4_E124`：strong noise(Uncertainty) motivates FISTA
  - `FLDQRAD4_E128`：strong noise(Uncertainty) motivates Low-pass filter
  - `FLDQRAD4_E129`：strong noise(Uncertainty) motivates SSTFA
  - `FLDQRAD4_E130`：strong noise(Uncertainty) motivates Envelope demodulation spectrum
