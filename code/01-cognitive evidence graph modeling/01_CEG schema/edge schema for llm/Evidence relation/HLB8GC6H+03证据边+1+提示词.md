...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `HLB8GC6H_E079` | `motivates` | 13-Noise Level | fluctuating voltage(High Noise) |  | 15-Data Preprocessing Algorithm | Discrete Wavelet Transform |  |
| 2 | `HLB8GC6H_E080` | `motivates` | 13-Noise Level | fluctuating voltage(High Noise) |  | 16-Feature Extraction Algorithm | Scale Transform |  |
| 3 | `HLB8GC6H_E081` | `motivates` | 09-Problem Scenario | scale variation due to machine size and load conditions(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | Discrete Wavelet Transform |  |
| 4 | `HLB8GC6H_E082` | `motivates` | 09-Problem Scenario | supply voltage fluctuations and noises(Uncertainty) |  | 15-Data Preprocessing Algorithm | Discrete Wavelet Transform |  |
| 5 | `HLB8GC6H_E083` | `motivates` | 09-Problem Scenario | scale variation due to machine size and load conditions(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | Scale Transform |  |
| 6 | `HLB8GC6H_E084` | `motivates` | 09-Problem Scenario | supply voltage fluctuations and noises(Uncertainty) |  | 16-Feature Extraction Algorithm | Scale Transform |  |
| 7 | `HLB8GC6H_E085` | `motivates` | 09-Problem Scenario | scale variation due to machine size and load conditions(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | 2-D correlation coefficient |  |
| 8 | `HLB8GC6H_E086` | `motivates` | 09-Problem Scenario | supply voltage fluctuations and noises(Uncertainty) |  | 17-Core Classifier Algorithm | 2-D correlation coefficient |  |
| 9 | `HLB8GC6H_E087` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Scale Transform |  |
| 10 | `HLB8GC6H_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | 2-D correlation coefficient |  |
| 11 | `HLB8GC6H_E089` | `motivates` | 06-Fault Severity | 1 broken bar, 2 broken bars(Multiple Severities) |  | 17-Core Classifier Algorithm | 2-D correlation coefficient |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `HLB8GC6H_E081`：scale variation due to machine size and load conditions(Distribution Discrepancy) motivates Discrete Wavelet Transform
  - `HLB8GC6H_E082`：supply voltage fluctuations and noises(Uncertainty) motivates Discrete Wavelet Transform
  - `HLB8GC6H_E083`：scale variation due to machine size and load conditions(Distribution Discrepancy) motivates Scale Transform
  - `HLB8GC6H_E084`：supply voltage fluctuations and noises(Uncertainty) motivates Scale Transform
  - `HLB8GC6H_E085`：scale variation due to machine size and load conditions(Distribution Discrepancy) motivates 2-D correlation coefficient
  - `HLB8GC6H_E086`：supply voltage fluctuations and noises(Uncertainty) motivates 2-D correlation coefficient
