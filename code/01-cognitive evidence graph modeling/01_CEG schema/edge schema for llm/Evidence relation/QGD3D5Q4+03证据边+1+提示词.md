...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `QGD3D5Q4_E079` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 15-Data Preprocessing Algorithm | TIR |  |
| 2 | `QGD3D5Q4_E080` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 16-Feature Extraction Algorithm | WCCSR |  |
| 3 | `QGD3D5Q4_E081` | `motivates` | 09-Problem Scenario | Doppler effect(Other) |  | 15-Data Preprocessing Algorithm | TIR |  |
| 4 | `QGD3D5Q4_E082` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 15-Data Preprocessing Algorithm | TIR |  |
| 5 | `QGD3D5Q4_E083` | `motivates` | 09-Problem Scenario | Doppler effect(Other) |  | 16-Feature Extraction Algorithm | WCCSR |  |
| 6 | `QGD3D5Q4_E084` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 16-Feature Extraction Algorithm | WCCSR |  |
| 7 | `QGD3D5Q4_E085` | `motivates` | 09-Problem Scenario | Doppler effect(Other) |  | 17-Core Classifier Algorithm | Envelope Spectrum Analysis |  |
| 8 | `QGD3D5Q4_E086` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope Spectrum Analysis |  |
| 9 | `QGD3D5Q4_E087` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | WCCSR |  |
| 10 | `QGD3D5Q4_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Envelope Spectrum Analysis |  |
| 11 | `QGD3D5Q4_E089` | `motivates` | 06-Fault Severity | 0.18 mm width(Single Severity) |  | 17-Core Classifier Algorithm | Envelope Spectrum Analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `QGD3D5Q4_E081`：Doppler effect(Other) motivates TIR
  - `QGD3D5Q4_E082`：heavy background noise(Uncertainty) motivates TIR
  - `QGD3D5Q4_E083`：Doppler effect(Other) motivates WCCSR
  - `QGD3D5Q4_E084`：heavy background noise(Uncertainty) motivates WCCSR
  - `QGD3D5Q4_E085`：Doppler effect(Other) motivates Envelope Spectrum Analysis
  - `QGD3D5Q4_E086`：heavy background noise(Uncertainty) motivates Envelope Spectrum Analysis
