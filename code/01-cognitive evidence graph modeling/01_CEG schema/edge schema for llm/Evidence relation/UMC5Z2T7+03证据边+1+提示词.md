...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `UMC5Z2T7_E084` | `motivates` | 13-Noise Level | Gaussian white noises(High Noise) |  | 16-Feature Extraction Algorithm | Canonical Variate Analysis |  |
| 2 | `UMC5Z2T7_E085` | `motivates` | 09-Problem Scenario | faulty dataset problem(Zero Fault Samples) |  | 16-Feature Extraction Algorithm | Canonical Variate Analysis |  |
| 3 | `UMC5Z2T7_E086` | `motivates` | 09-Problem Scenario | external noise(Uncertainty) |  | 16-Feature Extraction Algorithm | Canonical Variate Analysis |  |
| 4 | `UMC5Z2T7_E087` | `motivates` | 09-Problem Scenario | feature selection problem(Other) |  | 16-Feature Extraction Algorithm | Canonical Variate Analysis |  |
| 5 | `UMC5Z2T7_E088` | `motivates` | 09-Problem Scenario | faulty dataset problem(Zero Fault Samples) |  | 17-Core Classifier Algorithm | Kernel Density Estimation |  |
| 6 | `UMC5Z2T7_E089` | `motivates` | 09-Problem Scenario | external noise(Uncertainty) |  | 17-Core Classifier Algorithm | Kernel Density Estimation |  |
| 7 | `UMC5Z2T7_E090` | `motivates` | 09-Problem Scenario | feature selection problem(Other) |  | 17-Core Classifier Algorithm | Kernel Density Estimation |  |
| 8 | `UMC5Z2T7_E091` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Canonical Variate Analysis |  |
| 9 | `UMC5Z2T7_E092` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Kernel Density Estimation |  |
| 10 | `UMC5Z2T7_E093` | `motivates` | 06-Fault Severity | 0.007'', 0.014'', 0.021''(Multiple Severities) |  | 17-Core Classifier Algorithm | Kernel Density Estimation |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `UMC5Z2T7_E085`：faulty dataset problem(Zero Fault Samples) motivates Canonical Variate Analysis
  - `UMC5Z2T7_E086`：external noise(Uncertainty) motivates Canonical Variate Analysis
  - `UMC5Z2T7_E087`：feature selection problem(Other) motivates Canonical Variate Analysis
  - `UMC5Z2T7_E088`：faulty dataset problem(Zero Fault Samples) motivates Kernel Density Estimation
  - `UMC5Z2T7_E089`：external noise(Uncertainty) motivates Kernel Density Estimation
  - `UMC5Z2T7_E090`：feature selection problem(Other) motivates Kernel Density Estimation
