...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `CBAZUCRE_E214` | `motivates` | 13-Noise Level | presence of noise(High Noise) |  | 15-Data Preprocessing Algorithm | Wavelet Transform |  |
| 2 | `CBAZUCRE_E215` | `motivates` | 09-Problem Scenario | complex hybrid system with subsystems from different physical domains(Complex Systems) |  | 15-Data Preprocessing Algorithm | Wavelet Transform |  |
| 3 | `CBAZUCRE_E216` | `motivates` | 09-Problem Scenario | measurement noise corruption and threshold determination uncertainty(Uncertainty) |  | 15-Data Preprocessing Algorithm | Wavelet Transform |  |
| 4 | `CBAZUCRE_E217` | `motivates` | 09-Problem Scenario | complex hybrid system with subsystems from different physical domains(Complex Systems) |  | 17-Core Classifier Algorithm | Causality Inversion Method |  |
| 5 | `CBAZUCRE_E218` | `motivates` | 09-Problem Scenario | measurement noise corruption and threshold determination uncertainty(Uncertainty) |  | 17-Core Classifier Algorithm | Causality Inversion Method |  |
| 6 | `CBAZUCRE_E219` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Causality Inversion Method |  |
| 7 | `CBAZUCRE_E220` | `motivates` | 06-Fault Severity | 5% shorted, 10% reduction, pressure drop, broken tooth(Single Severity) |  | 17-Core Classifier Algorithm | Causality Inversion Method |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `CBAZUCRE_E215`：complex hybrid system with subsystems from different physical domains(Complex Systems) motivates Wavelet Transform
  - `CBAZUCRE_E216`：measurement noise corruption and threshold determination uncertainty(Uncertainty) motivates Wavelet Transform
  - `CBAZUCRE_E217`：complex hybrid system with subsystems from different physical domains(Complex Systems) motivates Causality Inversion Method
  - `CBAZUCRE_E218`：measurement noise corruption and threshold determination uncertainty(Uncertainty) motivates Causality Inversion Method
