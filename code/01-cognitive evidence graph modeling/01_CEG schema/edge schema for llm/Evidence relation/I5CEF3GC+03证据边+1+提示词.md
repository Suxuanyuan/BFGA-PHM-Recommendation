...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `I5CEF3GC_E108` | `motivates` | 09-Problem Scenario | smaller number of dataset(Small Fault Samples) |  | 19-Training Optimization Algorithm | Genetic Algorithm |  |
| 2 | `I5CEF3GC_E109` | `motivates` | 09-Problem Scenario | noise-contaminated sensor signals(Uncertainty) |  | 19-Training Optimization Algorithm | Genetic Algorithm |  |
| 3 | `I5CEF3GC_E110` | `motivates` | 12-Training Data Availability | Each condition has 40 samples, and there are total 160 samples, in which 80 samples will be randomly selected as the training data(Scarce) |  | 19-Training Optimization Algorithm | Genetic Algorithm |  |
| 4 | `I5CEF3GC_E111` | `motivates` | 13-Noise Level | noise-contaminated sensor signals(High Noise) |  | 15-Data Preprocessing Algorithm | Local Mean Decomposition |  |
| 5 | `I5CEF3GC_E112` | `motivates` | 13-Noise Level | noise-contaminated sensor signals(High Noise) |  | 16-Feature Extraction Algorithm | Multiscale Permutation Entropy |  |
| 6 | `I5CEF3GC_E113` | `motivates` | 09-Problem Scenario | smaller number of dataset(Small Fault Samples) |  | 15-Data Preprocessing Algorithm | Local Mean Decomposition |  |
| 7 | `I5CEF3GC_E114` | `motivates` | 09-Problem Scenario | noise-contaminated sensor signals(Uncertainty) |  | 15-Data Preprocessing Algorithm | Local Mean Decomposition |  |
| 8 | `I5CEF3GC_E115` | `motivates` | 09-Problem Scenario | smaller number of dataset(Small Fault Samples) |  | 16-Feature Extraction Algorithm | Multiscale Permutation Entropy |  |
| 9 | `I5CEF3GC_E116` | `motivates` | 09-Problem Scenario | noise-contaminated sensor signals(Uncertainty) |  | 16-Feature Extraction Algorithm | Multiscale Permutation Entropy |  |
| 10 | `I5CEF3GC_E117` | `motivates` | 09-Problem Scenario | smaller number of dataset(Small Fault Samples) |  | 17-Core Classifier Algorithm | Improved Support Vector Machine based Binary Tree |  |
| 11 | `I5CEF3GC_E118` | `motivates` | 09-Problem Scenario | noise-contaminated sensor signals(Uncertainty) |  | 17-Core Classifier Algorithm | Improved Support Vector Machine based Binary Tree |  |
| 12 | `I5CEF3GC_E119` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Multiscale Permutation Entropy |  |
| 13 | `I5CEF3GC_E120` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Improved Support Vector Machine based Binary Tree |  |
| 14 | `I5CEF3GC_E121` | `motivates` | 06-Fault Severity | 0.1778 mm(Single Severity) |  | 17-Core Classifier Algorithm | Improved Support Vector Machine based Binary Tree |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `I5CEF3GC_E108`：smaller number of dataset(Small Fault Samples) motivates Genetic Algorithm
  - `I5CEF3GC_E109`：noise-contaminated sensor signals(Uncertainty) motivates Genetic Algorithm
  - `I5CEF3GC_E113`：smaller number of dataset(Small Fault Samples) motivates Local Mean Decomposition
  - `I5CEF3GC_E114`：noise-contaminated sensor signals(Uncertainty) motivates Local Mean Decomposition
  - `I5CEF3GC_E115`：smaller number of dataset(Small Fault Samples) motivates Multiscale Permutation Entropy
  - `I5CEF3GC_E116`：noise-contaminated sensor signals(Uncertainty) motivates Multiscale Permutation Entropy
  - `I5CEF3GC_E117`：smaller number of dataset(Small Fault Samples) motivates Improved Support Vector Machine based Binary Tree
  - `I5CEF3GC_E118`：noise-contaminated sensor signals(Uncertainty) motivates Improved Support Vector Machine based Binary Tree
