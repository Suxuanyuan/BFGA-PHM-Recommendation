...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `35HRZGE3_E083` | `motivates` | 13-Noise Level | heavy background noise / heavy environmental noise(High Noise) |  | 15-Data Preprocessing Algorithm | LOWESS |  |
| 2 | `35HRZGE3_E084` | `motivates` | 13-Noise Level | heavy background noise / heavy environmental noise(High Noise) |  | 16-Feature Extraction Algorithm | IOVMD |  |
| 3 | `35HRZGE3_E085` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 15-Data Preprocessing Algorithm | LOWESS |  |
| 4 | `35HRZGE3_E086` | `motivates` | 09-Problem Scenario | compound fault(Compound Faults) |  | 15-Data Preprocessing Algorithm | LOWESS |  |
| 5 | `35HRZGE3_E087` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 16-Feature Extraction Algorithm | IOVMD |  |
| 6 | `35HRZGE3_E088` | `motivates` | 09-Problem Scenario | compound fault(Compound Faults) |  | 16-Feature Extraction Algorithm | IOVMD |  |
| 7 | `35HRZGE3_E089` | `motivates` | 09-Problem Scenario | heavy background noise(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 8 | `35HRZGE3_E090` | `motivates` | 09-Problem Scenario | compound fault(Compound Faults) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 9 | `35HRZGE3_E091` | `motivates` | 07-Compound Fault | compound high-speed locomotive bearing fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | IOVMD |  |
| 10 | `35HRZGE3_E092` | `motivates` | 07-Compound Fault | compound high-speed locomotive bearing fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 11 | `35HRZGE3_E093` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `35HRZGE3_E085`：heavy background noise(Uncertainty) motivates LOWESS
  - `35HRZGE3_E086`：compound fault(Compound Faults) motivates LOWESS
  - `35HRZGE3_E087`：heavy background noise(Uncertainty) motivates IOVMD
  - `35HRZGE3_E088`：compound fault(Compound Faults) motivates IOVMD
  - `35HRZGE3_E089`：heavy background noise(Uncertainty) motivates Envelope Analysis
  - `35HRZGE3_E090`：compound fault(Compound Faults) motivates Envelope Analysis
