...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `Y6LDGUXT_E127` | `motivates` | 09-Problem Scenario | distribution discrepancy(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | PK-MMD |  |
| 2 | `Y6LDGUXT_E128` | `motivates` | 09-Problem Scenario | unlabeled target domain data(Small Fault Samples) |  | 19-Training Optimization Algorithm | PK-MMD |  |
| 3 | `Y6LDGUXT_E129` | `motivates` | 12-Training Data Availability | 1616 samples in dataset A, 1088 samples in dataset B, 4368 samples in dataset C(Sufficient) |  | 19-Training Optimization Algorithm | PK-MMD |  |
| 4 | `Y6LDGUXT_E130` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | ResNet |  |
| 5 | `Y6LDGUXT_E131` | `motivates` | 09-Problem Scenario | distribution discrepancy(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | ResNet |  |
| 6 | `Y6LDGUXT_E132` | `motivates` | 09-Problem Scenario | unlabeled target domain data(Small Fault Samples) |  | 16-Feature Extraction Algorithm | ResNet |  |
| 7 | `Y6LDGUXT_E133` | `motivates` | 09-Problem Scenario | distribution discrepancy(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `Y6LDGUXT_E134` | `motivates` | 09-Problem Scenario | unlabeled target domain data(Small Fault Samples) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `Y6LDGUXT_E135` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | ResNet |  |
| 10 | `Y6LDGUXT_E136` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `Y6LDGUXT_E137` | `motivates` | 06-Fault Severity | 0.0014 inches(Single Severity) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `Y6LDGUXT_E127`：distribution discrepancy(Distribution Discrepancy) motivates PK-MMD
  - `Y6LDGUXT_E128`：unlabeled target domain data(Small Fault Samples) motivates PK-MMD
  - `Y6LDGUXT_E131`：distribution discrepancy(Distribution Discrepancy) motivates ResNet
  - `Y6LDGUXT_E132`：unlabeled target domain data(Small Fault Samples) motivates ResNet
  - `Y6LDGUXT_E133`：distribution discrepancy(Distribution Discrepancy) motivates Softmax
  - `Y6LDGUXT_E134`：unlabeled target domain data(Small Fault Samples) motivates Softmax
