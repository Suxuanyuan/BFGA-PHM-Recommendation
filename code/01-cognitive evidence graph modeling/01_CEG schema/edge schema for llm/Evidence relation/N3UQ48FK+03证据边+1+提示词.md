...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `N3UQ48FK_E099` | `motivates` | 09-Problem Scenario | limited training data set(Small Fault Samples) |  | 19-Training Optimization Algorithm | GA |  |
| 2 | `N3UQ48FK_E100` | `motivates` | 09-Problem Scenario | noise interference and non-Gaussian feature distribution(Uncertainty) |  | 19-Training Optimization Algorithm | GA |  |
| 3 | `N3UQ48FK_E101` | `motivates` | 12-Training Data Availability | 90 1.5-second AE signals / 30 feature vectors for each bearing condition in the training set(Scarce) |  | 19-Training Optimization Algorithm | GA |  |
| 4 | `N3UQ48FK_E102` | `motivates` | 13-Noise Level | background noise level increases with the rotational speed(High Noise) |  | 15-Data Preprocessing Algorithm | WPT |  |
| 5 | `N3UQ48FK_E103` | `motivates` | 13-Noise Level | background noise level increases with the rotational speed(High Noise) |  | 16-Feature Extraction Algorithm | REWPN |  |
| 6 | `N3UQ48FK_E104` | `motivates` | 09-Problem Scenario | limited training data set(Small Fault Samples) |  | 15-Data Preprocessing Algorithm | WPT |  |
| 7 | `N3UQ48FK_E105` | `motivates` | 09-Problem Scenario | noise interference and non-Gaussian feature distribution(Uncertainty) |  | 15-Data Preprocessing Algorithm | WPT |  |
| 8 | `N3UQ48FK_E106` | `motivates` | 09-Problem Scenario | limited training data set(Small Fault Samples) |  | 16-Feature Extraction Algorithm | REWPN |  |
| 9 | `N3UQ48FK_E107` | `motivates` | 09-Problem Scenario | noise interference and non-Gaussian feature distribution(Uncertainty) |  | 16-Feature Extraction Algorithm | REWPN |  |
| 10 | `N3UQ48FK_E108` | `motivates` | 09-Problem Scenario | limited training data set(Small Fault Samples) |  | 17-Core Classifier Algorithm | MCSVM |  |
| 11 | `N3UQ48FK_E109` | `motivates` | 09-Problem Scenario | noise interference and non-Gaussian feature distribution(Uncertainty) |  | 17-Core Classifier Algorithm | MCSVM |  |
| 12 | `N3UQ48FK_E110` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | REWPN |  |
| 13 | `N3UQ48FK_E111` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | MCSVM |  |
| 14 | `N3UQ48FK_E112` | `motivates` | 06-Fault Severity | width: 0.1 mm, 0.6 mm, 0.7 mm, 1.6 mm(Single Severity) |  | 17-Core Classifier Algorithm | MCSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `N3UQ48FK_E099`：limited training data set(Small Fault Samples) motivates GA
  - `N3UQ48FK_E100`：noise interference and non-Gaussian feature distribution(Uncertainty) motivates GA
  - `N3UQ48FK_E104`：limited training data set(Small Fault Samples) motivates WPT
  - `N3UQ48FK_E105`：noise interference and non-Gaussian feature distribution(Uncertainty) motivates WPT
  - `N3UQ48FK_E106`：limited training data set(Small Fault Samples) motivates REWPN
  - `N3UQ48FK_E107`：noise interference and non-Gaussian feature distribution(Uncertainty) motivates REWPN
  - `N3UQ48FK_E108`：limited training data set(Small Fault Samples) motivates MCSVM
  - `N3UQ48FK_E109`：noise interference and non-Gaussian feature distribution(Uncertainty) motivates MCSVM
