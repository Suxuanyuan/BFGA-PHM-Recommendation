...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `DTEIDYRY_E102` | `motivates` | 09-Problem Scenario | uncertain modeling parameters(Uncertainty) |  | 19-Training Optimization Algorithm | Orthogonality Principle |  |
| 2 | `DTEIDYRY_E103` | `motivates` | 12-Training Data Availability | each mode has 50 sampling times(Sufficient) |  | 19-Training Optimization Algorithm | Orthogonality Principle |  |
| 3 | `DTEIDYRY_E104` | `motivates` | 13-Noise Level | noisy measurements(High Noise) |  | 16-Feature Extraction Algorithm | Kalman Filter |  |
| 4 | `DTEIDYRY_E105` | `motivates` | 09-Problem Scenario | uncertain modeling parameters(Uncertainty) |  | 16-Feature Extraction Algorithm | Kalman Filter |  |
| 5 | `DTEIDYRY_E106` | `motivates` | 09-Problem Scenario | uncertain modeling parameters(Uncertainty) |  | 17-Core Classifier Algorithm | RIMM |  |
| 6 | `DTEIDYRY_E107` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Kalman Filter |  |
| 7 | `DTEIDYRY_E108` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | RIMM |  |
| 8 | `DTEIDYRY_E109` | `motivates` | 06-Fault Severity | opened to 90%, 60% power, reduced by 40% and 70%(Single Severity) |  | 17-Core Classifier Algorithm | RIMM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `DTEIDYRY_E102`：uncertain modeling parameters(Uncertainty) motivates Orthogonality Principle
  - `DTEIDYRY_E105`：uncertain modeling parameters(Uncertainty) motivates Kalman Filter
  - `DTEIDYRY_E106`：uncertain modeling parameters(Uncertainty) motivates RIMM
