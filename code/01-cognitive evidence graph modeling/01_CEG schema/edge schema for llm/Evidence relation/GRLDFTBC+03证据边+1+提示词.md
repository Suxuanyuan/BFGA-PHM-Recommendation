...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `GRLDFTBC_E108` | `motivates` | 09-Problem Scenario | domain adaptation for cross-location and cross-machine tasks(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | SDRS |  |
| 2 | `GRLDFTBC_E109` | `motivates` | 09-Problem Scenario | low confidence of predicted labels and confusion interval(Uncertainty) |  | 19-Training Optimization Algorithm | SDRS |  |
| 3 | `GRLDFTBC_E110` | `motivates` | 12-Training Data Availability | 4000 samples per dataset(Sufficient) |  | 19-Training Optimization Algorithm | SDRS |  |
| 4 | `GRLDFTBC_E111` | `motivates` | 13-Noise Level | additional noise, noise background(High Noise) |  | 16-Feature Extraction Algorithm | CNN |  |
| 5 | `GRLDFTBC_E112` | `motivates` | 09-Problem Scenario | domain adaptation for cross-location and cross-machine tasks(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | CNN |  |
| 6 | `GRLDFTBC_E113` | `motivates` | 09-Problem Scenario | low confidence of predicted labels and confusion interval(Uncertainty) |  | 16-Feature Extraction Algorithm | CNN |  |
| 7 | `GRLDFTBC_E114` | `motivates` | 09-Problem Scenario | domain adaptation for cross-location and cross-machine tasks(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `GRLDFTBC_E115` | `motivates` | 09-Problem Scenario | low confidence of predicted labels and confusion interval(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `GRLDFTBC_E116` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CNN |  |
| 10 | `GRLDFTBC_E117` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `GRLDFTBC_E118` | `motivates` | 06-Fault Severity | 0.007 inches(Single Severity) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `GRLDFTBC_E108`：domain adaptation for cross-location and cross-machine tasks(Distribution Discrepancy) motivates SDRS
  - `GRLDFTBC_E109`：low confidence of predicted labels and confusion interval(Uncertainty) motivates SDRS
  - `GRLDFTBC_E112`：domain adaptation for cross-location and cross-machine tasks(Distribution Discrepancy) motivates CNN
  - `GRLDFTBC_E113`：low confidence of predicted labels and confusion interval(Uncertainty) motivates CNN
  - `GRLDFTBC_E114`：domain adaptation for cross-location and cross-machine tasks(Distribution Discrepancy) motivates Softmax
  - `GRLDFTBC_E115`：low confidence of predicted labels and confusion interval(Uncertainty) motivates Softmax
