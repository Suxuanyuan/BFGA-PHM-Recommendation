...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `IDP29JMR_E071` | `motivates` | 13-Noise Level | noisy nonlinear systems, measurement noise, external disturbances(High Noise) |  | 16-Feature Extraction Algorithm | FFT |  |
| 2 | `IDP29JMR_E072` | `motivates` | 09-Problem Scenario | parametric variation due to the motor heating or other perturbation(Uncertainty) |  | 16-Feature Extraction Algorithm | FFT |  |
| 3 | `IDP29JMR_E073` | `motivates` | 09-Problem Scenario | closed-loop control masks the fault effect(Other) |  | 16-Feature Extraction Algorithm | FFT |  |
| 4 | `IDP29JMR_E074` | `motivates` | 09-Problem Scenario | parametric variation due to the motor heating or other perturbation(Uncertainty) |  | 17-Core Classifier Algorithm | EKF |  |
| 5 | `IDP29JMR_E075` | `motivates` | 09-Problem Scenario | closed-loop control masks the fault effect(Other) |  | 17-Core Classifier Algorithm | EKF |  |
| 6 | `IDP29JMR_E076` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | FFT |  |
| 7 | `IDP29JMR_E077` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | EKF |  |
| 8 | `IDP29JMR_E078` | `motivates` | 06-Fault Severity | adjacent two broken rotor bars, 3.1mm diameter hole(Single Severity) |  | 17-Core Classifier Algorithm | EKF |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `IDP29JMR_E072`：parametric variation due to the motor heating or other perturbation(Uncertainty) motivates FFT
  - `IDP29JMR_E073`：closed-loop control masks the fault effect(Other) motivates FFT
  - `IDP29JMR_E074`：parametric variation due to the motor heating or other perturbation(Uncertainty) motivates EKF
  - `IDP29JMR_E075`：closed-loop control masks the fault effect(Other) motivates EKF
