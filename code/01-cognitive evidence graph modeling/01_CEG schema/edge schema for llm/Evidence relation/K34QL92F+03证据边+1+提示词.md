...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `K34QL92F_E113` | `motivates` | 13-Noise Level | different degrees of signal-to-noise (SNR) is added into the original bearing signals... TFT method has relatively high diagnosis accuracy under noise interference environment(High Noise) |  | 15-Data Preprocessing Algorithm | Synchrosqueezed Wavelet Transform |  |
| 2 | `K34QL92F_E114` | `motivates` | 13-Noise Level | different degrees of signal-to-noise (SNR) is added into the original bearing signals... TFT method has relatively high diagnosis accuracy under noise interference environment(High Noise) |  | 16-Feature Extraction Algorithm | Transformer Encoder |  |
| 3 | `K34QL92F_E115` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | Synchrosqueezed Wavelet Transform |  |
| 4 | `K34QL92F_E116` | `motivates` | 09-Problem Scenario | multi-channel TFRs(Multi-Source Heterogeneous / Multimodal Data) |  | 15-Data Preprocessing Algorithm | Synchrosqueezed Wavelet Transform |  |
| 5 | `K34QL92F_E117` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | Transformer Encoder |  |
| 6 | `K34QL92F_E118` | `motivates` | 09-Problem Scenario | multi-channel TFRs(Multi-Source Heterogeneous / Multimodal Data) |  | 16-Feature Extraction Algorithm | Transformer Encoder |  |
| 7 | `K34QL92F_E119` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | MLP |  |
| 8 | `K34QL92F_E120` | `motivates` | 09-Problem Scenario | multi-channel TFRs(Multi-Source Heterogeneous / Multimodal Data) |  | 17-Core Classifier Algorithm | MLP |  |
| 9 | `K34QL92F_E121` | `motivates` | 07-Compound Fault | compound fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Transformer Encoder |  |
| 10 | `K34QL92F_E122` | `motivates` | 07-Compound Fault | compound fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | MLP |  |
| 11 | `K34QL92F_E123` | `motivates` | 06-Fault Severity | weak fault / fault(Multiple Severities) |  | 17-Core Classifier Algorithm | MLP |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `K34QL92F_E115`：noise robustness(Uncertainty) motivates Synchrosqueezed Wavelet Transform
  - `K34QL92F_E116`：multi-channel TFRs(Multi-Source Heterogeneous / Multimodal Data) motivates Synchrosqueezed Wavelet Transform
  - `K34QL92F_E117`：noise robustness(Uncertainty) motivates Transformer Encoder
  - `K34QL92F_E118`：multi-channel TFRs(Multi-Source Heterogeneous / Multimodal Data) motivates Transformer Encoder
  - `K34QL92F_E119`：noise robustness(Uncertainty) motivates MLP
  - `K34QL92F_E120`：multi-channel TFRs(Multi-Source Heterogeneous / Multimodal Data) motivates MLP
