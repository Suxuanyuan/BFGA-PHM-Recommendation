...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `AQQ8L44N_E125` | `motivates` | 09-Problem Scenario | large fault samples are difficult to obtain in practice(Small Fault Samples) |  | 19-Training Optimization Algorithm | Mutual Information |  |
| 2 | `AQQ8L44N_E126` | `motivates` | 12-Training Data Availability | 160 experimental datasets(Scarce) |  | 19-Training Optimization Algorithm | Mutual Information |  |
| 3 | `AQQ8L44N_E127` | `motivates` | 13-Noise Level | noise(High Noise) |  | 16-Feature Extraction Algorithm | Multiscale Entropy |  |
| 4 | `AQQ8L44N_E128` | `motivates` | 09-Problem Scenario | large fault samples are difficult to obtain in practice(Small Fault Samples) |  | 16-Feature Extraction Algorithm | Multiscale Entropy |  |
| 5 | `AQQ8L44N_E129` | `motivates` | 09-Problem Scenario | large fault samples are difficult to obtain in practice(Small Fault Samples) |  | 17-Core Classifier Algorithm | Support Vector Machine |  |
| 6 | `AQQ8L44N_E130` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Multiscale Entropy |  |
| 7 | `AQQ8L44N_E131` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Support Vector Machine |  |
| 8 | `AQQ8L44N_E132` | `motivates` | 06-Fault Severity | unbalance mass: 8.4 g; broken bars: 12; rotor deflection: 0.075 mm(Single Severity) |  | 17-Core Classifier Algorithm | Support Vector Machine |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `AQQ8L44N_E125`：large fault samples are difficult to obtain in practice(Small Fault Samples) motivates Mutual Information
  - `AQQ8L44N_E128`：large fault samples are difficult to obtain in practice(Small Fault Samples) motivates Multiscale Entropy
  - `AQQ8L44N_E129`：large fault samples are difficult to obtain in practice(Small Fault Samples) motivates Support Vector Machine
