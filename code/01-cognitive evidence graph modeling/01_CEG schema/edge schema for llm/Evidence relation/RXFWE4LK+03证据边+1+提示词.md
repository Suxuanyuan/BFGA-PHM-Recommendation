...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `RXFWE4LK_E075` | `motivates` | 13-Noise Level | Useless harmonics eliminated by digital low-pass filters(Normal) |  | 15-Data Preprocessing Algorithm | Digital low-pass filter |  |
| 2 | `RXFWE4LK_E076` | `motivates` | 13-Noise Level | Useless harmonics eliminated by digital low-pass filters(Normal) |  | 16-Feature Extraction Algorithm | Coordinate projection |  |
| 3 | `RXFWE4LK_E077` | `motivates` | 09-Problem Scenario | load variation(Uncertainty) |  | 15-Data Preprocessing Algorithm | Digital low-pass filter |  |
| 4 | `RXFWE4LK_E078` | `motivates` | 09-Problem Scenario | nonintrusive diagnostics without extra sensors(Other) |  | 15-Data Preprocessing Algorithm | Digital low-pass filter |  |
| 5 | `RXFWE4LK_E079` | `motivates` | 09-Problem Scenario | load variation(Uncertainty) |  | 16-Feature Extraction Algorithm | Coordinate projection |  |
| 6 | `RXFWE4LK_E080` | `motivates` | 09-Problem Scenario | nonintrusive diagnostics without extra sensors(Other) |  | 16-Feature Extraction Algorithm | Coordinate projection |  |
| 7 | `RXFWE4LK_E081` | `motivates` | 09-Problem Scenario | load variation(Uncertainty) |  | 17-Core Classifier Algorithm | Trajectory distortion analysis |  |
| 8 | `RXFWE4LK_E082` | `motivates` | 09-Problem Scenario | nonintrusive diagnostics without extra sensors(Other) |  | 17-Core Classifier Algorithm | Trajectory distortion analysis |  |
| 9 | `RXFWE4LK_E083` | `motivates` | 07-Compound Fault | double switches open-circuit faults(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Coordinate projection |  |
| 10 | `RXFWE4LK_E084` | `motivates` | 07-Compound Fault | double switches open-circuit faults(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Trajectory distortion analysis |  |
| 11 | `RXFWE4LK_E085` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Trajectory distortion analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `RXFWE4LK_E077`：load variation(Uncertainty) motivates Digital low-pass filter
  - `RXFWE4LK_E078`：nonintrusive diagnostics without extra sensors(Other) motivates Digital low-pass filter
  - `RXFWE4LK_E079`：load variation(Uncertainty) motivates Coordinate projection
  - `RXFWE4LK_E080`：nonintrusive diagnostics without extra sensors(Other) motivates Coordinate projection
  - `RXFWE4LK_E081`：load variation(Uncertainty) motivates Trajectory distortion analysis
  - `RXFWE4LK_E082`：nonintrusive diagnostics without extra sensors(Other) motivates Trajectory distortion analysis
