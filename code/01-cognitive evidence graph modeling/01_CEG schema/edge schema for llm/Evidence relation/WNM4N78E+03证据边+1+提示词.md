...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `WNM4N78E_E072` | `motivates` | 09-Problem Scenario | no data is available about faulty behaviors(Zero Fault Samples) |  | 17-Core Classifier Algorithm | AuDyC |  |
| 2 | `WNM4N78E_E073` | `motivates` | 09-Problem Scenario | multiple switch faults(Compound Faults) |  | 17-Core Classifier Algorithm | AuDyC |  |
| 3 | `WNM4N78E_E074` | `motivates` | 09-Problem Scenario | drift-like fault / early stage fault detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | AuDyC |  |
| 4 | `WNM4N78E_E075` | `motivates` | 07-Compound Fault | multiple switch faults(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | AuDyC |  |
| 5 | `WNM4N78E_E076` | `motivates` | 06-Fault Severity | drift-like fault(Multiple Severities) |  | 17-Core Classifier Algorithm | AuDyC |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `WNM4N78E_E072`：no data is available about faulty behaviors(Zero Fault Samples) motivates AuDyC
  - `WNM4N78E_E073`：multiple switch faults(Compound Faults) motivates AuDyC
  - `WNM4N78E_E074`：drift-like fault / early stage fault detection(Early Degradation Prediction) motivates AuDyC
