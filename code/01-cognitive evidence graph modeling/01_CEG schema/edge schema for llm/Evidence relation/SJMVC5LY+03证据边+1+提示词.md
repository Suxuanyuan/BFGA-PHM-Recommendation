...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `SJMVC5LY_E125` | `motivates` | 13-Noise Level | raw vibration signals(Normal) |  | 15-Data Preprocessing Algorithm | Min-Max Normalization |  |
| 2 | `SJMVC5LY_E126` | `motivates` | 13-Noise Level | raw vibration signals(Normal) |  | 16-Feature Extraction Algorithm | MWT |  |
| 3 | `SJMVC5LY_E127` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 15-Data Preprocessing Algorithm | Min-Max Normalization |  |
| 4 | `SJMVC5LY_E128` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 16-Feature Extraction Algorithm | MWT |  |
| 5 | `SJMVC5LY_E129` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 17-Core Classifier Algorithm | WSVM |  |
| 6 | `SJMVC5LY_E130` | `motivates` | 07-Compound Fault | bearing with combined fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | MWT |  |
| 7 | `SJMVC5LY_E131` | `motivates` | 07-Compound Fault | bearing with combined fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | WSVM |  |
| 8 | `SJMVC5LY_E132` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | WSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `SJMVC5LY_E127`：compound faults(Compound Faults) motivates Min-Max Normalization
  - `SJMVC5LY_E128`：compound faults(Compound Faults) motivates MWT
  - `SJMVC5LY_E129`：compound faults(Compound Faults) motivates WSVM
