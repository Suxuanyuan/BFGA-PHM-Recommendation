...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `IUYAHCGS_E106` | `motivates` | 13-Noise Level | process noise and measurement noise(High Noise) |  | 16-Feature Extraction Algorithm | Stable Image Representation |  |
| 2 | `IUYAHCGS_E107` | `motivates` | 09-Problem Scenario | incipient faults(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | Stable Image Representation |  |
| 3 | `IUYAHCGS_E108` | `motivates` | 09-Problem Scenario | disturbances or noises, modeling error(Uncertainty) |  | 16-Feature Extraction Algorithm | Stable Image Representation |  |
| 4 | `IUYAHCGS_E109` | `motivates` | 09-Problem Scenario | incipient faults(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Data-Driven Gap Metric |  |
| 5 | `IUYAHCGS_E110` | `motivates` | 09-Problem Scenario | disturbances or noises, modeling error(Uncertainty) |  | 17-Core Classifier Algorithm | Data-Driven Gap Metric |  |
| 6 | `IUYAHCGS_E111` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Stable Image Representation |  |
| 7 | `IUYAHCGS_E112` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Data-Driven Gap Metric |  |
| 8 | `IUYAHCGS_E113` | `motivates` | 06-Fault Severity | theta, ς, R, L1, C1 variations(Multiple Severities) |  | 17-Core Classifier Algorithm | Data-Driven Gap Metric |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `IUYAHCGS_E107`：incipient faults(Early Degradation Prediction) motivates Stable Image Representation
  - `IUYAHCGS_E108`：disturbances or noises, modeling error(Uncertainty) motivates Stable Image Representation
  - `IUYAHCGS_E109`：incipient faults(Early Degradation Prediction) motivates Data-Driven Gap Metric
  - `IUYAHCGS_E110`：disturbances or noises, modeling error(Uncertainty) motivates Data-Driven Gap Metric
