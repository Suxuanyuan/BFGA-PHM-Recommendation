...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `UCVHTKKU_E080` | `motivates` | 13-Noise Level | the white noise with SNR of 0.5 is also added to the original vibration signal(High Noise) |  | 15-Data Preprocessing Algorithm | LCD |  |
| 2 | `UCVHTKKU_E081` | `motivates` | 13-Noise Level | the white noise with SNR of 0.5 is also added to the original vibration signal(High Noise) |  | 16-Feature Extraction Algorithm | MF-DFA |  |
| 3 | `UCVHTKKU_E082` | `motivates` | 09-Problem Scenario | variation in working conditions(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | LCD |  |
| 4 | `UCVHTKKU_E083` | `motivates` | 09-Problem Scenario | external noise / contaminated vibration signals(Uncertainty) |  | 15-Data Preprocessing Algorithm | LCD |  |
| 5 | `UCVHTKKU_E084` | `motivates` | 09-Problem Scenario | variation in working conditions(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | MF-DFA |  |
| 6 | `UCVHTKKU_E085` | `motivates` | 09-Problem Scenario | external noise / contaminated vibration signals(Uncertainty) |  | 16-Feature Extraction Algorithm | MF-DFA |  |
| 7 | `UCVHTKKU_E086` | `motivates` | 09-Problem Scenario | variation in working conditions(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | PCA |  |
| 8 | `UCVHTKKU_E087` | `motivates` | 09-Problem Scenario | external noise / contaminated vibration signals(Uncertainty) |  | 17-Core Classifier Algorithm | PCA |  |
| 9 | `UCVHTKKU_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | MF-DFA |  |
| 10 | `UCVHTKKU_E089` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | PCA |  |
| 11 | `UCVHTKKU_E090` | `motivates` | 06-Fault Severity | fault diameters of 7, 14, and 21 mils(Multiple Severities) |  | 17-Core Classifier Algorithm | PCA |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `UCVHTKKU_E082`：variation in working conditions(Distribution Discrepancy) motivates LCD
  - `UCVHTKKU_E083`：external noise / contaminated vibration signals(Uncertainty) motivates LCD
  - `UCVHTKKU_E084`：variation in working conditions(Distribution Discrepancy) motivates MF-DFA
  - `UCVHTKKU_E085`：external noise / contaminated vibration signals(Uncertainty) motivates MF-DFA
  - `UCVHTKKU_E086`：variation in working conditions(Distribution Discrepancy) motivates PCA
  - `UCVHTKKU_E087`：external noise / contaminated vibration signals(Uncertainty) motivates PCA
