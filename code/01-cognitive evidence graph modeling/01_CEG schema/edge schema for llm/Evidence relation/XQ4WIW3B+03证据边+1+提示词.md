...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `XQ4WIW3B_E100` | `motivates` | 13-Noise Level | system non-idealities and noise on the controller action(High Noise) |  | 15-Data Preprocessing Algorithm | LPF |  |
| 2 | `XQ4WIW3B_E101` | `motivates` | 13-Noise Level | system non-idealities and noise on the controller action(High Noise) |  | 16-Feature Extraction Algorithm | FIS |  |
| 3 | `XQ4WIW3B_E102` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | LPF |  |
| 4 | `XQ4WIW3B_E103` | `motivates` | 09-Problem Scenario | closed-loop control masking fault signatures(Other) |  | 15-Data Preprocessing Algorithm | LPF |  |
| 5 | `XQ4WIW3B_E104` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | FIS |  |
| 6 | `XQ4WIW3B_E105` | `motivates` | 09-Problem Scenario | closed-loop control masking fault signatures(Other) |  | 16-Feature Extraction Algorithm | FIS |  |
| 7 | `XQ4WIW3B_E106` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Thresholding |  |
| 8 | `XQ4WIW3B_E107` | `motivates` | 09-Problem Scenario | closed-loop control masking fault signatures(Other) |  | 17-Core Classifier Algorithm | Thresholding |  |
| 9 | `XQ4WIW3B_E108` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | FIS |  |
| 10 | `XQ4WIW3B_E109` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Thresholding |  |
| 11 | `XQ4WIW3B_E110` | `motivates` | 06-Fault Severity | two, three, and five turns(Multiple Severities) |  | 17-Core Classifier Algorithm | Thresholding |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `XQ4WIW3B_E102`：incipient fault detection(Early Degradation Prediction) motivates LPF
  - `XQ4WIW3B_E103`：closed-loop control masking fault signatures(Other) motivates LPF
  - `XQ4WIW3B_E104`：incipient fault detection(Early Degradation Prediction) motivates FIS
  - `XQ4WIW3B_E105`：closed-loop control masking fault signatures(Other) motivates FIS
  - `XQ4WIW3B_E106`：incipient fault detection(Early Degradation Prediction) motivates Thresholding
  - `XQ4WIW3B_E107`：closed-loop control masking fault signatures(Other) motivates Thresholding
