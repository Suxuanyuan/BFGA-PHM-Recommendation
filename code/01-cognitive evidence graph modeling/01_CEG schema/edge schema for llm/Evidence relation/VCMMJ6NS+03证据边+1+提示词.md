...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `VCMMJ6NS_E079` | `motivates` | 13-Noise Level | additive white Gaussian noise, strong background noise(High Noise) |  | 15-Data Preprocessing Algorithm | Fast Spectral Kurtosis |  |
| 2 | `VCMMJ6NS_E080` | `motivates` | 13-Noise Level | additive white Gaussian noise, strong background noise(High Noise) |  | 16-Feature Extraction Algorithm | Synchroextracting Transform |  |
| 3 | `VCMMJ6NS_E081` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | Fast Spectral Kurtosis |  |
| 4 | `VCMMJ6NS_E082` | `motivates` | 09-Problem Scenario | resampling-free order tracking(Other) |  | 15-Data Preprocessing Algorithm | Fast Spectral Kurtosis |  |
| 5 | `VCMMJ6NS_E083` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | Synchroextracting Transform |  |
| 6 | `VCMMJ6NS_E084` | `motivates` | 09-Problem Scenario | resampling-free order tracking(Other) |  | 16-Feature Extraction Algorithm | Synchroextracting Transform |  |
| 7 | `VCMMJ6NS_E085` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | Generalized Demodulation |  |
| 8 | `VCMMJ6NS_E086` | `motivates` | 09-Problem Scenario | resampling-free order tracking(Other) |  | 17-Core Classifier Algorithm | Generalized Demodulation |  |
| 9 | `VCMMJ6NS_E087` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Synchroextracting Transform |  |
| 10 | `VCMMJ6NS_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Generalized Demodulation |  |
| 11 | `VCMMJ6NS_E089` | `motivates` | 06-Fault Severity | 0.2 mm(Single Severity) |  | 17-Core Classifier Algorithm | Generalized Demodulation |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `VCMMJ6NS_E081`：noise robustness(Uncertainty) motivates Fast Spectral Kurtosis
  - `VCMMJ6NS_E082`：resampling-free order tracking(Other) motivates Fast Spectral Kurtosis
  - `VCMMJ6NS_E083`：noise robustness(Uncertainty) motivates Synchroextracting Transform
  - `VCMMJ6NS_E084`：resampling-free order tracking(Other) motivates Synchroextracting Transform
  - `VCMMJ6NS_E085`：noise robustness(Uncertainty) motivates Generalized Demodulation
  - `VCMMJ6NS_E086`：resampling-free order tracking(Other) motivates Generalized Demodulation
