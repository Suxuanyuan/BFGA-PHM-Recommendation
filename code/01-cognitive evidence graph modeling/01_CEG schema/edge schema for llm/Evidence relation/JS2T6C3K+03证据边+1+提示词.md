...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `JS2T6C3K_E103` | `motivates` | 13-Noise Level | Gaussian white noise, Gaussian noise(High Noise) |  | 15-Data Preprocessing Algorithm | VSST |  |
| 2 | `JS2T6C3K_E104` | `motivates` | 13-Noise Level | Gaussian white noise, Gaussian noise(High Noise) |  | 16-Feature Extraction Algorithm | GLCM |  |
| 3 | `JS2T6C3K_E105` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | VSST |  |
| 4 | `JS2T6C3K_E106` | `motivates` | 09-Problem Scenario | combined inner and outer ring fault(Compound Faults) |  | 15-Data Preprocessing Algorithm | VSST |  |
| 5 | `JS2T6C3K_E107` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | GLCM |  |
| 6 | `JS2T6C3K_E108` | `motivates` | 09-Problem Scenario | combined inner and outer ring fault(Compound Faults) |  | 16-Feature Extraction Algorithm | GLCM |  |
| 7 | `JS2T6C3K_E109` | `motivates` | 09-Problem Scenario | noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | LS-SVM |  |
| 8 | `JS2T6C3K_E110` | `motivates` | 09-Problem Scenario | combined inner and outer ring fault(Compound Faults) |  | 17-Core Classifier Algorithm | LS-SVM |  |
| 9 | `JS2T6C3K_E111` | `motivates` | 07-Compound Fault | combined inner and outer ring fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | GLCM |  |
| 10 | `JS2T6C3K_E112` | `motivates` | 07-Compound Fault | combined inner and outer ring fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | LS-SVM |  |
| 11 | `JS2T6C3K_E113` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | LS-SVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `JS2T6C3K_E105`：noise robustness(Uncertainty) motivates VSST
  - `JS2T6C3K_E106`：combined inner and outer ring fault(Compound Faults) motivates VSST
  - `JS2T6C3K_E107`：noise robustness(Uncertainty) motivates GLCM
  - `JS2T6C3K_E108`：combined inner and outer ring fault(Compound Faults) motivates GLCM
  - `JS2T6C3K_E109`：noise robustness(Uncertainty) motivates LS-SVM
  - `JS2T6C3K_E110`：combined inner and outer ring fault(Compound Faults) motivates LS-SVM
