...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `8TLFWPD2_E113` | `motivates` | 13-Noise Level | process and measurement noise(High Noise) |  | 16-Feature Extraction Algorithm | SRCKF |  |
| 2 | `8TLFWPD2_E114` | `motivates` | 09-Problem Scenario | concurrent sensor faults(Compound Faults) |  | 16-Feature Extraction Algorithm | SRCKF |  |
| 3 | `8TLFWPD2_E115` | `motivates` | 09-Problem Scenario | process and measurement noise, modeling uncertainties(Uncertainty) |  | 16-Feature Extraction Algorithm | SRCKF |  |
| 4 | `8TLFWPD2_E116` | `motivates` | 09-Problem Scenario | concurrent sensor faults(Compound Faults) |  | 17-Core Classifier Algorithm | DBSCAN |  |
| 5 | `8TLFWPD2_E117` | `motivates` | 09-Problem Scenario | process and measurement noise, modeling uncertainties(Uncertainty) |  | 17-Core Classifier Algorithm | DBSCAN |  |
| 6 | `8TLFWPD2_E118` | `motivates` | 07-Compound Fault | concurrent sensor faults(Compound Fault Across Structures) |  | 16-Feature Extraction Algorithm | SRCKF |  |
| 7 | `8TLFWPD2_E119` | `motivates` | 07-Compound Fault | concurrent sensor faults(Compound Fault Across Structures) |  | 17-Core Classifier Algorithm | DBSCAN |  |
| 8 | `8TLFWPD2_E120` | `motivates` | 06-Fault Severity | maximum deviations of 3% and drift duration of 100 s(Multiple Severities) |  | 17-Core Classifier Algorithm | DBSCAN |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `8TLFWPD2_E114`：concurrent sensor faults(Compound Faults) motivates SRCKF
  - `8TLFWPD2_E115`：process and measurement noise, modeling uncertainties(Uncertainty) motivates SRCKF
  - `8TLFWPD2_E116`：concurrent sensor faults(Compound Faults) motivates DBSCAN
  - `8TLFWPD2_E117`：process and measurement noise, modeling uncertainties(Uncertainty) motivates DBSCAN
