...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `BTP3HNTC_E083` | `motivates` | 09-Problem Scenario | multiple faults (including actuator faults and sensor faults)(Compound Faults) |  | 19-Training Optimization Algorithm | Linear Matrix Inequality |  |
| 2 | `BTP3HNTC_E084` | `motivates` | 09-Problem Scenario | process disturbances and measurement noises(Uncertainty) |  | 19-Training Optimization Algorithm | Linear Matrix Inequality |  |
| 3 | `BTP3HNTC_E085` | `motivates` | 12-Training Data Availability | Sufficient |  | 19-Training Optimization Algorithm | Linear Matrix Inequality |  |
| 4 | `BTP3HNTC_E086` | `motivates` | 09-Problem Scenario | multiple faults (including actuator faults and sensor faults)(Compound Faults) |  | 17-Core Classifier Algorithm | Descriptor Observer |  |
| 5 | `BTP3HNTC_E087` | `motivates` | 09-Problem Scenario | process disturbances and measurement noises(Uncertainty) |  | 17-Core Classifier Algorithm | Descriptor Observer |  |
| 6 | `BTP3HNTC_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Descriptor Observer |  |
| 7 | `BTP3HNTC_E089` | `motivates` | 06-Fault Severity | 80% offset of the input signal, slope rate -0.1(Single Severity) |  | 17-Core Classifier Algorithm | Descriptor Observer |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `BTP3HNTC_E083`：multiple faults (including actuator faults and sensor faults)(Compound Faults) motivates Linear Matrix Inequality
  - `BTP3HNTC_E084`：process disturbances and measurement noises(Uncertainty) motivates Linear Matrix Inequality
  - `BTP3HNTC_E086`：multiple faults (including actuator faults and sensor faults)(Compound Faults) motivates Descriptor Observer
  - `BTP3HNTC_E087`：process disturbances and measurement noises(Uncertainty) motivates Descriptor Observer
