...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `YHR5CQ8D_E104` | `motivates` | 13-Noise Level | noisy simulation fault signals, submerged in the noise, low SNR signal(High Noise) |  | 15-Data Preprocessing Algorithm | Signal Segmentation |  |
| 2 | `YHR5CQ8D_E105` | `motivates` | 13-Noise Level | noisy simulation fault signals, submerged in the noise, low SNR signal(High Noise) |  | 16-Feature Extraction Algorithm | Matching Pursuit |  |
| 3 | `YHR5CQ8D_E106` | `motivates` | 09-Problem Scenario | early-stage fault / early faults(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | Signal Segmentation |  |
| 4 | `YHR5CQ8D_E107` | `motivates` | 09-Problem Scenario | composite fault(Compound Faults) |  | 15-Data Preprocessing Algorithm | Signal Segmentation |  |
| 5 | `YHR5CQ8D_E108` | `motivates` | 09-Problem Scenario | low SNR / noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | Signal Segmentation |  |
| 6 | `YHR5CQ8D_E109` | `motivates` | 09-Problem Scenario | early-stage fault / early faults(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | Matching Pursuit |  |
| 7 | `YHR5CQ8D_E110` | `motivates` | 09-Problem Scenario | composite fault(Compound Faults) |  | 16-Feature Extraction Algorithm | Matching Pursuit |  |
| 8 | `YHR5CQ8D_E111` | `motivates` | 09-Problem Scenario | low SNR / noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | Matching Pursuit |  |
| 9 | `YHR5CQ8D_E112` | `motivates` | 09-Problem Scenario | early-stage fault / early faults(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Fast Fourier Transform |  |
| 10 | `YHR5CQ8D_E113` | `motivates` | 09-Problem Scenario | composite fault(Compound Faults) |  | 17-Core Classifier Algorithm | Fast Fourier Transform |  |
| 11 | `YHR5CQ8D_E114` | `motivates` | 09-Problem Scenario | low SNR / noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | Fast Fourier Transform |  |
| 12 | `YHR5CQ8D_E115` | `motivates` | 07-Compound Fault | composite fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Matching Pursuit |  |
| 13 | `YHR5CQ8D_E116` | `motivates` | 07-Compound Fault | composite fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Fast Fourier Transform |  |
| 14 | `YHR5CQ8D_E117` | `motivates` | 06-Fault Severity | diameter of 0.2 mm(Single Severity) |  | 17-Core Classifier Algorithm | Fast Fourier Transform |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `YHR5CQ8D_E106`：early-stage fault / early faults(Early Degradation Prediction) motivates Signal Segmentation
  - `YHR5CQ8D_E107`：composite fault(Compound Faults) motivates Signal Segmentation
  - `YHR5CQ8D_E108`：low SNR / noise robustness(Uncertainty) motivates Signal Segmentation
  - `YHR5CQ8D_E109`：early-stage fault / early faults(Early Degradation Prediction) motivates Matching Pursuit
  - `YHR5CQ8D_E110`：composite fault(Compound Faults) motivates Matching Pursuit
  - `YHR5CQ8D_E111`：low SNR / noise robustness(Uncertainty) motivates Matching Pursuit
  - `YHR5CQ8D_E112`：early-stage fault / early faults(Early Degradation Prediction) motivates Fast Fourier Transform
  - `YHR5CQ8D_E113`：composite fault(Compound Faults) motivates Fast Fourier Transform
  - `YHR5CQ8D_E114`：low SNR / noise robustness(Uncertainty) motivates Fast Fourier Transform
