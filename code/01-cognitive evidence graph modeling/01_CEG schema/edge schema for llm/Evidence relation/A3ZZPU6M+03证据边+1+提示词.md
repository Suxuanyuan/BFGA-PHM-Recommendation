...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `A3ZZPU6M_E123` | `motivates` | 13-Noise Level | white Gaussian noise, measurement noises and wide band structure borne noise(High Noise) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 2 | `A3ZZPU6M_E124` | `motivates` | 13-Noise Level | white Gaussian noise, measurement noises and wide band structure borne noise(High Noise) |  | 16-Feature Extraction Algorithm | PFS-EMD |  |
| 3 | `A3ZZPU6M_E125` | `motivates` | 09-Problem Scenario | concurrent faults / mixed fault(Compound Faults) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 4 | `A3ZZPU6M_E126` | `motivates` | 09-Problem Scenario | noise / uncertainty in fault frequencies(Uncertainty) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 5 | `A3ZZPU6M_E127` | `motivates` | 09-Problem Scenario | concurrent faults / mixed fault(Compound Faults) |  | 16-Feature Extraction Algorithm | PFS-EMD |  |
| 6 | `A3ZZPU6M_E128` | `motivates` | 09-Problem Scenario | noise / uncertainty in fault frequencies(Uncertainty) |  | 16-Feature Extraction Algorithm | PFS-EMD |  |
| 7 | `A3ZZPU6M_E129` | `motivates` | 09-Problem Scenario | concurrent faults / mixed fault(Compound Faults) |  | 17-Core Classifier Algorithm | L2-norm |  |
| 8 | `A3ZZPU6M_E130` | `motivates` | 09-Problem Scenario | noise / uncertainty in fault frequencies(Uncertainty) |  | 17-Core Classifier Algorithm | L2-norm |  |
| 9 | `A3ZZPU6M_E131` | `motivates` | 07-Compound Fault | mixed fault(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | PFS-EMD |  |
| 10 | `A3ZZPU6M_E132` | `motivates` | 07-Compound Fault | mixed fault(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | L2-norm |  |
| 11 | `A3ZZPU6M_E133` | `motivates` | 06-Fault Severity | run continuously till the failure, progression of a specific bearing fundamental fault with the time(Multiple Severities) |  | 17-Core Classifier Algorithm | L2-norm |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `A3ZZPU6M_E125`：concurrent faults / mixed fault(Compound Faults) motivates EMD
  - `A3ZZPU6M_E126`：noise / uncertainty in fault frequencies(Uncertainty) motivates EMD
  - `A3ZZPU6M_E127`：concurrent faults / mixed fault(Compound Faults) motivates PFS-EMD
  - `A3ZZPU6M_E128`：noise / uncertainty in fault frequencies(Uncertainty) motivates PFS-EMD
  - `A3ZZPU6M_E129`：concurrent faults / mixed fault(Compound Faults) motivates L2-norm
  - `A3ZZPU6M_E130`：noise / uncertainty in fault frequencies(Uncertainty) motivates L2-norm
