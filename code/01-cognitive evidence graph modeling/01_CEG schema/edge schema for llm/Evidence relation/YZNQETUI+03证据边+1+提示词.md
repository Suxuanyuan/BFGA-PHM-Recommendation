...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `YZNQETUI_E069` | `motivates` | 13-Noise Level | measurement noise, ripples, sampling effects(High Noise) |  | 15-Data Preprocessing Algorithm | High-pass filter |  |
| 2 | `YZNQETUI_E070` | `motivates` | 13-Noise Level | measurement noise, ripples, sampling effects(High Noise) |  | 16-Feature Extraction Algorithm | Pattern adapted wavelet transform |  |
| 3 | `YZNQETUI_E071` | `motivates` | 09-Problem Scenario | incipient inter-turn fault detection(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | High-pass filter |  |
| 4 | `YZNQETUI_E072` | `motivates` | 09-Problem Scenario | incipient inter-turn fault detection(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | Pattern adapted wavelet transform |  |
| 5 | `YZNQETUI_E073` | `motivates` | 09-Problem Scenario | incipient inter-turn fault detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Threshold decision |  |
| 6 | `YZNQETUI_E074` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Pattern adapted wavelet transform |  |
| 7 | `YZNQETUI_E075` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Threshold decision |  |
| 8 | `YZNQETUI_E076` | `motivates` | 06-Fault Severity | 50% of a stator phase winding(Single Severity) |  | 17-Core Classifier Algorithm | Threshold decision |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `YZNQETUI_E071`：incipient inter-turn fault detection(Early Degradation Prediction) motivates High-pass filter
  - `YZNQETUI_E072`：incipient inter-turn fault detection(Early Degradation Prediction) motivates Pattern adapted wavelet transform
  - `YZNQETUI_E073`：incipient inter-turn fault detection(Early Degradation Prediction) motivates Threshold decision
