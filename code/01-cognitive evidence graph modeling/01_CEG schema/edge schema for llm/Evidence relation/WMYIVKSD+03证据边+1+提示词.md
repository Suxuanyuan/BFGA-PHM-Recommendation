...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `WMYIVKSD_E078` | `motivates` | 12-Training Data Availability | dataset of 6550 cones(Sufficient) |  | 18-Data Generation Algorithm | SMOTE |  |
| 2 | `WMYIVKSD_E079` | `motivates` | 09-Problem Scenario | computationally challenging for large-scale and complex systems(Complex Systems) |  | 18-Data Generation Algorithm | SMOTE |  |
| 3 | `WMYIVKSD_E080` | `motivates` | 09-Problem Scenario | multiple fault diagnosis(Compound Faults) |  | 18-Data Generation Algorithm | SMOTE |  |
| 4 | `WMYIVKSD_E081` | `motivates` | 09-Problem Scenario | ungroundable abstract diagnoses in strong fault models(Other) |  | 18-Data Generation Algorithm | SMOTE |  |
| 5 | `WMYIVKSD_E082` | `motivates` | 09-Problem Scenario | computationally challenging for large-scale and complex systems(Complex Systems) |  | 17-Core Classifier Algorithm | WOpt |  |
| 6 | `WMYIVKSD_E083` | `motivates` | 09-Problem Scenario | multiple fault diagnosis(Compound Faults) |  | 17-Core Classifier Algorithm | WOpt |  |
| 7 | `WMYIVKSD_E084` | `motivates` | 09-Problem Scenario | ungroundable abstract diagnoses in strong fault models(Other) |  | 17-Core Classifier Algorithm | WOpt |  |
| 8 | `WMYIVKSD_E085` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | WOpt |  |
| 9 | `WMYIVKSD_E086` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | WOpt |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `WMYIVKSD_E079`：computationally challenging for large-scale and complex systems(Complex Systems) motivates SMOTE
  - `WMYIVKSD_E080`：multiple fault diagnosis(Compound Faults) motivates SMOTE
  - `WMYIVKSD_E081`：ungroundable abstract diagnoses in strong fault models(Other) motivates SMOTE
  - `WMYIVKSD_E082`：computationally challenging for large-scale and complex systems(Complex Systems) motivates WOpt
  - `WMYIVKSD_E083`：multiple fault diagnosis(Compound Faults) motivates WOpt
  - `WMYIVKSD_E084`：ungroundable abstract diagnoses in strong fault models(Other) motivates WOpt
