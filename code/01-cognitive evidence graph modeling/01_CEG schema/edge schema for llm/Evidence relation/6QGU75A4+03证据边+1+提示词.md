...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `6QGU75A4_E074` | `motivates` | 13-Noise Level | load oscillations, measurement errors, disturbance(High Noise) |  | 15-Data Preprocessing Algorithm | LPF |  |
| 2 | `6QGU75A4_E075` | `motivates` | 13-Noise Level | load oscillations, measurement errors, disturbance(High Noise) |  | 16-Feature Extraction Algorithm | DFT |  |
| 3 | `6QGU75A4_E076` | `motivates` | 09-Problem Scenario | load oscillations, temperature changes, and parameter mismatch(Uncertainty) |  | 15-Data Preprocessing Algorithm | LPF |  |
| 4 | `6QGU75A4_E077` | `motivates` | 09-Problem Scenario | load oscillations, temperature changes, and parameter mismatch(Uncertainty) |  | 16-Feature Extraction Algorithm | DFT |  |
| 5 | `6QGU75A4_E078` | `motivates` | 09-Problem Scenario | load oscillations, temperature changes, and parameter mismatch(Uncertainty) |  | 17-Core Classifier Algorithm | Threshold Decision |  |
| 6 | `6QGU75A4_E079` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | DFT |  |
| 7 | `6QGU75A4_E080` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Threshold Decision |  |
| 8 | `6QGU75A4_E081` | `motivates` | 06-Fault Severity | 0, 1, and 2 BRBs(Multiple Severities) |  | 17-Core Classifier Algorithm | Threshold Decision |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `6QGU75A4_E076`：load oscillations, temperature changes, and parameter mismatch(Uncertainty) motivates LPF
  - `6QGU75A4_E077`：load oscillations, temperature changes, and parameter mismatch(Uncertainty) motivates DFT
  - `6QGU75A4_E078`：load oscillations, temperature changes, and parameter mismatch(Uncertainty) motivates Threshold Decision
