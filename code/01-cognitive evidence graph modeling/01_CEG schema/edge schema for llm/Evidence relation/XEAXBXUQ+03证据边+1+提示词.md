...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `XEAXBXUQ_E101` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | Unsupervised Domain Adaptation |  |
| 2 | `XEAXBXUQ_E102` | `motivates` | 12-Training Data Availability | The training set has 400 samples for each category, 1500 samples(Sufficient) |  | 19-Training Optimization Algorithm | Unsupervised Domain Adaptation |  |
| 3 | `XEAXBXUQ_E103` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | Fourier Transform |  |
| 4 | `XEAXBXUQ_E104` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | CNN |  |
| 5 | `XEAXBXUQ_E105` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 15-Data Preprocessing Algorithm | Fourier Transform |  |
| 6 | `XEAXBXUQ_E106` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | CNN |  |
| 7 | `XEAXBXUQ_E107` | `motivates` | 09-Problem Scenario | domain shift(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `XEAXBXUQ_E108` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CNN |  |
| 9 | `XEAXBXUQ_E109` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 10 | `XEAXBXUQ_E110` | `motivates` | 06-Fault Severity | fault diameter(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `XEAXBXUQ_E101`：domain shift(Distribution Discrepancy) motivates Unsupervised Domain Adaptation
  - `XEAXBXUQ_E105`：domain shift(Distribution Discrepancy) motivates Fourier Transform
  - `XEAXBXUQ_E106`：domain shift(Distribution Discrepancy) motivates CNN
  - `XEAXBXUQ_E107`：domain shift(Distribution Discrepancy) motivates Softmax
