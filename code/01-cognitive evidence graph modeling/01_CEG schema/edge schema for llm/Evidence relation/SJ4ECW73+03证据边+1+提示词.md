...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `SJ4ECW73_E080` | `motivates` | 09-Problem Scenario | strong background noises(Uncertainty) |  | 19-Training Optimization Algorithm | Whale Optimization Algorithm |  |
| 2 | `SJ4ECW73_E081` | `motivates` | 12-Training Data Availability | Sufficient |  | 19-Training Optimization Algorithm | Whale Optimization Algorithm |  |
| 3 | `SJ4ECW73_E082` | `motivates` | 13-Noise Level | strong background noises, Gauss white noise, SNR is exceedingly low(High Noise) |  | 16-Feature Extraction Algorithm | Orthogonal Matching Pursuit |  |
| 4 | `SJ4ECW73_E083` | `motivates` | 09-Problem Scenario | strong background noises(Uncertainty) |  | 16-Feature Extraction Algorithm | Orthogonal Matching Pursuit |  |
| 5 | `SJ4ECW73_E084` | `motivates` | 09-Problem Scenario | strong background noises(Uncertainty) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 6 | `SJ4ECW73_E085` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Orthogonal Matching Pursuit |  |
| 7 | `SJ4ECW73_E086` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
| 8 | `SJ4ECW73_E087` | `motivates` | 06-Fault Severity | crack with a width of 0.6 mm(Single Severity) |  | 17-Core Classifier Algorithm | Envelope Analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `SJ4ECW73_E080`：strong background noises(Uncertainty) motivates Whale Optimization Algorithm
  - `SJ4ECW73_E083`：strong background noises(Uncertainty) motivates Orthogonal Matching Pursuit
  - `SJ4ECW73_E084`：strong background noises(Uncertainty) motivates Envelope Analysis
