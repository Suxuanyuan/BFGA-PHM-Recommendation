...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `DEYE6SYA_E079` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | Zero-Sequence Current Calculation |  |
| 2 | `DEYE6SYA_E080` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | STFT |  |
| 3 | `DEYE6SYA_E081` | `motivates` | 09-Problem Scenario | False alarms and signature masking caused by electromagnetic asymmetries and load oscillations(Uncertainty) |  | 15-Data Preprocessing Algorithm | Zero-Sequence Current Calculation |  |
| 4 | `DEYE6SYA_E082` | `motivates` | 09-Problem Scenario | Incipient rotor bar fault masking and detection(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | Zero-Sequence Current Calculation |  |
| 5 | `DEYE6SYA_E083` | `motivates` | 09-Problem Scenario | False alarms and signature masking caused by electromagnetic asymmetries and load oscillations(Uncertainty) |  | 16-Feature Extraction Algorithm | STFT |  |
| 6 | `DEYE6SYA_E084` | `motivates` | 09-Problem Scenario | Incipient rotor bar fault masking and detection(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | STFT |  |
| 7 | `DEYE6SYA_E085` | `motivates` | 09-Problem Scenario | False alarms and signature masking caused by electromagnetic asymmetries and load oscillations(Uncertainty) |  | 17-Core Classifier Algorithm | Visual Inspection |  |
| 8 | `DEYE6SYA_E086` | `motivates` | 09-Problem Scenario | Incipient rotor bar fault masking and detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Visual Inspection |  |
| 9 | `DEYE6SYA_E087` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | STFT |  |
| 10 | `DEYE6SYA_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Visual Inspection |  |
| 11 | `DEYE6SYA_E089` | `motivates` | 06-Fault Severity | one and two adjacent broken bars(Multiple Severities) |  | 17-Core Classifier Algorithm | Visual Inspection |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `DEYE6SYA_E081`：False alarms and signature masking caused by electromagnetic asymmetries and load oscillations(Uncertainty) motivates Zero-Sequence Current Calculation
  - `DEYE6SYA_E082`：Incipient rotor bar fault masking and detection(Early Degradation Prediction) motivates Zero-Sequence Current Calculation
  - `DEYE6SYA_E083`：False alarms and signature masking caused by electromagnetic asymmetries and load oscillations(Uncertainty) motivates STFT
  - `DEYE6SYA_E084`：Incipient rotor bar fault masking and detection(Early Degradation Prediction) motivates STFT
  - `DEYE6SYA_E085`：False alarms and signature masking caused by electromagnetic asymmetries and load oscillations(Uncertainty) motivates Visual Inspection
  - `DEYE6SYA_E086`：Incipient rotor bar fault masking and detection(Early Degradation Prediction) motivates Visual Inspection
