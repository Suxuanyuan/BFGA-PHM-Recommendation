...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `8ZY9TS8X_E106` | `motivates` | 09-Problem Scenario | High-level noise, weak impulsive feature extraction(Uncertainty) |  | 19-Training Optimization Algorithm | Majorization-Minimization |  |
| 2 | `8ZY9TS8X_E107` | `motivates` | 12-Training Data Availability | Sufficient |  | 19-Training Optimization Algorithm | Majorization-Minimization |  |
| 3 | `8ZY9TS8X_E108` | `motivates` | 13-Noise Level | high-level noise(High Noise) |  | 15-Data Preprocessing Algorithm | Spectral Kurtosis |  |
| 4 | `8ZY9TS8X_E109` | `motivates` | 13-Noise Level | high-level noise(High Noise) |  | 16-Feature Extraction Algorithm | CDSPGL |  |
| 5 | `8ZY9TS8X_E110` | `motivates` | 09-Problem Scenario | High-level noise, weak impulsive feature extraction(Uncertainty) |  | 15-Data Preprocessing Algorithm | Spectral Kurtosis |  |
| 6 | `8ZY9TS8X_E111` | `motivates` | 09-Problem Scenario | High-level noise, weak impulsive feature extraction(Uncertainty) |  | 16-Feature Extraction Algorithm | CDSPGL |  |
| 7 | `8ZY9TS8X_E112` | `motivates` | 09-Problem Scenario | High-level noise, weak impulsive feature extraction(Uncertainty) |  | 17-Core Classifier Algorithm | Hilbert Envelope Spectrum |  |
| 8 | `8ZY9TS8X_E113` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CDSPGL |  |
| 9 | `8ZY9TS8X_E114` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Hilbert Envelope Spectrum |  |
| 10 | `8ZY9TS8X_E115` | `motivates` | 06-Fault Severity | nonthrough defect(Single Severity) |  | 17-Core Classifier Algorithm | Hilbert Envelope Spectrum |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `8ZY9TS8X_E106`：High-level noise, weak impulsive feature extraction(Uncertainty) motivates Majorization-Minimization
  - `8ZY9TS8X_E110`：High-level noise, weak impulsive feature extraction(Uncertainty) motivates Spectral Kurtosis
  - `8ZY9TS8X_E111`：High-level noise, weak impulsive feature extraction(Uncertainty) motivates CDSPGL
  - `8ZY9TS8X_E112`：High-level noise, weak impulsive feature extraction(Uncertainty) motivates Hilbert Envelope Spectrum
