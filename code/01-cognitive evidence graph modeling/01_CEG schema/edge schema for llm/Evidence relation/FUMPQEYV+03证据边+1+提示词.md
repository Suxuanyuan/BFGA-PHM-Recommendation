...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `FUMPQEYV_E083` | `motivates` | 13-Noise Level | Input disturbances(High Noise) |  | 16-Feature Extraction Algorithm | First-order difference |  |
| 2 | `FUMPQEYV_E084` | `motivates` | 09-Problem Scenario | Without additional sensors(Other) |  | 16-Feature Extraction Algorithm | First-order difference |  |
| 3 | `FUMPQEYV_E085` | `motivates` | 09-Problem Scenario | Very fast switch fault detection(Other) |  | 16-Feature Extraction Algorithm | First-order difference |  |
| 4 | `FUMPQEYV_E086` | `motivates` | 09-Problem Scenario | Without additional sensors(Other) |  | 17-Core Classifier Algorithm | Finite State Machine |  |
| 5 | `FUMPQEYV_E087` | `motivates` | 09-Problem Scenario | Very fast switch fault detection(Other) |  | 17-Core Classifier Algorithm | Finite State Machine |  |
| 6 | `FUMPQEYV_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | First-order difference |  |
| 7 | `FUMPQEYV_E089` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Finite State Machine |  |
| 8 | `FUMPQEYV_E090` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Finite State Machine |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `FUMPQEYV_E084`：Without additional sensors(Other) motivates First-order difference
  - `FUMPQEYV_E085`：Very fast switch fault detection(Other) motivates First-order difference
  - `FUMPQEYV_E086`：Without additional sensors(Other) motivates Finite State Machine
  - `FUMPQEYV_E087`：Very fast switch fault detection(Other) motivates Finite State Machine
