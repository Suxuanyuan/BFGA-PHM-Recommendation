...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `VXXGDIYI_E109` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 19-Training Optimization Algorithm | Adaptive tracking learning algorithm |  |
| 2 | `VXXGDIYI_E110` | `motivates` | 09-Problem Scenario | heavy background noises(Uncertainty) |  | 19-Training Optimization Algorithm | Adaptive tracking learning algorithm |  |
| 3 | `VXXGDIYI_E111` | `motivates` | 12-Training Data Availability | 425 samples of each condition are used for training(Sufficient) |  | 19-Training Optimization Algorithm | Adaptive tracking learning algorithm |  |
| 4 | `VXXGDIYI_E112` | `motivates` | 13-Noise Level | influenced by very heavy background noises(High Noise) |  | 16-Feature Extraction Algorithm | Deep Wavelet Auto-Encoder |  |
| 5 | `VXXGDIYI_E113` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 16-Feature Extraction Algorithm | Deep Wavelet Auto-Encoder |  |
| 6 | `VXXGDIYI_E114` | `motivates` | 09-Problem Scenario | heavy background noises(Uncertainty) |  | 16-Feature Extraction Algorithm | Deep Wavelet Auto-Encoder |  |
| 7 | `VXXGDIYI_E115` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `VXXGDIYI_E116` | `motivates` | 09-Problem Scenario | heavy background noises(Uncertainty) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `VXXGDIYI_E117` | `motivates` | 07-Compound Fault | compound faults(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Deep Wavelet Auto-Encoder |  |
| 10 | `VXXGDIYI_E118` | `motivates` | 07-Compound Fault | compound faults(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `VXXGDIYI_E119` | `motivates` | 06-Fault Severity | slight outer race fault, serious outer race fault(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `VXXGDIYI_E109`：compound faults(Compound Faults) motivates Adaptive tracking learning algorithm
  - `VXXGDIYI_E110`：heavy background noises(Uncertainty) motivates Adaptive tracking learning algorithm
  - `VXXGDIYI_E113`：compound faults(Compound Faults) motivates Deep Wavelet Auto-Encoder
  - `VXXGDIYI_E114`：heavy background noises(Uncertainty) motivates Deep Wavelet Auto-Encoder
  - `VXXGDIYI_E115`：compound faults(Compound Faults) motivates Softmax
  - `VXXGDIYI_E116`：heavy background noises(Uncertainty) motivates Softmax
