...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `XMQ5YAER_E107` | `motivates` | 09-Problem Scenario | distribution discrepancy(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | Domain Adaptation |  |
| 2 | `XMQ5YAER_E108` | `motivates` | 09-Problem Scenario | insufficient labeled data(Small Fault Samples) |  | 19-Training Optimization Algorithm | Domain Adaptation |  |
| 3 | `XMQ5YAER_E109` | `motivates` | 12-Training Data Availability | 404 samples for each dataset, with 101 samples for each category(Sufficient) |  | 19-Training Optimization Algorithm | Domain Adaptation |  |
| 4 | `XMQ5YAER_E110` | `motivates` | 13-Noise Level | raw vibration data(Normal) |  | 16-Feature Extraction Algorithm | CNN |  |
| 5 | `XMQ5YAER_E111` | `motivates` | 09-Problem Scenario | distribution discrepancy(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | CNN |  |
| 6 | `XMQ5YAER_E112` | `motivates` | 09-Problem Scenario | insufficient labeled data(Small Fault Samples) |  | 16-Feature Extraction Algorithm | CNN |  |
| 7 | `XMQ5YAER_E113` | `motivates` | 09-Problem Scenario | distribution discrepancy(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `XMQ5YAER_E114` | `motivates` | 09-Problem Scenario | insufficient labeled data(Small Fault Samples) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `XMQ5YAER_E115` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CNN |  |
| 10 | `XMQ5YAER_E116` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `XMQ5YAER_E117` | `motivates` | 06-Fault Severity | 0.0014 inches(Single Severity) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `XMQ5YAER_E107`：distribution discrepancy(Distribution Discrepancy) motivates Domain Adaptation
  - `XMQ5YAER_E108`：insufficient labeled data(Small Fault Samples) motivates Domain Adaptation
  - `XMQ5YAER_E111`：distribution discrepancy(Distribution Discrepancy) motivates CNN
  - `XMQ5YAER_E112`：insufficient labeled data(Small Fault Samples) motivates CNN
  - `XMQ5YAER_E113`：distribution discrepancy(Distribution Discrepancy) motivates Softmax
  - `XMQ5YAER_E114`：insufficient labeled data(Small Fault Samples) motivates Softmax
