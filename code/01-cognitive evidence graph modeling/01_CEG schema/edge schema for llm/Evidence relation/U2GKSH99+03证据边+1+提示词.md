...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `U2GKSH99_E140` | `motivates` | 13-Noise Level | varying-speed and noisy conditions(High Noise) |  | 15-Data Preprocessing Algorithm | Hilbert transform |  |
| 2 | `U2GKSH99_E141` | `motivates` | 13-Noise Level | varying-speed and noisy conditions(High Noise) |  | 16-Feature Extraction Algorithm | Vold-Kalman filter |  |
| 3 | `U2GKSH99_E142` | `motivates` | 09-Problem Scenario | noisy conditions(Uncertainty) |  | 15-Data Preprocessing Algorithm | Hilbert transform |  |
| 4 | `U2GKSH99_E143` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 15-Data Preprocessing Algorithm | Hilbert transform |  |
| 5 | `U2GKSH99_E144` | `motivates` | 09-Problem Scenario | noisy conditions(Uncertainty) |  | 16-Feature Extraction Algorithm | Vold-Kalman filter |  |
| 6 | `U2GKSH99_E145` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 16-Feature Extraction Algorithm | Vold-Kalman filter |  |
| 7 | `U2GKSH99_E146` | `motivates` | 09-Problem Scenario | noisy conditions(Uncertainty) |  | 17-Core Classifier Algorithm | envelope spectrum analysis |  |
| 8 | `U2GKSH99_E147` | `motivates` | 09-Problem Scenario | compound faults(Compound Faults) |  | 17-Core Classifier Algorithm | envelope spectrum analysis |  |
| 9 | `U2GKSH99_E148` | `motivates` | 07-Compound Fault | fillet crack and misalignment; bearing faults(Compound Fault Across Structures) |  | 16-Feature Extraction Algorithm | Vold-Kalman filter |  |
| 10 | `U2GKSH99_E149` | `motivates` | 07-Compound Fault | fillet crack and misalignment; bearing faults(Compound Fault Across Structures) |  | 17-Core Classifier Algorithm | envelope spectrum analysis |  |
| 11 | `U2GKSH99_E150` | `motivates` | 06-Fault Severity | width of 0.2 mm, depth of 2 mm(Single Severity) |  | 17-Core Classifier Algorithm | envelope spectrum analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `U2GKSH99_E142`：noisy conditions(Uncertainty) motivates Hilbert transform
  - `U2GKSH99_E143`：compound faults(Compound Faults) motivates Hilbert transform
  - `U2GKSH99_E144`：noisy conditions(Uncertainty) motivates Vold-Kalman filter
  - `U2GKSH99_E145`：compound faults(Compound Faults) motivates Vold-Kalman filter
  - `U2GKSH99_E146`：noisy conditions(Uncertainty) motivates envelope spectrum analysis
  - `U2GKSH99_E147`：compound faults(Compound Faults) motivates envelope spectrum analysis
