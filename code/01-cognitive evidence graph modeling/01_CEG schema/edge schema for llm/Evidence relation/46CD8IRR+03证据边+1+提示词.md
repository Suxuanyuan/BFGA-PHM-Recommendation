...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `46CD8IRR_E090` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 15-Data Preprocessing Algorithm | High-pass filter |  |
| 2 | `46CD8IRR_E091` | `motivates` | 13-Noise Level | heavy background noise(High Noise) |  | 16-Feature Extraction Algorithm | K-SVD |  |
| 3 | `46CD8IRR_E092` | `motivates` | 09-Problem Scenario | heavy background noise and phase influence(Uncertainty) |  | 15-Data Preprocessing Algorithm | High-pass filter |  |
| 4 | `46CD8IRR_E093` | `motivates` | 09-Problem Scenario | slow calculating speed and dictionary redundancy(Other) |  | 15-Data Preprocessing Algorithm | High-pass filter |  |
| 5 | `46CD8IRR_E094` | `motivates` | 09-Problem Scenario | heavy background noise and phase influence(Uncertainty) |  | 16-Feature Extraction Algorithm | K-SVD |  |
| 6 | `46CD8IRR_E095` | `motivates` | 09-Problem Scenario | slow calculating speed and dictionary redundancy(Other) |  | 16-Feature Extraction Algorithm | K-SVD |  |
| 7 | `46CD8IRR_E096` | `motivates` | 09-Problem Scenario | heavy background noise and phase influence(Uncertainty) |  | 17-Core Classifier Algorithm | Hilbert transform |  |
| 8 | `46CD8IRR_E097` | `motivates` | 09-Problem Scenario | slow calculating speed and dictionary redundancy(Other) |  | 17-Core Classifier Algorithm | Hilbert transform |  |
| 9 | `46CD8IRR_E098` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | K-SVD |  |
| 10 | `46CD8IRR_E099` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Hilbert transform |  |
| 11 | `46CD8IRR_E100` | `motivates` | 06-Fault Severity | 0.2 mm wide and 0.5 mm deep, 0.2 mm wide and 1 mm deep(Single Severity) |  | 17-Core Classifier Algorithm | Hilbert transform |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `46CD8IRR_E092`：heavy background noise and phase influence(Uncertainty) motivates High-pass filter
  - `46CD8IRR_E093`：slow calculating speed and dictionary redundancy(Other) motivates High-pass filter
  - `46CD8IRR_E094`：heavy background noise and phase influence(Uncertainty) motivates K-SVD
  - `46CD8IRR_E095`：slow calculating speed and dictionary redundancy(Other) motivates K-SVD
  - `46CD8IRR_E096`：heavy background noise and phase influence(Uncertainty) motivates Hilbert transform
  - `46CD8IRR_E097`：slow calculating speed and dictionary redundancy(Other) motivates Hilbert transform
