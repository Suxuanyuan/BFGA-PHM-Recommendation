...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `QIYT9DMG_E079` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | DWT |  |
| 2 | `QIYT9DMG_E080` | `motivates` | 09-Problem Scenario | control-loop masks and compensates the fault effect(Other) |  | 16-Feature Extraction Algorithm | DWT |  |
| 3 | `QIYT9DMG_E081` | `motivates` | 09-Problem Scenario | continuous speed variation causes a non-stationary state(Other) |  | 16-Feature Extraction Algorithm | DWT |  |
| 4 | `QIYT9DMG_E082` | `motivates` | 09-Problem Scenario | control-loop masks and compensates the fault effect(Other) |  | 17-Core Classifier Algorithm | Energy Eigenvalue |  |
| 5 | `QIYT9DMG_E083` | `motivates` | 09-Problem Scenario | continuous speed variation causes a non-stationary state(Other) |  | 17-Core Classifier Algorithm | Energy Eigenvalue |  |
| 6 | `QIYT9DMG_E084` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | DWT |  |
| 7 | `QIYT9DMG_E085` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Energy Eigenvalue |  |
| 8 | `QIYT9DMG_E086` | `motivates` | 06-Fault Severity | 3.1 mm diameter(Single Severity) |  | 17-Core Classifier Algorithm | Energy Eigenvalue |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `QIYT9DMG_E080`：control-loop masks and compensates the fault effect(Other) motivates DWT
  - `QIYT9DMG_E081`：continuous speed variation causes a non-stationary state(Other) motivates DWT
  - `QIYT9DMG_E082`：control-loop masks and compensates the fault effect(Other) motivates Energy Eigenvalue
  - `QIYT9DMG_E083`：continuous speed variation causes a non-stationary state(Other) motivates Energy Eigenvalue
