...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `28G4B4RR_E171` | `motivates` | 09-Problem Scenario | process noise, measurement noise and unknown disturbances(Uncertainty) |  | 19-Training Optimization Algorithm | projection algorithm |  |
| 2 | `28G4B4RR_E172` | `motivates` | 09-Problem Scenario | strong nonlinearities(Other) |  | 19-Training Optimization Algorithm | projection algorithm |  |
| 3 | `28G4B4RR_E173` | `motivates` | 12-Training Data Availability | 5,000 samples, 1,800 samples(Sufficient) |  | 19-Training Optimization Algorithm | projection algorithm |  |
| 4 | `28G4B4RR_E174` | `motivates` | 13-Noise Level | process noise and measurement noise(High Noise) |  | 15-Data Preprocessing Algorithm | PCA |  |
| 5 | `28G4B4RR_E175` | `motivates` | 09-Problem Scenario | process noise, measurement noise and unknown disturbances(Uncertainty) |  | 15-Data Preprocessing Algorithm | PCA |  |
| 6 | `28G4B4RR_E176` | `motivates` | 09-Problem Scenario | strong nonlinearities(Other) |  | 15-Data Preprocessing Algorithm | PCA |  |
| 7 | `28G4B4RR_E177` | `motivates` | 09-Problem Scenario | process noise, measurement noise and unknown disturbances(Uncertainty) |  | 17-Core Classifier Algorithm | residual generator |  |
| 8 | `28G4B4RR_E178` | `motivates` | 09-Problem Scenario | strong nonlinearities(Other) |  | 17-Core Classifier Algorithm | residual generator |  |
| 9 | `28G4B4RR_E179` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | residual generator |  |
| 10 | `28G4B4RR_E180` | `motivates` | 06-Fault Severity | 5%/10%/15% of volume, 0.2/0.5/1.0, 90%/80%/70%(Multiple Severities) |  | 17-Core Classifier Algorithm | residual generator |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `28G4B4RR_E171`：process noise, measurement noise and unknown disturbances(Uncertainty) motivates projection algorithm
  - `28G4B4RR_E172`：strong nonlinearities(Other) motivates projection algorithm
  - `28G4B4RR_E175`：process noise, measurement noise and unknown disturbances(Uncertainty) motivates PCA
  - `28G4B4RR_E176`：strong nonlinearities(Other) motivates PCA
  - `28G4B4RR_E177`：process noise, measurement noise and unknown disturbances(Uncertainty) motivates residual generator
  - `28G4B4RR_E178`：strong nonlinearities(Other) motivates residual generator
