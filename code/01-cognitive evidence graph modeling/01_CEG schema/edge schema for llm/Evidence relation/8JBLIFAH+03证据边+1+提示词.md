...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `8JBLIFAH_E089` | `motivates` | 12-Training Data Availability | FEM and experimental data(Sufficient) |  | 18-Data Generation Algorithm | FEM |  |
| 2 | `8JBLIFAH_E090` | `motivates` | 13-Noise Level | measured current and ZSVC signals(Normal) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 3 | `8JBLIFAH_E091` | `motivates` | 13-Noise Level | measured current and ZSVC signals(Normal) |  | 16-Feature Extraction Algorithm | FFT |  |
| 4 | `8JBLIFAH_E092` | `motivates` | 09-Problem Scenario | early stage inter-turn fault detection(Early Degradation Prediction) |  | 18-Data Generation Algorithm | FEM |  |
| 5 | `8JBLIFAH_E093` | `motivates` | 09-Problem Scenario | early stage inter-turn fault detection(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 6 | `8JBLIFAH_E094` | `motivates` | 09-Problem Scenario | early stage inter-turn fault detection(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | FFT |  |
| 7 | `8JBLIFAH_E095` | `motivates` | 09-Problem Scenario | early stage inter-turn fault detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Thresholding |  |
| 8 | `8JBLIFAH_E096` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | FFT |  |
| 9 | `8JBLIFAH_E097` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Thresholding |  |
| 10 | `8JBLIFAH_E098` | `motivates` | 06-Fault Severity | one short-circuited turn(Single Severity) |  | 17-Core Classifier Algorithm | Thresholding |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `8JBLIFAH_E092`：early stage inter-turn fault detection(Early Degradation Prediction) motivates FEM
  - `8JBLIFAH_E093`：early stage inter-turn fault detection(Early Degradation Prediction) motivates Normalization
  - `8JBLIFAH_E094`：early stage inter-turn fault detection(Early Degradation Prediction) motivates FFT
  - `8JBLIFAH_E095`：early stage inter-turn fault detection(Early Degradation Prediction) motivates Thresholding
