...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `ZKYS7RZM_E060` | `motivates` | 13-Noise Level | signal to noise ratio, signal de-noising(High Noise) |  | 16-Feature Extraction Algorithm | Wavelet Transform |  |
| 2 | `ZKYS7RZM_E061` | `motivates` | 09-Problem Scenario | Delamination detection and localization(Other) |  | 16-Feature Extraction Algorithm | Wavelet Transform |  |
| 3 | `ZKYS7RZM_E062` | `motivates` | 09-Problem Scenario | Signal attenuation(Uncertainty) |  | 16-Feature Extraction Algorithm | Wavelet Transform |  |
| 4 | `ZKYS7RZM_E063` | `motivates` | 09-Problem Scenario | Delamination detection and localization(Other) |  | 17-Core Classifier Algorithm | Cross-correlation |  |
| 5 | `ZKYS7RZM_E064` | `motivates` | 09-Problem Scenario | Signal attenuation(Uncertainty) |  | 17-Core Classifier Algorithm | Cross-correlation |  |
| 6 | `ZKYS7RZM_E065` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Wavelet Transform |  |
| 7 | `ZKYS7RZM_E066` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Cross-correlation |  |
| 8 | `ZKYS7RZM_E067` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Cross-correlation |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `ZKYS7RZM_E061`：Delamination detection and localization(Other) motivates Wavelet Transform
  - `ZKYS7RZM_E062`：Signal attenuation(Uncertainty) motivates Wavelet Transform
  - `ZKYS7RZM_E063`：Delamination detection and localization(Other) motivates Cross-correlation
  - `ZKYS7RZM_E064`：Signal attenuation(Uncertainty) motivates Cross-correlation
