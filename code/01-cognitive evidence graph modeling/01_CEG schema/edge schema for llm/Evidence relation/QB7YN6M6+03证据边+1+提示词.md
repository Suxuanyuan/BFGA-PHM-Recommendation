...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `QB7YN6M6_E083` | `motivates` | 09-Problem Scenario | unknown time-varying disturbances(Uncertainty) |  | 19-Training Optimization Algorithm | Linear Quadratic Regulator |  |
| 2 | `QB7YN6M6_E084` | `motivates` | 09-Problem Scenario | simultaneous sensor faults(Compound Faults) |  | 19-Training Optimization Algorithm | Linear Quadratic Regulator |  |
| 3 | `QB7YN6M6_E085` | `motivates` | 12-Training Data Availability | N/A(Sufficient) |  | 19-Training Optimization Algorithm | Linear Quadratic Regulator |  |
| 4 | `QB7YN6M6_E086` | `motivates` | 13-Noise Level | contaminated with noise(High Noise) |  | 15-Data Preprocessing Algorithm | Low-pass Filter |  |
| 5 | `QB7YN6M6_E087` | `motivates` | 09-Problem Scenario | unknown time-varying disturbances(Uncertainty) |  | 15-Data Preprocessing Algorithm | Low-pass Filter |  |
| 6 | `QB7YN6M6_E088` | `motivates` | 09-Problem Scenario | simultaneous sensor faults(Compound Faults) |  | 15-Data Preprocessing Algorithm | Low-pass Filter |  |
| 7 | `QB7YN6M6_E089` | `motivates` | 09-Problem Scenario | unknown time-varying disturbances(Uncertainty) |  | 17-Core Classifier Algorithm | Sliding-mode Observer |  |
| 8 | `QB7YN6M6_E090` | `motivates` | 09-Problem Scenario | simultaneous sensor faults(Compound Faults) |  | 17-Core Classifier Algorithm | Sliding-mode Observer |  |
| 9 | `QB7YN6M6_E091` | `motivates` | 07-Compound Fault | simultaneous faults(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Sliding-mode Observer |  |
| 10 | `QB7YN6M6_E092` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Sliding-mode Observer |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `QB7YN6M6_E083`：unknown time-varying disturbances(Uncertainty) motivates Linear Quadratic Regulator
  - `QB7YN6M6_E084`：simultaneous sensor faults(Compound Faults) motivates Linear Quadratic Regulator
  - `QB7YN6M6_E087`：unknown time-varying disturbances(Uncertainty) motivates Low-pass Filter
  - `QB7YN6M6_E088`：simultaneous sensor faults(Compound Faults) motivates Low-pass Filter
  - `QB7YN6M6_E089`：unknown time-varying disturbances(Uncertainty) motivates Sliding-mode Observer
  - `QB7YN6M6_E090`：simultaneous sensor faults(Compound Faults) motivates Sliding-mode Observer
