...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `2UE2NIRS_E084` | `motivates` | 13-Noise Level | noise level of the input values, measurement disturbances(High Noise) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 2 | `2UE2NIRS_E085` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 3 | `2UE2NIRS_E086` | `motivates` | 09-Problem Scenario | mixed faults(Compound Faults) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 4 | `2UE2NIRS_E087` | `motivates` | 09-Problem Scenario | noise robustness, measurement disturbances(Uncertainty) |  | 15-Data Preprocessing Algorithm | FFT |  |
| 5 | `2UE2NIRS_E088` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | MLP |  |
| 6 | `2UE2NIRS_E089` | `motivates` | 09-Problem Scenario | mixed faults(Compound Faults) |  | 17-Core Classifier Algorithm | MLP |  |
| 7 | `2UE2NIRS_E090` | `motivates` | 09-Problem Scenario | noise robustness, measurement disturbances(Uncertainty) |  | 17-Core Classifier Algorithm | MLP |  |
| 8 | `2UE2NIRS_E091` | `motivates` | 07-Compound Fault | mixed failures(Compound Fault Across Structures) |  | 17-Core Classifier Algorithm | MLP |  |
| 9 | `2UE2NIRS_E092` | `motivates` | 06-Fault Severity | 0–10 shorted turns, 0–3 damaged rotor cage bars(Multiple Severities) |  | 17-Core Classifier Algorithm | MLP |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `2UE2NIRS_E085`：incipient fault detection(Early Degradation Prediction) motivates FFT
  - `2UE2NIRS_E086`：mixed faults(Compound Faults) motivates FFT
  - `2UE2NIRS_E087`：noise robustness, measurement disturbances(Uncertainty) motivates FFT
  - `2UE2NIRS_E088`：incipient fault detection(Early Degradation Prediction) motivates MLP
  - `2UE2NIRS_E089`：mixed faults(Compound Faults) motivates MLP
  - `2UE2NIRS_E090`：noise robustness, measurement disturbances(Uncertainty) motivates MLP
