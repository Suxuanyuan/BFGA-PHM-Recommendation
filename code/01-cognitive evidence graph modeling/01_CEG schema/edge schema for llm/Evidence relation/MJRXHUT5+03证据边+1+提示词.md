...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `MJRXHUT5_E060` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | Homogeneity |  |
| 2 | `MJRXHUT5_E061` | `motivates` | 09-Problem Scenario | incipient broken rotor bar diagnosis(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | Homogeneity |  |
| 3 | `MJRXHUT5_E062` | `motivates` | 09-Problem Scenario | high computational complexity for online detection(Other) |  | 16-Feature Extraction Algorithm | Homogeneity |  |
| 4 | `MJRXHUT5_E063` | `motivates` | 09-Problem Scenario | incipient broken rotor bar diagnosis(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Three-sigma rule |  |
| 5 | `MJRXHUT5_E064` | `motivates` | 09-Problem Scenario | high computational complexity for online detection(Other) |  | 17-Core Classifier Algorithm | Three-sigma rule |  |
| 6 | `MJRXHUT5_E065` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Homogeneity |  |
| 7 | `MJRXHUT5_E066` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Three-sigma rule |  |
| 8 | `MJRXHUT5_E067` | `motivates` | 06-Fault Severity | half, one, or two BRBs(Multiple Severities) |  | 17-Core Classifier Algorithm | Three-sigma rule |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `MJRXHUT5_E061`：incipient broken rotor bar diagnosis(Early Degradation Prediction) motivates Homogeneity
  - `MJRXHUT5_E062`：high computational complexity for online detection(Other) motivates Homogeneity
  - `MJRXHUT5_E063`：incipient broken rotor bar diagnosis(Early Degradation Prediction) motivates Three-sigma rule
  - `MJRXHUT5_E064`：high computational complexity for online detection(Other) motivates Three-sigma rule
