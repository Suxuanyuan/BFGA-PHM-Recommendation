...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `WFW46GXF_E077` | `motivates` | 13-Noise Level | white Gaussian noise / Signal-to-Noise Ratio (SNR)(High Noise) |  | 16-Feature Extraction Algorithm | TLS-ESPRIT |  |
| 2 | `WFW46GXF_E078` | `motivates` | 09-Problem Scenario | frequencies close to the fundamental one(Other) |  | 16-Feature Extraction Algorithm | TLS-ESPRIT |  |
| 3 | `WFW46GXF_E079` | `motivates` | 09-Problem Scenario | model order estimation(Other) |  | 16-Feature Extraction Algorithm | TLS-ESPRIT |  |
| 4 | `WFW46GXF_E080` | `motivates` | 09-Problem Scenario | frequencies close to the fundamental one(Other) |  | 17-Core Classifier Algorithm | Fault Severity Criterion |  |
| 5 | `WFW46GXF_E081` | `motivates` | 09-Problem Scenario | model order estimation(Other) |  | 17-Core Classifier Algorithm | Fault Severity Criterion |  |
| 6 | `WFW46GXF_E082` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | TLS-ESPRIT |  |
| 7 | `WFW46GXF_E083` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Fault Severity Criterion |  |
| 8 | `WFW46GXF_E084` | `motivates` | 06-Fault Severity | bearing hole diameter, number of broken rotor bars(Multiple Severities) |  | 17-Core Classifier Algorithm | Fault Severity Criterion |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `WFW46GXF_E078`：frequencies close to the fundamental one(Other) motivates TLS-ESPRIT
  - `WFW46GXF_E079`：model order estimation(Other) motivates TLS-ESPRIT
  - `WFW46GXF_E080`：frequencies close to the fundamental one(Other) motivates Fault Severity Criterion
  - `WFW46GXF_E081`：model order estimation(Other) motivates Fault Severity Criterion
