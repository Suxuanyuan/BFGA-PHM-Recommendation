...
- **Reference ID**: W9FX3YS4
- **Paper Title**: Fault Diagnosis of Photovoltaic Panels Using Dynamic Current-Voltage Characteristics
...
> **Input Material**: Reference ID `W9FX3YS4` PDF full text (see the [PDF Full Text Content] section at the end of this prompt).
>
...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `W9FX3YS4_E089` | `motivates` | 09-Problem Scenario | non-intrusive fault diagnosis without power generation interruption(Other) |  | 19-Training Optimization Algorithm | Modified Particle Swarm Optimization |  |
| 2 | `W9FX3YS4_E090` | `motivates` | 12-Training Data Availability | N = 300 samples(Sufficient) |  | 19-Training Optimization Algorithm | Modified Particle Swarm Optimization |  |
| 3 | `W9FX3YS4_E091` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | Moving window averaging |  |
| 4 | `W9FX3YS4_E092` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | Equivalent circuit modeling |  |
| 5 | `W9FX3YS4_E093` | `motivates` | 09-Problem Scenario | non-intrusive fault diagnosis without power generation interruption(Other) |  | 15-Data Preprocessing Algorithm | Moving window averaging |  |
| 6 | `W9FX3YS4_E094` | `motivates` | 09-Problem Scenario | non-intrusive fault diagnosis without power generation interruption(Other) |  | 16-Feature Extraction Algorithm | Equivalent circuit modeling |  |
| 7 | `W9FX3YS4_E095` | `motivates` | 09-Problem Scenario | non-intrusive fault diagnosis without power generation interruption(Other) |  | 17-Core Classifier Algorithm | Parameter drift analysis |  |
| 8 | `W9FX3YS4_E096` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Equivalent circuit modeling |  |
| 9 | `W9FX3YS4_E097` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Parameter drift analysis |  |
| 10 | `W9FX3YS4_E098` | `motivates` | 06-Fault Severity | different degrees of damage(Multiple Severities) |  | 17-Core Classifier Algorithm | Parameter drift analysis |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `W9FX3YS4_E089`：non-intrusive fault diagnosis without power generation interruption(Other) motivates Modified Particle Swarm Optimization
  - `W9FX3YS4_E093`：non-intrusive fault diagnosis without power generation interruption(Other) motivates Moving window averaging
  - `W9FX3YS4_E094`：non-intrusive fault diagnosis without power generation interruption(Other) motivates Equivalent circuit modeling
  - `W9FX3YS4_E095`：non-intrusive fault diagnosis without power generation interruption(Other) motivates Parameter drift analysis
