...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `QIC7MENK_E090` | `motivates` | 13-Noise Level | Normal |  | 15-Data Preprocessing Algorithm | AE Armature Current Estimation |  |
| 2 | `QIC7MENK_E091` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | Waveform Feature Extraction |  |
| 3 | `QIC7MENK_E092` | `motivates` | 09-Problem Scenario | signals on the rotor side are unmeasurable(Complex Systems) |  | 15-Data Preprocessing Algorithm | AE Armature Current Estimation |  |
| 4 | `QIC7MENK_E093` | `motivates` | 09-Problem Scenario | signals on the rotor side are unmeasurable(Complex Systems) |  | 16-Feature Extraction Algorithm | Waveform Feature Extraction |  |
| 5 | `QIC7MENK_E094` | `motivates` | 09-Problem Scenario | signals on the rotor side are unmeasurable(Complex Systems) |  | 17-Core Classifier Algorithm | Threshold-based Decision Logic |  |
| 6 | `QIC7MENK_E095` | `motivates` | 07-Compound Fault | two diodes open-circuit(Compound Fault Within Same Structure) |  | 16-Feature Extraction Algorithm | Waveform Feature Extraction |  |
| 7 | `QIC7MENK_E096` | `motivates` | 07-Compound Fault | two diodes open-circuit(Compound Fault Within Same Structure) |  | 17-Core Classifier Algorithm | Threshold-based Decision Logic |  |
| 8 | `QIC7MENK_E097` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Threshold-based Decision Logic |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `QIC7MENK_E092`：signals on the rotor side are unmeasurable(Complex Systems) motivates AE Armature Current Estimation
  - `QIC7MENK_E093`：signals on the rotor side are unmeasurable(Complex Systems) motivates Waveform Feature Extraction
  - `QIC7MENK_E094`：signals on the rotor side are unmeasurable(Complex Systems) motivates Threshold-based Decision Logic
