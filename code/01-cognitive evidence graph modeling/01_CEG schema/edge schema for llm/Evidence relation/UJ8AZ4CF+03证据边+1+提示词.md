...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `UJ8AZ4CF_E072` | `motivates` | 13-Noise Level | noisy environments / strong noise(High Noise) |  | 15-Data Preprocessing Algorithm | Connected component labeling |  |
| 2 | `UJ8AZ4CF_E073` | `motivates` | 13-Noise Level | noisy environments / strong noise(High Noise) |  | 16-Feature Extraction Algorithm | Spline-kernelled chirplet transform |  |
| 3 | `UJ8AZ4CF_E074` | `motivates` | 09-Problem Scenario | noisy environments(Uncertainty) |  | 15-Data Preprocessing Algorithm | Connected component labeling |  |
| 4 | `UJ8AZ4CF_E075` | `motivates` | 09-Problem Scenario | noisy environments(Uncertainty) |  | 16-Feature Extraction Algorithm | Spline-kernelled chirplet transform |  |
| 5 | `UJ8AZ4CF_E076` | `motivates` | 09-Problem Scenario | noisy environments(Uncertainty) |  | 17-Core Classifier Algorithm | Synchro spline-kernelled chirplet extracting transform |  |
| 6 | `UJ8AZ4CF_E077` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Spline-kernelled chirplet transform |  |
| 7 | `UJ8AZ4CF_E078` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Synchro spline-kernelled chirplet extracting transform |  |
| 8 | `UJ8AZ4CF_E079` | `motivates` | 06-Fault Severity | width and depth are 0.2 mm and 0.5 mm(Single Severity) |  | 17-Core Classifier Algorithm | Synchro spline-kernelled chirplet extracting transform |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `UJ8AZ4CF_E074`：noisy environments(Uncertainty) motivates Connected component labeling
  - `UJ8AZ4CF_E075`：noisy environments(Uncertainty) motivates Spline-kernelled chirplet transform
  - `UJ8AZ4CF_E076`：noisy environments(Uncertainty) motivates Synchro spline-kernelled chirplet extracting transform
