...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `J66EKKTD_E092` | `motivates` | 13-Noise Level | white Gaussian noise(High Noise) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 2 | `J66EKKTD_E093` | `motivates` | 13-Noise Level | white Gaussian noise(High Noise) |  | 16-Feature Extraction Algorithm | Hilbert Transform |  |
| 3 | `J66EKKTD_E094` | `motivates` | 09-Problem Scenario | time-varying vibration transfer path, load zone passing and time-varying angles(Complex Systems) |  | 15-Data Preprocessing Algorithm | EMD |  |
| 4 | `J66EKKTD_E095` | `motivates` | 09-Problem Scenario | time-varying vibration transfer path, load zone passing and time-varying angles(Complex Systems) |  | 16-Feature Extraction Algorithm | Hilbert Transform |  |
| 5 | `J66EKKTD_E096` | `motivates` | 09-Problem Scenario | time-varying vibration transfer path, load zone passing and time-varying angles(Complex Systems) |  | 17-Core Classifier Algorithm | Peak Matching |  |
| 6 | `J66EKKTD_E097` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Hilbert Transform |  |
| 7 | `J66EKKTD_E098` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Peak Matching |  |
| 8 | `J66EKKTD_E099` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | Peak Matching |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `J66EKKTD_E094`：time-varying vibration transfer path, load zone passing and time-varying angles(Complex Systems) motivates EMD
  - `J66EKKTD_E095`：time-varying vibration transfer path, load zone passing and time-varying angles(Complex Systems) motivates Hilbert Transform
  - `J66EKKTD_E096`：time-varying vibration transfer path, load zone passing and time-varying angles(Complex Systems) motivates Peak Matching
