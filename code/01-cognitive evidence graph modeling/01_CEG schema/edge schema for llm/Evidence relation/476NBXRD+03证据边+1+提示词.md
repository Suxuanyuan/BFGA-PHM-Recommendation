...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `476NBXRD_E084` | `motivates` | 13-Noise Level | random white noise with amplitude of 0.05(High Noise) |  | 15-Data Preprocessing Algorithm | VMD |  |
| 2 | `476NBXRD_E085` | `motivates` | 13-Noise Level | random white noise with amplitude of 0.05(High Noise) |  | 16-Feature Extraction Algorithm | Least square method |  |
| 3 | `476NBXRD_E086` | `motivates` | 09-Problem Scenario | feature extraction for nonlinear and non-stationary vibration signals(Other) |  | 15-Data Preprocessing Algorithm | VMD |  |
| 4 | `476NBXRD_E087` | `motivates` | 09-Problem Scenario | feature extraction for nonlinear and non-stationary vibration signals(Other) |  | 16-Feature Extraction Algorithm | Least square method |  |
| 5 | `476NBXRD_E088` | `motivates` | 09-Problem Scenario | feature extraction for nonlinear and non-stationary vibration signals(Other) |  | 17-Core Classifier Algorithm | SVM |  |
| 6 | `476NBXRD_E089` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Least square method |  |
| 7 | `476NBXRD_E090` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | SVM |  |
| 8 | `476NBXRD_E091` | `motivates` | 06-Fault Severity | groove diameter of 0.053 mm(Single Severity) |  | 17-Core Classifier Algorithm | SVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `476NBXRD_E086`：feature extraction for nonlinear and non-stationary vibration signals(Other) motivates VMD
  - `476NBXRD_E087`：feature extraction for nonlinear and non-stationary vibration signals(Other) motivates Least square method
  - `476NBXRD_E088`：feature extraction for nonlinear and non-stationary vibration signals(Other) motivates SVM
