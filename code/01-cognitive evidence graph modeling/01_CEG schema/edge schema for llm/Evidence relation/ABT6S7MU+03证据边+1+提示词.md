...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `ABT6S7MU_E080` | `motivates` | 13-Noise Level | strong background noise, Gaussian white noise(High Noise) |  | 15-Data Preprocessing Algorithm | Order statistic filter |  |
| 2 | `ABT6S7MU_E081` | `motivates` | 13-Noise Level | strong background noise, Gaussian white noise(High Noise) |  | 16-Feature Extraction Algorithm | Flexible frequency slice wavelet transform |  |
| 3 | `ABT6S7MU_E082` | `motivates` | 09-Problem Scenario | noise robustness, low frequency disturbance(Uncertainty) |  | 15-Data Preprocessing Algorithm | Order statistic filter |  |
| 4 | `ABT6S7MU_E083` | `motivates` | 09-Problem Scenario | noise robustness, low frequency disturbance(Uncertainty) |  | 16-Feature Extraction Algorithm | Flexible frequency slice wavelet transform |  |
| 5 | `ABT6S7MU_E084` | `motivates` | 09-Problem Scenario | noise robustness, low frequency disturbance(Uncertainty) |  | 17-Core Classifier Algorithm | Hilbert transform |  |
| 6 | `ABT6S7MU_E085` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Flexible frequency slice wavelet transform |  |
| 7 | `ABT6S7MU_E086` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Hilbert transform |  |
| 8 | `ABT6S7MU_E087` | `motivates` | 06-Fault Severity | 0.25 mm deep and 0.7 mm wide(Single Severity) |  | 17-Core Classifier Algorithm | Hilbert transform |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `ABT6S7MU_E082`：noise robustness, low frequency disturbance(Uncertainty) motivates Order statistic filter
  - `ABT6S7MU_E083`：noise robustness, low frequency disturbance(Uncertainty) motivates Flexible frequency slice wavelet transform
  - `ABT6S7MU_E084`：noise robustness, low frequency disturbance(Uncertainty) motivates Hilbert transform
