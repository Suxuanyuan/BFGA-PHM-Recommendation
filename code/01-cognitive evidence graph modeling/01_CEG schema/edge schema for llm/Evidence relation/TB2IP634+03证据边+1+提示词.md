...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `TB2IP634_E096` | `motivates` | 09-Problem Scenario | noise-induced interference(Uncertainty) |  | 19-Training Optimization Algorithm | Particle Swarm Optimization |  |
| 2 | `TB2IP634_E097` | `motivates` | 12-Training Data Availability | 19 training samples for each group(Scarce) |  | 19-Training Optimization Algorithm | Particle Swarm Optimization |  |
| 3 | `TB2IP634_E098` | `motivates` | 13-Noise Level | vibration signals inevitably contain a significant degree of noise(High Noise) |  | 15-Data Preprocessing Algorithm | Wavelet thresholding denoising |  |
| 4 | `TB2IP634_E099` | `motivates` | 13-Noise Level | vibration signals inevitably contain a significant degree of noise(High Noise) |  | 16-Feature Extraction Algorithm | CEEMDAN |  |
| 5 | `TB2IP634_E100` | `motivates` | 09-Problem Scenario | noise-induced interference(Uncertainty) |  | 15-Data Preprocessing Algorithm | Wavelet thresholding denoising |  |
| 6 | `TB2IP634_E101` | `motivates` | 09-Problem Scenario | noise-induced interference(Uncertainty) |  | 16-Feature Extraction Algorithm | CEEMDAN |  |
| 7 | `TB2IP634_E102` | `motivates` | 09-Problem Scenario | noise-induced interference(Uncertainty) |  | 17-Core Classifier Algorithm | LSSVM |  |
| 8 | `TB2IP634_E103` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | CEEMDAN |  |
| 9 | `TB2IP634_E104` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | LSSVM |  |
| 10 | `TB2IP634_E105` | `motivates` | 06-Fault Severity | 0.1778, 0.3556, and 0.5334 mm(Multiple Severities) |  | 17-Core Classifier Algorithm | LSSVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `TB2IP634_E096`：noise-induced interference(Uncertainty) motivates Particle Swarm Optimization
  - `TB2IP634_E100`：noise-induced interference(Uncertainty) motivates Wavelet thresholding denoising
  - `TB2IP634_E101`：noise-induced interference(Uncertainty) motivates CEEMDAN
  - `TB2IP634_E102`：noise-induced interference(Uncertainty) motivates LSSVM
