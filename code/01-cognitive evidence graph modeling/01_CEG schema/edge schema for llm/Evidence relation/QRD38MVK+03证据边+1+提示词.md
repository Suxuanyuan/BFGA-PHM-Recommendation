...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `QRD38MVK_E088` | `motivates` | 13-Noise Level | Moving Average Filter decreases the noise in discrete time signals(High Noise) |  | 15-Data Preprocessing Algorithm | Moving Average Filter |  |
| 2 | `QRD38MVK_E089` | `motivates` | 13-Noise Level | Moving Average Filter decreases the noise in discrete time signals(High Noise) |  | 16-Feature Extraction Algorithm | HHT |  |
| 3 | `QRD38MVK_E090` | `motivates` | 09-Problem Scenario | inconspicuous gear fault classification(Other) |  | 15-Data Preprocessing Algorithm | Moving Average Filter |  |
| 4 | `QRD38MVK_E091` | `motivates` | 09-Problem Scenario | inconspicuous gear fault classification(Other) |  | 16-Feature Extraction Algorithm | HHT |  |
| 5 | `QRD38MVK_E092` | `motivates` | 09-Problem Scenario | inconspicuous gear fault classification(Other) |  | 17-Core Classifier Algorithm | SOM |  |
| 6 | `QRD38MVK_E093` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | HHT |  |
| 7 | `QRD38MVK_E094` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | SOM |  |
| 8 | `QRD38MVK_E095` | `motivates` | 06-Fault Severity | Single Severity |  | 17-Core Classifier Algorithm | SOM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `QRD38MVK_E090`：inconspicuous gear fault classification(Other) motivates Moving Average Filter
  - `QRD38MVK_E091`：inconspicuous gear fault classification(Other) motivates HHT
  - `QRD38MVK_E092`：inconspicuous gear fault classification(Other) motivates SOM
