...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `HAI9YZSD_E095` | `motivates` | 09-Problem Scenario | interdomain distribution discrepancy(Distribution Discrepancy) |  | 19-Training Optimization Algorithm | CORAL |  |
| 2 | `HAI9YZSD_E096` | `motivates` | 09-Problem Scenario | insufficient labeled data(Small Fault Samples) |  | 19-Training Optimization Algorithm | CORAL |  |
| 3 | `HAI9YZSD_E097` | `motivates` | 12-Training Data Availability | each operating condition contains 1500 samples (CWRU) and 600 samples (JNU)(Sufficient) |  | 19-Training Optimization Algorithm | CORAL |  |
| 4 | `HAI9YZSD_E098` | `motivates` | 13-Noise Level | Normal |  | 16-Feature Extraction Algorithm | 1D-CNN |  |
| 5 | `HAI9YZSD_E099` | `motivates` | 09-Problem Scenario | interdomain distribution discrepancy(Distribution Discrepancy) |  | 16-Feature Extraction Algorithm | 1D-CNN |  |
| 6 | `HAI9YZSD_E100` | `motivates` | 09-Problem Scenario | insufficient labeled data(Small Fault Samples) |  | 16-Feature Extraction Algorithm | 1D-CNN |  |
| 7 | `HAI9YZSD_E101` | `motivates` | 09-Problem Scenario | interdomain distribution discrepancy(Distribution Discrepancy) |  | 17-Core Classifier Algorithm | Softmax |  |
| 8 | `HAI9YZSD_E102` | `motivates` | 09-Problem Scenario | insufficient labeled data(Small Fault Samples) |  | 17-Core Classifier Algorithm | Softmax |  |
| 9 | `HAI9YZSD_E103` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | 1D-CNN |  |
| 10 | `HAI9YZSD_E104` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Softmax |  |
| 11 | `HAI9YZSD_E105` | `motivates` | 06-Fault Severity | 0.007 inch, 0.014 inch, and 0.021 inch fault diameters(Multiple Severities) |  | 17-Core Classifier Algorithm | Softmax |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `HAI9YZSD_E095`：interdomain distribution discrepancy(Distribution Discrepancy) motivates CORAL
  - `HAI9YZSD_E096`：insufficient labeled data(Small Fault Samples) motivates CORAL
  - `HAI9YZSD_E099`：interdomain distribution discrepancy(Distribution Discrepancy) motivates 1D-CNN
  - `HAI9YZSD_E100`：insufficient labeled data(Small Fault Samples) motivates 1D-CNN
  - `HAI9YZSD_E101`：interdomain distribution discrepancy(Distribution Discrepancy) motivates Softmax
  - `HAI9YZSD_E102`：insufficient labeled data(Small Fault Samples) motivates Softmax
