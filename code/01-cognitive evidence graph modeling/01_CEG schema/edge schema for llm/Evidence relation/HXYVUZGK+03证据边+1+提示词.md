...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `HXYVUZGK_E100` | `motivates` | 13-Noise Level | Gaussian white noise from 10% to 40%(High Noise) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 2 | `HXYVUZGK_E101` | `motivates` | 13-Noise Level | Gaussian white noise from 10% to 40%(High Noise) |  | 16-Feature Extraction Algorithm | Power Spectral Density |  |
| 3 | `HXYVUZGK_E102` | `motivates` | 09-Problem Scenario | noise tolerance / noise robustness(Uncertainty) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 4 | `HXYVUZGK_E103` | `motivates` | 09-Problem Scenario | interpretability of the decision tree(Trustworthiness / Interpretability) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 5 | `HXYVUZGK_E104` | `motivates` | 09-Problem Scenario | noise tolerance / noise robustness(Uncertainty) |  | 16-Feature Extraction Algorithm | Power Spectral Density |  |
| 6 | `HXYVUZGK_E105` | `motivates` | 09-Problem Scenario | interpretability of the decision tree(Trustworthiness / Interpretability) |  | 16-Feature Extraction Algorithm | Power Spectral Density |  |
| 7 | `HXYVUZGK_E106` | `motivates` | 09-Problem Scenario | noise tolerance / noise robustness(Uncertainty) |  | 17-Core Classifier Algorithm | FMM-CART |  |
| 8 | `HXYVUZGK_E107` | `motivates` | 09-Problem Scenario | interpretability of the decision tree(Trustworthiness / Interpretability) |  | 17-Core Classifier Algorithm | FMM-CART |  |
| 9 | `HXYVUZGK_E108` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Power Spectral Density |  |
| 10 | `HXYVUZGK_E109` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | FMM-CART |  |
| 11 | `HXYVUZGK_E110` | `motivates` | 06-Fault Severity | one broken bar, two broken bars, 10% turn shorts, 5% unbalanced supply(Multiple Severities) |  | 17-Core Classifier Algorithm | FMM-CART |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `HXYVUZGK_E102`：noise tolerance / noise robustness(Uncertainty) motivates Normalization
  - `HXYVUZGK_E103`：interpretability of the decision tree(Trustworthiness / Interpretability) motivates Normalization
  - `HXYVUZGK_E104`：noise tolerance / noise robustness(Uncertainty) motivates Power Spectral Density
  - `HXYVUZGK_E105`：interpretability of the decision tree(Trustworthiness / Interpretability) motivates Power Spectral Density
  - `HXYVUZGK_E106`：noise tolerance / noise robustness(Uncertainty) motivates FMM-CART
  - `HXYVUZGK_E107`：interpretability of the decision tree(Trustworthiness / Interpretability) motivates FMM-CART
