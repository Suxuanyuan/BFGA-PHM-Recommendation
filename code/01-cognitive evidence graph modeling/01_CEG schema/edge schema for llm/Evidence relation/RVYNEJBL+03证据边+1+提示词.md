...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `RVYNEJBL_E119` | `motivates` | 09-Problem Scenario | irrelevant, noisy, and high dimensional features(Other) |  | 19-Training Optimization Algorithm | VRGA |  |
| 2 | `RVYNEJBL_E120` | `motivates` | 12-Training Data Availability | Each stage has 100 experimental data samples; 60 samples are used to train the fault diagnosis model, and the rest are used for the purpose of testing.(Sufficient) |  | 19-Training Optimization Algorithm | VRGA |  |
| 3 | `RVYNEJBL_E121` | `motivates` | 13-Noise Level | Frequency of the decomposed modes c1(t), c2(t), ..., cn(t) ranges from high to low, and it may include noise or interference of high frequency.(Normal) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 4 | `RVYNEJBL_E122` | `motivates` | 13-Noise Level | Frequency of the decomposed modes c1(t), c2(t), ..., cn(t) ranges from high to low, and it may include noise or interference of high frequency.(Normal) |  | 16-Feature Extraction Algorithm | EMD |  |
| 5 | `RVYNEJBL_E123` | `motivates` | 09-Problem Scenario | irrelevant, noisy, and high dimensional features(Other) |  | 15-Data Preprocessing Algorithm | Normalization |  |
| 6 | `RVYNEJBL_E124` | `motivates` | 09-Problem Scenario | irrelevant, noisy, and high dimensional features(Other) |  | 16-Feature Extraction Algorithm | EMD |  |
| 7 | `RVYNEJBL_E125` | `motivates` | 09-Problem Scenario | irrelevant, noisy, and high dimensional features(Other) |  | 17-Core Classifier Algorithm | SVM |  |
| 8 | `RVYNEJBL_E126` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | EMD |  |
| 9 | `RVYNEJBL_E127` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | SVM |  |
| 10 | `RVYNEJBL_E128` | `motivates` | 06-Fault Severity | weights 0.1–1.0 g, 1.5–2.4 g, 3.0–3.8 g, 4.8–5.7 g; sizes 7 mil, 14 mil, 21 mil(Multiple Severities) |  | 17-Core Classifier Algorithm | SVM |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `RVYNEJBL_E119`：irrelevant, noisy, and high dimensional features(Other) motivates VRGA
  - `RVYNEJBL_E123`：irrelevant, noisy, and high dimensional features(Other) motivates Normalization
  - `RVYNEJBL_E124`：irrelevant, noisy, and high dimensional features(Other) motivates EMD
  - `RVYNEJBL_E125`：irrelevant, noisy, and high dimensional features(Other) motivates SVM
