...
|------|---------|-----------|------------|------------|------------------|------------|------------|-------------------|
| 1 | `HVU9WHN5_E079` | `motivates` | 13-Noise Level | strong background noise environment(High Noise) |  | 15-Data Preprocessing Algorithm | Slip Matrix |  |
| 2 | `HVU9WHN5_E080` | `motivates` | 13-Noise Level | strong background noise environment(High Noise) |  | 16-Feature Extraction Algorithm | Singular Value Decomposition |  |
| 3 | `HVU9WHN5_E081` | `motivates` | 09-Problem Scenario | strong background noise environment(Uncertainty) |  | 15-Data Preprocessing Algorithm | Slip Matrix |  |
| 4 | `HVU9WHN5_E082` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 15-Data Preprocessing Algorithm | Slip Matrix |  |
| 5 | `HVU9WHN5_E083` | `motivates` | 09-Problem Scenario | strong background noise environment(Uncertainty) |  | 16-Feature Extraction Algorithm | Singular Value Decomposition |  |
| 6 | `HVU9WHN5_E084` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 16-Feature Extraction Algorithm | Singular Value Decomposition |  |
| 7 | `HVU9WHN5_E085` | `motivates` | 09-Problem Scenario | strong background noise environment(Uncertainty) |  | 17-Core Classifier Algorithm | Optimal FIR Filter |  |
| 8 | `HVU9WHN5_E086` | `motivates` | 09-Problem Scenario | incipient fault detection(Early Degradation Prediction) |  | 17-Core Classifier Algorithm | Optimal FIR Filter |  |
| 9 | `HVU9WHN5_E087` | `motivates` | 07-Compound Fault | No Compound Fault |  | 16-Feature Extraction Algorithm | Singular Value Decomposition |  |
| 10 | `HVU9WHN5_E088` | `motivates` | 07-Compound Fault | No Compound Fault |  | 17-Core Classifier Algorithm | Optimal FIR Filter |  |
| 11 | `HVU9WHN5_E089` | `motivates` | 06-Fault Severity | RMS(Multiple Severities) |  | 17-Core Classifier Algorithm | Optimal FIR Filter |  |
...
The following candidate edges are of the type "09-Problem Scenario → Nodes 15~19 (Algorithm)"; **at least 1 of them must be judged as "existing" and extracted as an evidence edge**:
  - `HVU9WHN5_E081`：strong background noise environment(Uncertainty) motivates Slip Matrix
  - `HVU9WHN5_E082`：incipient fault detection(Early Degradation Prediction) motivates Slip Matrix
  - `HVU9WHN5_E083`：strong background noise environment(Uncertainty) motivates Singular Value Decomposition
  - `HVU9WHN5_E084`：incipient fault detection(Early Degradation Prediction) motivates Singular Value Decomposition
  - `HVU9WHN5_E085`：strong background noise environment(Uncertainty) motivates Optimal FIR Filter
  - `HVU9WHN5_E086`：incipient fault detection(Early Degradation Prediction) motivates Optimal FIR Filter
